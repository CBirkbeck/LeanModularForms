\section{The multi-point Cauchy principal value}\label{sec:cpv}
...
\begin{definition}[Multi-point Cauchy principal value]
  \label{def:has-cauchy-pv-on}
  \lean{HasCauchyPVOn}
  \uses{def:closed-pwc1-immersion}
  \leanok
  Let $\gamma : [0,1] \to \mathbb{C}$ be a piecewise-$C^{1}$ path
  from $x$ to $y$, let $S \subseteq \mathbb{C}$ be a finite set, let
  ...
  \[
    ...
    \end{cases}
  \]
  We say that the \emph{multi-point Cauchy principal value of
