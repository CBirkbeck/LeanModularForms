\section{Paper conditions (A') and (B)}\label{sec:conditions}

The Hungerbühler--Wasem generalised residue theorem requires two
hypotheses on the geometry of the curve at crossings. \emph{Condition
(A')} controls the flatness of the curve transverse to its tangent at
each crossing; \emph{condition (B)} controls the relationship between
the crossing angle and the Laurent coefficients of $f$ at the
corresponding pole.

\begin{definition}[Condition (A')]
  \label{def:condition-a-prime}
  \lean{SatisfiesConditionA'}
  ...
  $\gamma$ is said to \emph{satisfy condition (A')} (relative to
  ...
  \begin{quote}
    ...
    is \emph{flat of order $\operatorname{poleOrder}(s)$} at
