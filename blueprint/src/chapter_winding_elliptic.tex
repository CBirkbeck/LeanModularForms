\section{Generalised winding numbers at the elliptic points}\label{sec:winding-elliptic}
...
five segments
\begin{itemize}
  \item segment 0: the upward vertical edge $\operatorname{Re} z = 1/2$,
    from $1/2 + iH$ down to $\rho + 1 = 1/2 + i\sqrt{3}/2$;
  \item segment 1: the right arc of the unit circle from
    $\rho + 1$ counterclockwise to $i$;
  \item segment 2: the left arc of the unit circle from $i$ to
    $\rho = -1/2 + i\sqrt{3}/2$;
  \item segment 3: the upward vertical edge $\operatorname{Re} z = -1/2$,
    from $\rho$ down to $-1/2 + iH$;
  \item segment 4: the horizontal top edge from $-1/2 + iH$ to
    $1/2 + iH$.
\end{itemize}
...
formula of \cref{def:generalized-winding-number}.

\medskip\noindent
Although the project's file docstrings advertise headline theorems
named \texttt{gWN\_fdBoundary\_H\_at\_*}, the actual finalised
content is delivered by the three Cauchy-principal-value-limit
theorems \texttt{pv\_integral\_at\_*\_tendsto}. By
\cref{def:generalized-winding-number} these PV-limit theorems
\emph{determine} $\operatorname{gWN}(\partial \mathcal{D}_H, p)$ at
...
\begin{theorem}[Generalised winding number at $i$]
  \label{thm:gwn-at-i}
  \lean{pv_integral_at_i_tendsto}
  \uses{def:generalized-winding-number, def:has-cauchy-pv-on}
  \leanok
  For every $H > 1$,
  \[
...
\begin{theorem}[Generalised winding number at $\rho$]
  \label{thm:gwn-at-rho}
  \lean{pv_integral_at_rho_tendsto}
  \uses{def:generalized-winding-number, def:has-cauchy-pv-on}
  \leanok
...
\begin{theorem}[Generalised winding number at $\rho + 1$]
  \label{thm:gwn-at-rho-plus-one}
  \lean{pv_integral_at_rho_plus_one_tendsto}
  \uses{def:generalized-winding-number, def:has-cauchy-pv-on}
  \leanok
  For every $H > \sqrt{3}/2$,
  \[
