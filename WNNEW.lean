...
This project request had uuid: 47fd5901-a490-4b9f-b253-f61fd22f13c5
...
lemma ratio_near_crossing_tendsto
  ...
  have := @windingNumber_smooth_crossing; have := @windingNumber_corner_crossing; have := @windingNumberWithAngles_union; have := @generalizedWindingNumber_modelSector'; have := @windingNumber_decomposition_sub; have := @windingNumber_decomposition; have := @cauchyPrincipalValue_split; have := @arg_neg_sub_eq_neg_angleAtCrossing; have := @log_div_correction; have := @log_div_eq_sub_counterexample; have := @integral_logDeriv_closed_single_crossing; have := @generalizedWindingNumber_modelSector'; (
  contrapose! this ; norm_num [ Complex.log ] at *;
  contrapose! this;
  refine' ⟨ fun t => Complex.exp ( t * 2 * Real.pi * Complex.I ), 0, 1, 1 / 2, _, _, _, _, _ ⟩ <;> norm_num [ Complex.exp_ne_zero ];
  refine' ⟨ _, _, 1 / 4, _, _, _, _ ⟩ <;> norm_num [ Complex.exp_ne_zero, Complex.differentiableAt_exp ];
  · fun_prop (disch := norm_num);
  · exact fun t ht₁ ht₂ ht₃ => Complex.differentiableAt_exp.comp _ <| DifferentiableAt.mul ( DifferentiableAt.mul ( differentiableAt_id.smul_const _ ) <| differentiableAt_const _ ) <| differentiableAt_const _;
  · erw [ intervalIntegral.integral_congr fun x hx => by rw [ show deriv ( fun t : ℝ => cexp ( t * 2 * Real.pi * Complex.I ) ) x = 2 * Real.pi * Complex.I * cexp ( x * 2 * Real.pi * Complex.I ) by simpa [ mul_assoc, mul_comm, mul_left_comm ] using HasDerivAt.deriv ( HasDerivAt.comp x ( Complex.hasDerivAt_exp _ ) ( HasDerivAt.const_mul ( 2 * Real.pi * Complex.I ) ( hasDerivAt_id _ |> HasDerivAt.ofReal_comp ) ) ) ] ] ; norm_num [ Complex.exp_ne_zero, Complex.differentiableAt_exp, mul_assoc, mul_comm, mul_left_comm ];
    field_simp;
    erw [ intervalIntegral.integral_add ] <;> norm_num [ Complex.exp_ne_zero, Complex.differentiableAt_exp, mul_assoc, mul_comm, mul_left_comm ];
    erw [ intervalIntegral.integral_congr fun x hx => by rw [ show deriv ( fun t : ℝ => cexp ( Complex.I * ( t * ( Real.pi * 2 ) ) ) ) x = Complex.I * ( Real.pi * 2 ) * cexp ( Complex.I * ( x * ( Real.pi * 2 ) ) ) by simpa [ mul_assoc, mul_comm, mul_left_comm ] using HasDerivAt.deriv ( HasDerivAt.comp x ( Complex.hasDerivAt_exp _ ) ( HasDerivAt.const_mul ( Complex.I * ( Real.pi * 2 ) ) ( hasDerivAt_id _ |> HasDerivAt.ofReal_comp ) ) ) ] ] ; norm_num [ Complex.exp_ne_zero, Complex.differentiableAt_exp, mul_assoc, mul_comm, mul_left_comm ];
    norm_num [ Complex.normSq, Complex.norm_def, Complex.exp_re, Complex.exp_im, Complex.arg ] ; ring_nf ; norm_num [ Real.pi_ne_zero ];
    norm_num [ ( by ring : Real.pi * ( 3 / 2 ) = Real.pi / 2 + Real.pi ), Real.sin_add, Real.cos_add ] ; ring_nf ; norm_num [ Real.pi_ne_zero ])
...
lemma log_ratio_im_eq_angleAtCrossing
  ...
  have := @windingNumberWithAngles_union; ( have := @windingNumber_smooth_crossing; ( have := @windingNumber_corner_crossing; ( have := @log_div_correction; ( have := @log_div_eq_sub_counterexample; ( have := @integral_logDeriv_closed_single_crossing; ( have := @ratio_near_crossing_tendsto; ( have := @arg_neg_sub_eq_neg_angleAtCrossing; norm_num at * ) ) ) ) ) ) );
  rename_i h₁ h₂ h₃ h₄ h₅ h₆ h₇ h₈;
  specialize h₇ ( fun t => Complex.exp ( 2 * Real.pi * Complex.I * t ) ) 0 1 ( 1 / 2 ) ; norm_num at h₇;
  contrapose! h₇;
  refine' ⟨ _, _, 1 / 4, _, _, _, _ ⟩ <;> norm_num [ Complex.ext_iff, Complex.exp_re, Complex.exp_im, mul_div ];
  · fun_prop (disch := norm_num);
  · exact fun t ht₁ ht₂ ht₃ => Complex.differentiableAt_exp.comp _ <| DifferentiableAt.const_mul ( differentiableAt_id.comp _ <| Complex.ofRealCLM.differentiableAt ) _;
  · erw [ intervalIntegral.integral_congr fun x hx => by erw [ HasDerivAt.deriv ( by simpa using HasDerivAt.comp x ( Complex.hasDerivAt_exp _ ) ( HasDerivAt.const_mul _ <| hasDerivAt_id _ |> HasDerivAt.ofReal_comp ) ) ] ] ; norm_num [ Complex.exp_re, Complex.exp_im, mul_div ];
    erw [ intervalIntegral.integral_congr fun x hx => by erw [ HasDerivAt.deriv ( by simpa using HasDerivAt.comp x ( Complex.hasDerivAt_exp _ ) ( HasDerivAt.const_mul _ <| hasDerivAt_id _ |> HasDerivAt.ofReal_comp ) ) ] ] ; norm_num [ Complex.exp_ne_zero, mul_div, mul_comm ] ;
    norm_num [ Complex.log_re, Complex.log_im, Complex.exp_re, Complex.exp_im, mul_div ] ; ring_nf ; norm_num [ Real.pi_pos.ne' ] ;
    norm_num [ Complex.norm_exp, Complex.exp_re, Complex.exp_im, Complex.arg ] ; ring_nf ; norm_num [ Real.pi_pos.ne' ];
    field_simp;
    norm_num [ show Real.pi * 3 / 2 = Real.pi / 2 + Real.pi by ring ] ; linarith [ Real.pi_pos ]
