...
This project request had uuid: 4f50935c-d9cb-4a2b-9a5b-57c5e0f06e44
  ...
  have h_deriv : HasDerivAt (fun s' => ∫ t in a..b, F s' t) (J b s - J a s) s := by
    ...
    have h_param : HasDerivAt (fun s' => ∫ t in a..b, F s' t) (∫ t in a..b, deriv (fun t' => J t' s) t) s := by
      ...
      have h_diff : ∀ᵐ t ∂volume, t ∈ Ι a b → ∀ s' ∈ Metric.ball s ε,
        ...
        -- Since $H$ is continuously differentiable, its derivatives are continuous. Therefore, the product $f(H(t, s')) * \deriv H(t', s') t$ is differentiable with respect to $s''$.
        have h_diff_s'' : ∀ t ∈ Set.Icc a b, ∀ s' ∈ Set.Icc (s - ε) (s + ε), DifferentiableAt ℝ (fun s'' => F s'' t) s' := by
          intros t ht s' hs'
          have h_diff_s'' : DifferentiableAt ℝ (fun s'' => f (H (t, s''))) s' := by
            exact DifferentiableAt.comp s' ( hf_differentiable.differentiableAt.restrictScalars ℝ ) ( hH_diff.differentiableAt.comp s' ( differentiableAt_const _ |> DifferentiableAt.prodMk <| differentiableAt_id ) )
          generalize_proofs at *; (
          exact DifferentiableAt.mul h_diff_s'' ( h_partialT.differentiable le_rfl |> Differentiable.differentiableAt |> DifferentiableAt.comp _ <| differentiableAt_const _ |> DifferentiableAt.prodMk <| differentiableAt_id ) |> DifferentiableAt.congr_of_eventuallyEq <| Filter.eventuallyEq_of_mem ( Metric.ball_mem_nhds _ zero_lt_one ) fun x hx => rfl;);
        exact Filter.Eventually.of_forall fun t ht s' hs' => DifferentiableAt.hasDerivAt ( h_diff_s'' t ( h_uIoc_subset ht ) s' ( h_ball_subset hs' ) ) -- HasDerivAt from product rule (h_g_diff.mul h_h_diff).hasDerivAt
