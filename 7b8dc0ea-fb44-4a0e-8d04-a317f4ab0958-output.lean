...
This project request had uuid: 7b8dc0ea-fb44-4a0e-8d04-a317f4ab0958
...
theorem numerator_big_O_squared (t₀ : ℝ)
  ...
  -- x(t) = (t - t₀)·ẋ(t₀) + O((t-t₀)²)
  -- y(t) = (t - t₀)·ẏ(t₀) + O((t-t₀)²)
  -- x·ẏ - y·ẋ = (t-t₀)²·(ẋ(t₀)·ẏ(t₀) - ẏ(t₀)·ẋ(t₀)) + O((t-t₀)³) = O((t-t₀)²)
  sorry

/- Aristotle failed to find a proof. -/
/-- Alternative formulation using Asymptotics.IsBigO -/
theorem numerator_isBigO_squared (t₀ : ℝ)
    (hγ_zero : γ.toFun t₀ = 0)
    (hLip : LipschitzOnWith 1 (deriv γ.toFun) (Icc γ.a γ.b)) :
    (fun t => (γ.toFun t).re * (deriv γ.toFun t).im -
              (γ.toFun t).im * (deriv γ.toFun t).re) =O[𝓝 t₀]
    (fun t => (t - t₀)^2) := by
  sorry
