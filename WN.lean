...
This project request had uuid: 90573722-7732-454c-a8bd-f3c79343edee
...
- theorem windingNumberWithAngles_union (γ : PiecewiseC1Immersion) (z₀ : ℂ)
    ...
    windingNumberWithAngles' γ z₀ T hT_in hT_at
...
theorem windingNumberWithAngles_union (γ : PiecewiseC1Immersion) (z₀ : ℂ)
  ...
  rw [ show ( Finset.univ : Finset { x : ℝ // x ∈ S ∪ T } ) = Finset.image ( fun x : { x : ℝ // x ∈ S } => ⟨ x.val, by aesop ⟩ ) ( Finset.univ : Finset { x : ℝ // x ∈ S } ) ∪ Finset.image ( fun x : { x : ℝ // x ∈ T } => ⟨ x.val, by aesop ⟩ ) ( Finset.univ : Finset { x : ℝ // x ∈ T } ) from ?_ ];
  · rw [ Finset.sum_union, Finset.sum_image, Finset.sum_image ] <;> norm_num [ Finset.disjoint_left, hST ];
    exact fun a ha x hx hx' => by rw [ Finset.disjoint_left ] at hST; aesop;
  · ext ⟨ x, hx ⟩ ; aesop
