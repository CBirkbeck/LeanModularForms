...
def SatisfiesConditionA (γ : PiecewiseC1Immersion) (_f : ℂ → ℂ) (S0 : Finset ℂ) : Prop :=
...
def SatisfiesConditionA' (γ : PiecewiseC1Immersion) (_f : ℂ → ℂ) (S0 : Finset ℂ)
    (poleOrder : ℂ → ℕ) : Prop :=
  ∀ s ∈ S0, ∀ t₀ ∈ Icc γ.a γ.b, γ.toFun t₀ = s →
    t₀ ∈ Ioo γ.a γ.b →
