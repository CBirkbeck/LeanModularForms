...
lemma fdBoundary_H_eq_seg2_H {t : ℝ} (H : ℝ)
  ...
  simp only [fdBoundary_H, show ¬t ≤ 1 from not_le.mpr ht1, ht2, ite_true, ite_false,
    fdBoundary_seg2_H, fdBoundary_seg2]

lemma fdBoundary_H_eq_seg3_H {t : ℝ} (H : ℝ)
    (ht2 : 2 < t) (ht3 : t ≤ 3) :
    fdBoundary_H H t = fdBoundary_seg3_H t := by
  simp only [fdBoundary_H, show ¬t ≤ 1 from not_le.mpr (by linarith),
...
lemma fdBoundary_H_eq_seg5_H {H t : ℝ} (ht4 : 4 < t) :
  ...
  simp only [fdBoundary_H, show ¬t ≤ 1 from not_le.mpr (by linarith),
    ...
    show ¬t ≤ 4 from not_le.mpr ht4, ite_false, fdBoundary_seg5_H]
...
lemma fdBoundary_eq_seg5 {t : ℝ} (ht4 : 4 < t) :
  ...
  simp only [fdBoundary, show ¬t ≤ 1 from not_le.mpr (by linarith),
    show ¬t ≤ 2 from not_le.mpr (by linarith), show ¬t ≤ 3 from not_le.mpr (by linarith),
    show ¬t ≤ 4 from not_le.mpr ht4, ite_false, fdBoundary_seg5]
