...
theorem exit_arg_tendsto_right
  ...
  refine (((Complex.continuousAt_arg hL_slit).tendsto.comp
...
theorem normalized_chord_close_right
  ...
  have h_div : Tendsto
      ...
      (𝓝[>] t₀) (𝓝 (L / L)) :=
    (chord_div_t_tendsto_right h_deriv h_at).div_const _
  ...
  have h_tendsto_one : Tendsto
      (fun t : ℝ => (γ t - s) / (L * ((t - t₀ : ℝ) : ℂ))) (𝓝[>] t₀) (𝓝 1) :=
    h_div.congr' h_eq
...
theorem normalized_chord_close_left
  ...
  have h_div : Tendsto
      (fun t : ℝ => (γ t - s) / ((t - t₀ : ℝ) : ℂ) / L)
      (𝓝[<] t₀) (𝓝 (L / L)) :=
    (chord_div_t_tendsto_left h_deriv h_at).div_const _
  rw [div_self hL] at h_div
  have h_eq : ∀ᶠ t in 𝓝[<] t₀,
      (γ t - s) / ((t - t₀ : ℝ) : ℂ) / L =
        (γ t - s) / (L * ((t - t₀ : ℝ) : ℂ)) := by
...
theorem annulus_FTC_right
  ...
  segment_log_FTC hab hP_count hγ_cont hγ_diff h_ne h_slit h_int
...
theorem right_annular_log_diff
  ...
  have h_slit_ab : ∀ t ∈ Icc a b, (γf t - s) / (γf a - s) ∈ Complex.slitPlane :=
    fun t ht_in => h_slit_R a t ha_gt ht_in.1 (le_trans ht_in.2 hb_le)
  have ha_ne : γf a - s ≠ 0 := by
    intro h_eq
    have h_in_01 : a ∈ Icc (0 : ℝ) 1 := ⟨by linarith, by linarith⟩
  ...
  have hγ_diff : ∀ t ∈ Ioo a b \ P, HasDerivAt γf (deriv γf t) t := by
    ...
    exact (γ.toPwC1Immersion.toPiecewiseC1Path.differentiable_off t ht_in_Ioo ht.2).hasDerivAt
...
theorem left_annular_log_diff
  ...
  have h_slit_ab : ∀ t ∈ Icc a b, (γf t - s) / (γf a - s) ∈ Complex.slitPlane :=
    fun t ht_in => h_slit_L a t ha_ge ht_in.1 (lt_of_le_of_lt ht_in.2 hb_lt)
  have ha_ne : γf a - s ≠ 0 := by
    intro h_eq
    have h_in_01 : a ∈ Icc (0 : ℝ) 1 := ⟨hr_le_t₀, by linarith⟩
    have : a = t₀ := h_unique a h_in_01 (sub_eq_zero.mp h_eq)
    simp only [a] at this
    linarith
  have hγ_cont : ContinuousOn γf (Icc a b) :=
  ...
  have hγ_diff : ∀ t ∈ Ioo a b \ P, HasDerivAt γf (deriv γf t) t := by
    intro t ht
    have ht_in_Ioo : t ∈ Ioo (0 : ℝ) 1 :=
      ⟨by linarith [ht.1.1, hr_le_t₀], by linarith [ht.1.2, hb_lt]⟩
    exact (γ.toPwC1Immersion.toPiecewiseC1Path.differentiable_off t ht_in_Ioo ht.2).hasDerivAt
  have h_int : IntervalIntegrable
      (fun t => deriv γf t / (γf t - s)) MeasureTheory.volume a b := by
...
theorem arg_right_annular_tendsto
  ...
  have h_rw : (γ (t₀ + r) - s) * (((δ_right ε : ℝ) : ℂ) / (γ (t₀ + δ_right ε) - s)) =
      ((δ_right ε : ℝ) : ℂ) * ((γ (t₀ + r) - s) / (γ (t₀ + δ_right ε) - s)) := by ring
  rw [h_rw]
...
theorem DerivedAsymmetricCutoffs.δ_right_tendsto_zero
  ...
  · rw [Metric.tendsto_nhds]
    ...
    filter_upwards [Ioo_mem_nhdsGT hε_star_pos] with ε hε
...
private theorem cpvFullSetup
  ...
  have h_γPlus_div_LR : (γf (t₀ + r) - s) / L_R ∈ Complex.slitPlane := by
    ...
    have h_div_eq : (γf (t₀ + r) - s) / L_R =
        ((r : ℝ) : ℂ) * ((γf (t₀ + r) - s) / (L_R * ((r : ℝ) : ℂ))) := by field_simp
    ...
    rw [h_re_calc]
    have h1 : 0 < r * (3/4 : ℝ) := by positivity
    exact lt_of_lt_of_le h1 (mul_le_mul_of_nonneg_left h_re_chord_close hr_pos.le)
  have h_LL_neg_div_γMinus : (-L_L) / (γf (t₀ - r) - s) ∈ Complex.slitPlane := by
    ...
    have h_neg_inv_q_close : ‖(-1 / q) - 1‖ ≤ 1/3 := by
      ...
      have h_one_plus_q : ‖(1 : ℂ) + q‖ = ‖-q - 1‖ := by
        ...
        rw [this, norm_neg]
      rw [h_one_plus_q, div_le_iff₀ (norm_pos_iff.mpr hq_ne)]
    ...
    have h_re_ge : (2/3 : ℝ) ≤ (-1 / q).re := by linarith [(abs_le.mp h_abs_re_le).1]
    have h_inv_r_pos : 0 < 1/r := by positivity
    have h1 : 0 < (1/r) * (2/3 : ℝ) := by positivity
