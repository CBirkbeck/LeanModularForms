...
private lemma correction_eventuallyEq_analyticExt {S : Finset ℂ} {z : ℂ}
  ...
  apply Filter.mem_of_superset <| Filter.inter_mem (hV_open.mem_nhds hz_V)
    ((S.erase z).finite_toSet.isClosed.isOpen_compl.mem_nhds hz_not_erase)
  intro w ⟨hw_V, hw_erase⟩
  change correction w = g_z w
  by_cases hwz : w = z
  · rw [hwz, h_at_z]
  · have hw_not_S : w ∉ (↑S : Set ℂ) :=
      fun hmem => hw_erase (Finset.mem_erase.mpr ⟨hwz, hmem⟩)
    exact (if_neg hw_not_S).trans (hV_eq ⟨hw_V, hwz⟩)

private lemma correction_eventuallyEq_rem {S : Finset ℂ} {z : ℂ}
    (rem : ℂ → ℂ) (hzS : z ∉ (↑S : Set ℂ)) :
    (fun w => if w ∈ (↑S : Set ℂ) then limUnder (𝓝[≠] w) rem else rem w) =ᶠ[𝓝 z] rem :=
  Filter.mem_of_superset (S.finite_toSet.isClosed.isOpen_compl.mem_nhds hzS) fun _ hw => if_neg hw
...
theorem sub_principalPartSum_corrected_differentiableOn {f : ℂ → ℂ} {U : Set ℂ}
  ...
  · intro z hz
    ...
    · exact ((((hf_diff z ⟨hz, hzS⟩).differentiableAt
          ((hU_open.sdiff S.finite_toSet.isClosed).mem_nhds ⟨hz, hzS⟩)).sub
            (principalPartSum_differentiableAt (hz := hzS))).congr_of_eventuallyEq
