...
theorem exit_time_right_exists
    {γ : ℝ → ℂ} {t₀ : ℝ} {s : ℂ} {δ : ℝ} (hδ : 0 < δ)
    (hγ_cont : ContinuousOn γ (Icc t₀ (t₀ + δ))) (h_s : γ t₀ = s)
    {ε : ℝ} (hε_pos : 0 ≤ ε) (hε_le : ε ≤ ‖γ (t₀ + δ) - s‖) :
    ∃ t_ε ∈ Icc t₀ (t₀ + δ), ‖γ t_ε - s‖ = ε :=
  intermediate_value_Icc (by linarith) (hγ_cont.sub continuousOn_const).norm
    (by simpa [h_s] using ⟨hε_pos, hε_le⟩)
...
theorem exit_time_left_exists
    {γ : ℝ → ℂ} {t₀ : ℝ} {s : ℂ} {δ : ℝ} (hδ : 0 < δ)
    (hγ_cont : ContinuousOn γ (Icc (t₀ - δ) t₀)) (h_s : γ t₀ = s)
    {ε : ℝ} (hε_pos : 0 ≤ ε) (hε_le : ε ≤ ‖γ (t₀ - δ) - s‖) :
    ∃ t_ε ∈ Icc (t₀ - δ) t₀, ‖γ t_ε - s‖ = ε :=
  intermediate_value_Icc' (by linarith) (hγ_cont.sub continuousOn_const).norm
    (by simpa [h_s] using ⟨hε_pos, hε_le⟩)
...
theorem exists_right_modulus
    ...
    ∃ η ∈ Ioc (0 : ℝ) δ, ∀ t ∈ Icc t₀ (t₀ + η), ‖γ t - s‖ < ε := by
  have h_eventually : ∀ᶠ t in 𝓝[Icc t₀ (t₀ + δ)] t₀, ‖γ t - s‖ < ε :=
    (((hγ_cont t₀ ⟨le_rfl, by linarith⟩).sub continuousWithinAt_const).norm.tendsto).eventually_lt_const
      (by simp [h_s, hε_pos])
  obtain ⟨η₀, hη₀_pos, hη₀⟩ := Metric.nhdsWithin_basis_ball.eventually_iff.mp h_eventually
  refine ⟨min (η₀ / 2) (δ / 2), ⟨by positivity, min_le_of_right_le (by linarith)⟩, fun t ht => ?_⟩
...
theorem exists_left_modulus
    ...
    (hγ_cont : ContinuousOn γ (Icc (t₀ - δ) t₀))
    (h_s : γ t₀ = s) {ε : ℝ} (hε_pos : 0 < ε) :
    ∃ η ∈ Ioc (0 : ℝ) δ, ∀ t ∈ Icc (t₀ - η) t₀, ‖γ t - s‖ < ε := by
  have h_eventually : ∀ᶠ t in 𝓝[Icc (t₀ - δ) t₀] t₀, ‖γ t - s‖ < ε :=
    (((hγ_cont t₀ ⟨by linarith, le_rfl⟩).sub continuousWithinAt_const).norm.tendsto).eventually_lt_const
      (by simp [h_s, hε_pos])
  obtain ⟨η₀, hη₀_pos, hη₀⟩ := Metric.nhdsWithin_basis_ball.eventually_iff.mp h_eventually
  refine ⟨min (η₀ / 2) (δ / 2), ⟨by positivity, min_le_of_right_le (by linarith)⟩, fun t ht => ?_⟩
...
theorem firstExitTimeRight_mono_of_witness
    ...
    (h_witness : ∃ t ∈ Icc t₀ (t₀ + δ), ε₂ ≤ ‖γ t - s‖) :
    firstExitTimeRight γ t₀ δ s ε₁ ≤ firstExitTimeRight γ t₀ δ s ε₂ :=
  csInf_le_csInf ⟨t₀, firstExitTimeRight_set_lb γ t₀ δ ε₁ s⟩ h_witness
...
theorem firstExitTimeLeft_anti_of_witness
    (γ : ℝ → ℂ) (t₀ δ : ℝ) (s : ℂ) {ε₁ ε₂ : ℝ} (h_le : ε₁ ≤ ε₂)
    (h_witness : ∃ t ∈ Icc (t₀ - δ) t₀, ε₂ ≤ ‖γ t - s‖) :
    firstExitTimeLeft γ t₀ δ s ε₂ ≤ firstExitTimeLeft γ t₀ δ s ε₁ :=
  csSup_le_csSup ⟨t₀, firstExitTimeLeft_set_ub γ t₀ δ ε₁ s⟩ h_witness
    fun _ ⟨hmem, h_far⟩ => ⟨hmem, h_le.trans h_far⟩
