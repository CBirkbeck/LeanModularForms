...
private theorem pv_piecewise_measurable (g : ℂ → ℂ) (γ : PiecewiseC1Curve) (z₀ : ℂ)
    (h_integrable :
      IntervalIntegrable (fun t => g (γ.toFun t) * deriv γ.toFun t) volume γ.a γ.b) :
    ∀ᶠ ε in 𝓝[>] (0 : ℝ), AEStronglyMeasurable
      (fun t => if ε < ‖γ.toFun t - z₀‖ then g (γ.toFun t) * deriv γ.toFun t else 0)
      (volume.restrict (Ι γ.a γ.b)) := by
  filter_upwards [self_mem_nhdsWithin] with ε _
  rw [show Ι γ.a γ.b = Ioc γ.a γ.b from uIoc_of_le γ.hab.le]
  refine (h_integrable.aestronglyMeasurable.indicator
    (measurableSet_pv_support γ.toFun γ.a γ.b z₀ ε γ.continuous_toFun)).congr ?_
  filter_upwards [ae_restrict_mem measurableSet_Ioc] with t ht
  by_cases h : ε < ‖γ.toFun t - z₀‖
  · rw [indicator_of_mem (show t ∈ {t | ε < ‖γ.toFun t - z₀‖} ∩ Icc γ.a γ.b from
      ⟨h, Ioc_subset_Icc_self ht⟩), if_pos h]
  · rw [indicator_of_notMem (fun hmem => h hmem.1), if_neg h]

private theorem pv_piecewise_bound (γ : PiecewiseC1Curve) (z₀ : ℂ) (g : ℂ → ℂ) :
...
private theorem pv_piecewise_pointwise (γ : PiecewiseC1Curve) (z₀ : ℂ) (g : ℂ → ℂ)
    (h_finite_preimage : Set.Finite {t ∈ Icc γ.a γ.b | γ.toFun t = z₀}) :
  ...
  filter_upwards [h_finite_preimage.countable.ae_notMem _] with t ht ht_uIoc
...
private theorem pv_simple_pole_integrand_split (γ_fun : ℝ → ℂ) (z₀ c : ℂ) (g : ℂ → ℂ)
    (ε : ℝ) (t : ℝ) :
    (if ε < ‖γ_fun t - z₀‖ then (c / (γ_fun t - z₀) + g (γ_fun t)) * deriv γ_fun t else 0) =
    (if ε < ‖γ_fun t - z₀‖ then c / (γ_fun t - z₀) * deriv γ_fun t else 0) +
    (if ε < ‖γ_fun t - z₀‖ then g (γ_fun t) * deriv γ_fun t else 0) := by
  split_ifs <;> ring

private theorem pv_simple_pole_tendsto (γ : PiecewiseC1Immersion) (z₀ c : ℂ) (g : ℂ → ℂ)
    (Ls Lg : ℂ)
    ...
    (hLg : Tendsto (fun ε => ∫ t in γ.a..γ.b,
      if ε < ‖γ.toFun t - z₀‖ then g (γ.toFun t) * deriv γ.toFun t else 0)
      (𝓝[>] 0) (𝓝 Lg))
    (h_int : ∀ ε > 0, IntervalIntegrable (fun t =>
        if ε < ‖γ.toFun t - z₀‖ then c / (γ.toFun t - z₀) * deriv γ.toFun t else 0)
        volume γ.a γ.b ∧
  ...
  filter_upwards [self_mem_nhdsWithin] with ε hε
  symm
  exact intervalIntegral.integral_add (h_int ε (mem_Ioi.mp hε)).1 (h_int ε (mem_Ioi.mp hε)).2
