...
import LeanModularForms.ForMathlib.GeneralizedResidueTheory.Residue.Flatness
...
private lemma cpv_crossing_null (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) :
  ...
  have h_single_null : ∀ s ∈ S0,
      volume {t | t ∈ Icc γ.a γ.b ∧ γ.toFun t = s} = 0 := by
    intro s _
  ...
  have h_eq :
    ...
    · intro ⟨s, hs, hin, heq⟩
      exact ⟨hin, heq ▸ hs⟩
  rw [h_eq, measure_biUnion_null_iff (Set.Finite.countable (Finset.finite_toSet S0))]
  intro s hs
...
private lemma holomorphic_closed_integral_zero (U : Set ℂ) (hU : IsOpen U)
  ...
  have h_int :
      IntervalIntegrable (fun t => g (γ.toFun t) * deriv γ.toFun t) volume γ.a γ.b := by
...
private lemma single_pole_pv_base_exists
  ...
  have hL' : Tendsto (fun ε => c * ∫ t in γ.a..γ.b,
    ...
    convert hL using 1
    ext ε
    exact (cpv_integral_factor_const γ s c ε).symm
...
private lemma generalizedResidueTheorem'_crossing_formula
  ...
  calc cauchyPrincipalValueOn S0 f γ.toFun γ.a γ.b
    ...
    _ = ∑ s ∈ S0, (2 * Real.pi * I *
        ...
        Finset.sum_congr rfl h_single_pole_formula
    _ = 2 * Real.pi * I *
          ∑ s ∈ S0, generalizedWindingNumber' γ.toFun γ.a γ.b s *
            residueSimplePole f s := by
        rw [Finset.mul_sum]
...
theorem residueSimplePole_eq_of_decomposition (f : ℂ → ℂ) (z₀ c : ℂ) (g : ℂ → ℂ)
  ...
  have h_tend : Tendsto (fun z => c + (z - z₀) * g z) (𝓝[≠] z₀) (𝓝 c) := by
    have := (tendsto_const_nhds (x := c)).add h_prod
...
private lemma residueAt_eq_of_simple_pole_decomp (f : ℂ → ℂ) (z₀ c : ℂ) (g : ℂ → ℂ)
  ...
  have h_ci_inv : CircleIntegrable (fun z => (z - z₀)⁻¹) z₀ r :=
    circleIntegrable_sub_inv_iff.mpr (Or.inr (by
      rw [Metric.mem_sphere, dist_self, abs_of_pos hr_pos]
      exact hr_ne.symm))
...
lemma continuousAt_sum_remainder (S0 : Finset ℂ) (c : ℂ → ℂ) (s : ℂ) (hs : s ∈ S0) :
  ...
  have h_rem : (fun z => (∑ s' ∈ S0, c s' / (z - s')) - c s / (z - s)) =
    ...
    funext z
    rw [← Finset.add_sum_erase S0 (fun s' => c s' / (z - s')) hs]
    ring
  rw [h_rem]
