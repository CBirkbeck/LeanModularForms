  ...
  | succ n ih =>
    ...
    · obtain ⟨t₁, ht₁_mem⟩ := Finset.nonempty_of_ne_empty h_empty
      ...
      have h_card_cα : h_fin_cα.toFinset.card ≤ n := by
        have h_sub_finset : h_fin_cα.toFinset ⊆ h_fin_cd.toFinset.erase t₁ := fun t ht_mem =>
          Finset.mem_erase.mpr ⟨fun heq => ht₁_not_cα (heq ▸ h_fin_cα.mem_toFinset.mp ht_mem),
            h_fin_cd.mem_toFinset.mpr
      ...
      have h_card_βd : h_fin_βd.toFinset.card ≤ n := by
        have h_sub_finset : h_fin_βd.toFinset ⊆ h_fin_cd.toFinset.erase t₁ := fun t ht_mem =>
          Finset.mem_erase.mpr ⟨fun heq => ht₁_not_βd (heq ▸ h_fin_βd.mem_toFinset.mp ht_mem),
            h_fin_cd.mem_toFinset.mpr
