...
lemma pv_step_bound_ratio_two_uniform {γ : ℝ → ℂ} {a b t₀ : ℝ} {L : ℂ} (hab : a < b)
  ...
  obtain ⟨ρ, hρ_pos, h_far_bound⟩ := no_return_of_inj_continuous hδ₀δ₁_pos hγ_cont h_inj
  ...
  have h_sing_bound :
      ‖∫ t in a..b,
          if ε₂ < ‖γ t - γ t₀‖ ∧ ‖γ t - γ t₀‖ ≤ ε₁ then (↑(t - t₀) : ℂ)⁻¹ else 0‖ ≤
        Csing * ε₁ :=
    h_singular ε₁ ε₂ hε₂_pos hε₂_le h_ratio hε₁_lt_δ_sing
  have h_loc_for_rem : ∀ t ∈ Set.Icc a b, ‖γ t - γ t₀‖ ≤ ε₁ → |t - t₀| < min δ₀ δ_lo :=
    fun t ht hγ =>
      lt_of_lt_of_le (h_localize t ht hγ) (min_le_min_left δ₀ (min_le_left δ_lo δ_up))
  have h_rem_bound :
