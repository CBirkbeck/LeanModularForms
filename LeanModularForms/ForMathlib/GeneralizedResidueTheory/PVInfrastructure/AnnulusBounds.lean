...
lemma remainder_integral_bound_on_annulus {γ : ℝ → ℂ} {a b t₀ : ℝ} {C δ₀ δ₁ ε₁ ε₂ : ℝ} {L : ℂ}
  ...
  have hab : a < b :=
    (Set.mem_Ioo.mp hat₀).1.trans_le (Set.mem_Ioo.mp hat₀).2.le
...
lemma near_threshold_implies_r_in_shell {L_norm r ε K₀ R_max : ℝ} (hL_pos : 0 < L_norm)
...
lemma annulus_symmDiff_measure_bound {γ : ℝ → ℂ} {a b t₀ : ℝ} {L : ℂ} (_hab : a < b)
  ...
  have h_lower_bound : ∀ t, |t - t₀| < δ₁ → ‖γ t - γ t₀‖ ≥ ‖L‖ / 2 * |t - t₀| := by
    ...
    have h6 : |t - t₀| * (‖L‖ - K₀ * |t - t₀|) ≥ |t - t₀| * (‖L‖ / 2) :=
      mul_le_mul_of_nonneg_left h5.le (abs_nonneg _)
    calc ‖γ t - γ t₀‖
        ≥ |t - t₀| * (‖L‖ - K₀ * |t - t₀|) := h2
      _ ≥ |t - t₀| * (‖L‖ / 2) := h6
      _ = ‖L‖ / 2 * |t - t₀| := by ring
  ...
  have h_subset :
    ...
    have hxor' :
      ...
      · refine Or.inl ⟨⟨hγ_lo, hγ_hi⟩, ?_⟩
        intro ⟨ht_lo', ht_hi'⟩
        exact ht_not_tAnn ⟨ht_Icc, ht_localized, ht_lo', ht_hi'⟩
      · refine Or.inr ⟨⟨ht_lo, ht_hi⟩, ?_⟩
        intro ⟨hγ_lo', hγ_hi'⟩
        exact ht_not_γAnn ⟨ht_Icc, ht_localized, hγ_lo', hγ_hi'⟩
      ...
      · have ⟨_, _, _, ht_upper⟩ := ht_tAnn
        ...
        calc |t - t₀|
            ≤ ε₁ / ‖L‖ := by
              ...
              exact h1'
          _ ≤ 2 * ε₁ / ‖L‖ := by
              apply div_le_div_of_nonneg_right _ hL_nonneg
              linarith
