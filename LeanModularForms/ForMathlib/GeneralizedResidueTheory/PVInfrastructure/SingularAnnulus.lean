...
lemma norm_annulus_condition_iff {t₀ : ℝ} {L : ℂ} {ε₁ ε₂ : ℝ} (hL_pos : 0 < ‖L‖) (t : ℝ) :
    (ε₂ < ‖L‖ * |t - t₀| ∧ ‖L‖ * |t - t₀| ≤ ε₁) ↔
    (ε₂ / ‖L‖ < |t - t₀| ∧ |t - t₀| ≤ ε₁ / ‖L‖) := by
  constructor
  · rintro ⟨h1, h2⟩
...
lemma intervalIntegral_eq_zero_of_ae_eq_zero {a b : ℝ} {φ : ℝ → ℂ}
    (_hI : IntervalIntegrable φ volume a b)
...
lemma singular_tAnnLin_inside_interval {t₀ a b : ℝ} (hat₀ : t₀ ∈ Set.Ioo a b) {L : ℂ}
    (hL_pos : 0 < ‖L‖) {ε₁ : ℝ} (hε₁_small : ε₁ < ‖L‖ * min (t₀ - a) (b - t₀))
...
lemma singular_tAnnLin_cancel (t₀ : ℝ) {L : ℂ} (hL_pos : 0 < ‖L‖) (ε₁ ε₂ : ℝ)
...
lemma singular_symmDiff_sup_bound {t₀ c : ℝ} (hc_pos : 0 < c) {t : ℝ}
    (ht_lower : c ≤ |t - t₀|) :
    ‖(↑(t - t₀) : ℂ)⁻¹‖ ≤ 1 / c := by
  have ht_pos : (0 : ℝ) < |t - t₀| :=
...
lemma singular_annulus_f_lin_intervalIntegrable {t₀ : ℝ} {L : ℂ} {ε₁ ε₂ : ℝ}
...
private lemma lin_indicator_zero_left {a t₀ c₁ c₂ : ℝ} (_hc₂_nonneg : 0 ≤ c₂)
    (ha_lt_mc₂ : a < t₀ - c₂) {φ : ℝ → ℂ} (hI : IntervalIntegrable φ volume a (t₀ - c₂))
...
private lemma lin_indicator_zero_right {t₀ c₁ c₂ b : ℝ} (_hc₂_nonneg : 0 ≤ c₂)
    (hpc₂_lt_b : t₀ + c₂ < b) {φ : ℝ → ℂ} (hI : IntervalIntegrable φ volume (t₀ + c₂) b)
    (hφ_zero : ∀ t, ¬(c₁ < |t - t₀| ∧ |t - t₀| ≤ c₂) → φ t = 0) :
    ∫ t in (t₀ + c₂)..b, φ t = 0 :=
  intervalIntegral_eq_zero_of_ae_eq_zero hI (by
...
private lemma lin_indicator_eq_inv_left {t₀ c₁ c₂ : ℝ} (_hc₁_nonneg : 0 ≤ c₁)
...
private lemma lin_indicator_eq_inv_right {t₀ c₁ c₂ : ℝ} (_hc₁_nonneg : 0 ≤ c₁)
...
private lemma singular_annulus_lin_integral_zero {a b t₀ : ℝ} {L : ℂ} {ε₁ ε₂ : ℝ}
    (hL_pos : 0 < ‖L‖) (_hε₁_pos : 0 < ε₁) (hε₂_pos : 0 < ε₂) (hε₂_le : ε₂ ≤ ε₁)
...
lemma singular_annulus_bound_explicit {γ : ℝ → ℂ} {a b t₀ : ℝ} {L : ℂ} (hab : a < b)
  ...
  have hf_lin_meas : Measurable f_lin := by
    ...
    exact singular_annulus_f_lin_measurable
  have hf_lin_bound :
    ...
    · simp only [ht_S, ↓reduceIte]
      ...
      · have h_agree :
          ...
          have h'_iff_lin :
            ...
            constructor
            · intro ⟨h1, h2⟩
              exact (h_not_sd.1 ⟨ht_Icc, hδ, h1, h2⟩).2.2
            · intro ⟨h1, h2⟩
              exact (h_not_sd.2 ⟨ht_Icc, hδ, h1, h2⟩).2.2
          by_cases ht_eq : t = t₀
          · subst ht_eq
