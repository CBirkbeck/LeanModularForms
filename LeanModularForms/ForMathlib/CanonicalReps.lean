...
theorem repStrict_mem_s₀ {p : ℍ} (hp : p ∈ repStrict f hf) : p ∈ s₀ f hf :=
  (Finset.mem_filter.mp hp).1

theorem repLeftVert_mem_s₀ {p : ℍ} (hp : p ∈ repLeftVert f hf) : p ∈ s₀ f hf :=
  (Finset.mem_filter.mp hp).1

theorem repLeftArc_mem_s₀ {p : ℍ} (hp : p ∈ repLeftArc f hf) : p ∈ s₀ f hf :=
  (Finset.mem_filter.mp hp).1
