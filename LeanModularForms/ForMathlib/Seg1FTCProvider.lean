...
private lemma neg_seg1_h₀_left_slitPlane {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
    {z₀ : ℂ} (hz_re : z₀.re = 1/2) (hc_lo : Real.sqrt 3 / 2 < z₀.im) (hc_hi : z₀.im < H)
    {δ : ℝ} (hδ_pos : 0 < δ)
    {t : ℝ} (ht0 : 0 ≤ t) (htd : t ≤ seg1T₀ H z₀.im - δ) :
  ...
  -- Goal: -(H - 5 * t * (H - √3 / 2) - z₀.im) ≠ 0
  -- i.e. H - 5*t*K_factor - z₀.im ≠ 0
  ...
  have h_im_eq : H - 5 * t * (H - Real.sqrt 3 / 2) - z₀.im = 0 := by linarith
  have h_t_eq : 5 * t * (H - Real.sqrt 3 / 2) = H - z₀.im := by linarith
  -- This means t = t₀, contradicting t ≤ t₀ - δ < t₀.
  have h_t : t = seg1T₀ H z₀.im := by
    have : seg1Speed H * t = seg1Speed H * seg1T₀ H z₀.im := by
      rw [hK_eq]; unfold seg1Speed; linarith
    have h_ne : seg1Speed H ≠ 0 := ne_of_gt hK
...
private lemma neg_seg1_h₀_right_slitPlane {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
    ...
    (hc_hi : z₀.im < H)
    {δ : ℝ} (hδ_pos : 0 < δ)
    {t : ℝ} (htd : seg1T₀ H z₀.im + δ ≤ t) (ht1 : t ≤ 1/5) :
    -(seg1_h₀ H z₀ t) ∈ Complex.slitPlane := by
  rw [Complex.mem_slitPlane_iff]; right
  rw [seg1_h₀_eq_pure_im hz_re]
  simp only [Complex.neg_im, Complex.mul_im, Complex.ofReal_re, Complex.ofReal_im,
  ...
  have h_t : t = seg1T₀ H z₀.im := by
    have : seg1Speed H * t = seg1Speed H * seg1T₀ H z₀.im := by
      rw [hK_eq]; unfold seg1Speed; linarith
    exact mul_left_cancel₀ (ne_of_gt hK) this
  linarith
...
private lemma neg_seg1_h_arc_slitPlane {z₀ : ℂ} (hz_re : z₀.re = 1/2)
  ...
  · rw [Complex.mem_slitPlane_iff]; left
    ...
    have h_cos_lt : Real.cos (fdArcAngle t) < 1/2 := by
      have h_pi3 : (Real.pi / 3) ∈ Icc (0 : ℝ) Real.pi := ⟨by linarith, by linarith⟩
      have hθ_Icc : fdArcAngle t ∈ Icc (0 : ℝ) Real.pi := ⟨by linarith, by linarith⟩
      have := Real.strictAntiOn_cos h_pi3 hθ_Icc hθ_lo
...
private lemma seg1_left_ftc {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
  ...
  apply LogDerivFTCFM.ftc_log_neg_on_segment (by linarith : (0:ℝ) ≤ seg1T₀ H z₀.im - δ)
    (seg1_h₀_continuous H z₀).continuousOn
    (fun t _ => (hasDerivAt_seg1_h₀ H z₀ t).differentiableAt)
    (by
      rw [show deriv (seg1_h₀ H z₀) = fun _ => -(seg1Speed H : ℂ) * I from
        funext (deriv_seg1_h₀ H z₀)]
      exact continuousOn_const)
  intro t ⟨ht0, htd⟩
  exact neg_seg1_h₀_left_slitPlane hH hz_re hc_lo hc_hi hδ_pos ht0 htd

/-- FTC for the seg1 right half `[t₀ + δ, 1/5]`. -/
private lemma seg1_right_ftc {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
    {z₀ : ℂ} (hz_re : z₀.re = 1/2) (hc_hi : z₀.im < H)
  ...
  apply LogDerivFTCFM.ftc_log_neg_on_segment (by linarith : seg1T₀ H z₀.im + δ ≤ 1/5)
    (seg1_h₀_continuous H z₀).continuousOn
    (fun t _ => (hasDerivAt_seg1_h₀ H z₀ t).differentiableAt)
    (by
      rw [show deriv (seg1_h₀ H z₀) = fun _ => -(seg1Speed H : ℂ) * I from
      ...
      exact continuousOn_const)
  intro t ⟨htd, ht1⟩
  exact neg_seg1_h₀_right_slitPlane hH hz_re hc_hi hδ_pos htd ht1
...
private lemma seg1_arc_ftc {z₀ : ℂ} (hz_re : z₀.re = 1/2)
  ...
  apply LogDerivFTCFM.ftc_log_neg_on_segment (by norm_num : (1/5 : ℝ) ≤ 3/5)
    ...
    (by
      ...
      have h_eq : deriv (seg1_h_arc z₀) =
          fun t => ↑(5 * Real.pi / 6) * I * exp (↑(fdArcAngle t) * I) :=
        funext (deriv_seg1_h_arc z₀)
      rw [h_eq]; exact this)
  intro t ⟨ht1, ht3⟩
...
private lemma seg1_seg5_ftc (H : ℝ) {z₀ : ℂ} (hc_hi : z₀.im < H) :
  ...
  apply LogDerivFTCFM.ftc_log_neg_on_segment (by norm_num : (4/5 : ℝ) ≤ 1)
    (seg1_h₅_continuous H z₀).continuousOn
    (fun t _ => (hasDerivAt_seg1_h₅ H z₀ t).differentiableAt)
    (by
      rw [show deriv (seg1_h₅ H z₀) = fun _ => (5 : ℂ) from
        funext (deriv_seg1_h₅ H z₀)]
      exact continuousOn_const)
  intro t _
...
private lemma seg1_log_diff_eq_neg_pi_I {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
  ...
  have h_minus : seg1_h₀ H z₀ (seg1T₀ H z₀.im - δ) =
    ...
    rw [seg1_h₀_eq_pure_im hz_re]
    congr 1
    have h_real : H - 5 * (seg1T₀ H z₀.im - δ) * (H - Real.sqrt 3 / 2) - z₀.im =
        seg1Speed H * δ := by
      have h_speed : 5 * (seg1T₀ H z₀.im - δ) * (H - Real.sqrt 3 / 2) =
          seg1Speed H * (seg1T₀ H z₀.im - δ) := by unfold seg1Speed; ring
      rw [h_speed, mul_sub, seg1Speed_mul_t₀ hH]
      ring
  ...
  have h_plus : seg1_h₀ H z₀ (seg1T₀ H z₀.im + δ) =
    ...
    have h_real : H - 5 * (seg1T₀ H z₀.im + δ) * (H - Real.sqrt 3 / 2) - z₀.im =
      ...
      have h_speed : 5 * (seg1T₀ H z₀.im + δ) * (H - Real.sqrt 3 / 2) =
          seg1Speed H * (seg1T₀ H z₀.im + δ) := by unfold seg1Speed; ring
      rw [h_speed, mul_add, seg1Speed_mul_t₀ hH]
      ring
    exact_mod_cast h_real
  rw [h_minus, h_plus]
...
private lemma ae_eq_seg1_h₀ (H : ℝ) (z₀ : ℂ) {a b : ℝ} (hab : a ≤ b) (hb : b ≤ 1/5) :
  ...
  have ht_lt_one_fifth : t < 1/5 := lt_of_lt_of_le ht_lt_b hb
  have h_eq : fdBoundaryFun H t - z₀ = seg1_h₀ H z₀ t :=
    fdBoundary_sub_eq_seg1_h₀ H z₀ t (le_of_lt ht_lt_one_fifth)
  have h_evEq : (fun s => fdBoundaryFun H s - z₀) =ᶠ[𝓝 t] seg1_h₀ H z₀ :=
    Filter.eventually_of_mem (Iio_mem_nhds ht_lt_one_fifth)
      (fun s hs => fdBoundary_sub_eq_seg1_h₀ H z₀ s (le_of_lt hs))
  have h_deriv : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (seg1_h₀ H z₀) t :=
    h_evEq.deriv_eq
...
private lemma ae_eq_seg1_h_arc (H : ℝ) (z₀ : ℂ) :
  ...
  have ht3_lt : t < 3/5 :=
    lt_of_le_of_ne ht_mem.2 (fun h => ht_ne (Set.mem_singleton_iff.mpr h))
  have h_eq : fdBoundaryFun H t - z₀ = seg1_h_arc z₀ t :=
    fdBoundary_sub_eq_seg1_h_arc z₀ ht1 (le_of_lt ht3_lt)
  have h_evEq : (fun s => fdBoundaryFun H s - z₀) =ᶠ[𝓝 t] seg1_h_arc z₀ :=
    Filter.eventually_of_mem (Filter.inter_mem (Ioi_mem_nhds ht1) (Iio_mem_nhds ht3_lt))
      fun s ⟨hs1, hs3⟩ => fdBoundary_sub_eq_seg1_h_arc z₀ hs1 (le_of_lt hs3)
  have h_deriv : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (seg1_h_arc z₀) t :=
    h_evEq.deriv_eq
  have h_deriv_sub : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (fdBoundaryFun H) t :=
    deriv_sub_const (f := fdBoundaryFun H) _
  rw [h_eq, ← h_deriv_sub, h_deriv, div_eq_mul_inv, mul_comm]
...
private lemma ae_eq_seg1_h₃ (H : ℝ) (z₀ : ℂ) :
  ...
  have ht4_lt : t < 4/5 :=
    lt_of_le_of_ne ht_mem.2 (fun h => ht_ne (Set.mem_singleton_iff.mpr h))
  have h_eq : fdBoundaryFun H t - z₀ = seg1_h₃ H z₀ t :=
    fdBoundary_sub_eq_seg1_h₃ H z₀ ht3 (le_of_lt ht4_lt)
  have h_evEq : (fun s => fdBoundaryFun H s - z₀) =ᶠ[𝓝 t] seg1_h₃ H z₀ :=
    Filter.eventually_of_mem (Filter.inter_mem (Ioi_mem_nhds ht3) (Iio_mem_nhds ht4_lt))
      fun s ⟨hs3, hs4⟩ => fdBoundary_sub_eq_seg1_h₃ H z₀ hs3 (le_of_lt hs4)
  have h_deriv : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (seg1_h₃ H z₀) t :=
    h_evEq.deriv_eq
  have h_deriv_sub : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (fdBoundaryFun H) t :=
    deriv_sub_const (f := fdBoundaryFun H) _
...
private lemma ae_eq_seg1_h₅ (H : ℝ) (z₀ : ℂ) :
  ...
  have h_eq : fdBoundaryFun H t - z₀ = seg1_h₅ H z₀ t :=
    fdBoundary_sub_eq_seg1_h₅ H z₀ ht4
  have h_evEq : (fun s => fdBoundaryFun H s - z₀) =ᶠ[𝓝 t] seg1_h₅ H z₀ :=
    Filter.eventually_of_mem (Ioi_mem_nhds ht4)
      fun s hs => fdBoundary_sub_eq_seg1_h₅ H z₀ hs
  have h_deriv : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (seg1_h₅ H z₀) t :=
    h_evEq.deriv_eq
  have h_deriv_sub : deriv (fun s => fdBoundaryFun H s - z₀) t = deriv (fdBoundaryFun H) t :=
    deriv_sub_const (f := fdBoundaryFun H) _
...
theorem fdBoundary_ftc_telescope_seg1 {H : ℝ} (hH : Real.sqrt 3 / 2 < H)
    ...
    (∫ t in (seg1T₀ H z₀.im + δ)..(1 : ℝ),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
    -(↑Real.pi * I) := by
  set t₀ := seg1T₀ H z₀.im with ht₀_def
  have ht₀_pos : 0 < t₀ := seg1T₀_pos hH hc_hi
  have ht₀_lt : t₀ < 1/5 := seg1T₀_lt_one_fifth hH hc_lo
  -- Per-segment FTC pieces
  have h_left := seg1_left_ftc hH hz_re hc_lo hc_hi hδ_pos hδ_lt_t₀
  have h_right := seg1_right_ftc hH hz_re hc_hi hδ_pos hδ_lt_one_fifth_sub
  ...
  have h_int_left :
    ...
    rw [show (∫ t in (0 : ℝ)..(t₀ - δ),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
        ∫ t in (0 : ℝ)..(t₀ - δ), deriv (seg1_h₀ H z₀) t / seg1_h₀ H z₀ t from
  ...
  have h_int_right_seg1 :
    ...
    rw [show (∫ t in (t₀ + δ)..(1/5 : ℝ),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
        ∫ t in (t₀ + δ)..(1/5 : ℝ), deriv (seg1_h₀ H z₀) t / seg1_h₀ H z₀ t from
      intervalIntegral.integral_congr_ae
  ...
  have h_int_arc :
    ...
    rw [show (∫ t in (1/5 : ℝ)..(3/5),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
        ∫ t in (1/5 : ℝ)..(3/5), deriv (seg1_h_arc z₀) t / seg1_h_arc z₀ t from
      intervalIntegral.integral_congr_ae (ae_eq_seg1_h_arc H z₀)]
    exact h_arc.2
  have h_int_seg4 :
      ∫ t in (3/5 : ℝ)..(4/5),
          (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t =
      Complex.log (-(seg1_h₃ H z₀ (4/5))) - Complex.log (-(seg1_h₃ H z₀ (3/5))) := by
    rw [show (∫ t in (3/5 : ℝ)..(4/5),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
        ∫ t in (3/5 : ℝ)..(4/5), deriv (seg1_h₃ H z₀) t / seg1_h₃ H z₀ t from
      intervalIntegral.integral_congr_ae (ae_eq_seg1_h₃ H z₀)]
    exact h_seg4.2
  have h_int_seg5 :
      ∫ t in (4/5 : ℝ)..(1 : ℝ),
          (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t =
      Complex.log (-(seg1_h₅ H z₀ 1)) - Complex.log (-(seg1_h₅ H z₀ (4/5))) := by
    rw [show (∫ t in (4/5 : ℝ)..(1 : ℝ),
        (fdBoundaryFun H t - z₀)⁻¹ * deriv (fdBoundaryFun H) t) =
        ∫ t in (4/5 : ℝ)..(1 : ℝ), deriv (seg1_h₅ H z₀) t / seg1_h₅ H z₀ t from
      intervalIntegral.integral_congr_ae (ae_eq_seg1_h₅ H z₀)]
    exact h_seg5.2
