...
private lemma nhdsNE_eventuallyEq_to_nhds {z₀ : ℂ} {f g : ℂ → ℂ} (hfg : f =ᶠ[𝓝[≠] z₀] g) :
...
private lemma logDeriv_zpow_mul_eq {z₀ z : ℂ} {n : ℤ} {g : ℂ → ℂ} (hzsub : z - z₀ ≠ 0)
    (hgz_ne : g z ≠ 0) (hgz_diff : DifferentiableAt ℂ g z) :
    logDeriv (fun w => (w - z₀) ^ n * g w) z = ↑n / (z - z₀) + logDeriv g z := by
  simp only [logDeriv_apply]
  have h_sub : HasDerivAt (· - z₀) 1 z := by
    have h := (hasDerivAt_id z).sub (hasDerivAt_const z z₀)
    rwa [sub_zero] at h
...
private lemma logDeriv_eventually_eq_pole_decomp {f : ℂ → ℂ} {z₀ : ℂ} {n : ℤ} {g : ℂ → ℂ}
  ...
  have hflog : logDeriv f z = logDeriv (fun w => (w - z₀) ^ n * g w) z := by
...
theorem logDeriv_hasSimplePoleAt_of_order {f : ℂ → ℂ} {z₀ : ℂ} {n : ℤ} (hf : MeromorphicAt f z₀)
...
theorem logDeriv_residue_eq_order {f : ℂ → ℂ} {z₀ : ℂ} (hf : MeromorphicAt f z₀)
    (hord : meromorphicOrderAt f z₀ ≠ ⊤) :
...
theorem logDeriv_residue_eq_int {f : ℂ → ℂ} {z₀ : ℂ} {n : ℤ} (hf : MeromorphicAt f z₀)
    (hord : meromorphicOrderAt f z₀ = (n : ℤ)) :
    residue (logDeriv f) z₀ = ↑n := by
  have hne : meromorphicOrderAt f z₀ ≠ ⊤ := by
    rw [hord]
