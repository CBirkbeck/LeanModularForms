...
/-- **A-1b foundational helper**: A differentiable `f : ℝ → ℂ` with bounded derivative
on a convex set `s` is `LipschitzOnWith C f s`. Specialization of
`Convex.lipschitzOnWith_of_nnnorm_hasDerivWithin_le` to `ℝ → ℂ`. -/
theorem lipschitzOnWith_of_nnnorm_deriv_le
    {f : ℝ → ℂ} {s : Set ℝ} (hs : Convex ℝ s)
    {C : NNReal}
