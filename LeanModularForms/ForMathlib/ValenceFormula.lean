...
* `ordOrbitQFM` — the order of vanishing on non-elliptic orbits, cast to `ℂ`

## Main results

* `finsum_nonell_eq_repCanon_sumFM` — the `∑ᶠ` over non-elliptic orbits equals the
  `repCanonFM` Finset sum
...
def ordOrbitQFM (q : NonEllOrbitFM) : ℂ := (ordOrbitFM f q.val : ℂ)

/-! ### Elliptic point auxiliary lemmas -/

private lemma ellipticPointI'_coeFM : (ellipticPointI' : ℂ) = Complex.I := rfl
private lemma ellipticPointI'_imFM : (ellipticPointI' : ℍ).im = 1 := Complex.I_im

private lemma ellipticPointRho'_reFM : (ellipticPointRho' : ℍ).re = -1/2 := by
  show (-1/2 + (Real.sqrt 3 / 2) * I : ℂ).re = -1/2
  simp only [add_re, mul_re, I_re, I_im]; norm_num

private lemma ellipticPointRho'_imFM :
...
private lemma ellipticPointRhoPlusOne'_reFM :
    (ellipticPointRhoPlusOne' : ℍ).re = 1/2 := by
  show (1/2 + (Real.sqrt 3 / 2) * I : ℂ).re = 1/2
  simp only [add_re, mul_re, I_re, I_im]; norm_num

private lemma ellipticPointRhoPlusOne'_imFM :
...
omit f hf in
private lemma sl2_detFM (g : SL(2, ℤ)) :
...
omit f hf in
private lemma denom_at_IFM (g : SL(2, ℤ)) :
...
omit f hf in
private lemma normSq_denom_at_IFM (g : SL(2, ℤ)) :
...
private lemma normSq_denom_eq_one_of_smul_i_in_fdFM (g : SL(2, ℤ))
  ...
  rw [ellipticPointI'_imFM, show (ellipticPointI' : ℂ) = I from rfl, normSq_denom_at_IFM] at h_im
...
omit f hf in
private lemma re_smul_ellipticPointIFM (g : SL(2, ℤ)) :
    (g • ellipticPointI').re =
  ...
  rw [UpperHalfPlane.coe_specialLinearGroup_apply, ellipticPointI'_coeFM]
...
theorem fd_orbit_i_eq_i (p : ℍ) (hp : p ∈ 𝒟) (horb : orbFM p = oiFM) :
  ...
  have h_cd1 := normSq_denom_eq_one_of_smul_i_in_fdFM g hp
  have h_im := ModularGroup.im_smul_eq_div_normSq g ellipticPointI'
  rw [ellipticPointI'_imFM, show (ellipticPointI' : ℂ) = I from rfl,
      normSq_denom_at_IFM, h_cd1, div_one] at h_im
  have h_re := re_smul_ellipticPointIFM g
  ...
  exact UpperHalfPlane.ext_re_im
    ...
    (by linarith [h_im, ellipticPointI'_imFM])

/-! ### SL₂(ℤ) denominator computations at ρ -/

omit f hf in
private lemma denom_at_rhoFM (g : SL(2, ℤ)) :
    UpperHalfPlane.denom (Matrix.SpecialLinearGroup.toGL
...
omit f hf in
private lemma normSq_denom_at_rhoFM (g : SL(2, ℤ)) :
    Complex.normSq (UpperHalfPlane.denom (Matrix.SpecialLinearGroup.toGL
      ((Matrix.SpecialLinearGroup.map (Int.castRingHom ℝ)) g)) (ellipticPointRho' : ℍ)) =
    ((g : Matrix (Fin 2) (Fin 2) ℤ) 1 0 : ℝ) ^ 2 -
    ((g : Matrix (Fin 2) (Fin 2) ℤ) 1 0 : ℝ) * ((g : Matrix (Fin 2) (Fin 2) ℤ) 1 1 : ℝ) +
    ((g : Matrix (Fin 2) (Fin 2) ℤ) 1 1 : ℝ) ^ 2 := by
  rw [denom_at_rhoFM, Complex.normSq_apply]
...
omit f hf in
private lemma normSq_denom_eq_one_of_smul_rho_in_fdFM (g : SL(2, ℤ))
  ...
  rw [ellipticPointRho'_imFM, normSq_denom_at_rhoFM] at h_im
...
omit f hf in
private lemma abs_re_eq_half_of_smul_rho_in_fdFM (g : SL(2, ℤ))
    (h_fd : g • ellipticPointRho' ∈ 𝒟)
  ...
  have h_im_eq : (g • ellipticPointRho').im = Real.sqrt 3 / 2 := by
    have h_im := ModularGroup.im_smul_eq_div_normSq g ellipticPointRho'
    rw [ellipticPointRho'_imFM, normSq_denom_at_rhoFM, h_cd1, div_one] at h_im; exact h_im
...
theorem fd_orbit_rho_eq (p : ℍ) (hp : p ∈ 𝒟) (horb : orbFM p = orhoFM) :
  ...
  set a := (g : Matrix (Fin 2) (Fin 2) ℤ) 0 0 with ha_def
  set b := (g : Matrix (Fin 2) (Fin 2) ℤ) 0 1 with hb_def
  have h_cd1 := normSq_denom_eq_one_of_smul_rho_in_fdFM g hp
  have h_im : (g • ellipticPointRho').im = Real.sqrt 3 / 2 := by
    have := ModularGroup.im_smul_eq_div_normSq g ellipticPointRho'
    rw [ellipticPointRho'_imFM, normSq_denom_at_rhoFM, h_cd1, div_one] at this; exact this
  have h_re_eq : (g • ellipticPointRho').re = -1/2 ∨ (g • ellipticPointRho').re = 1/2 := by
    have h_re_abs := abs_re_eq_half_of_smul_rho_in_fdFM g hp h_cd1
  ...
  · left
    exact UpperHalfPlane.ext_re_im (by linarith [h_re_left, ellipticPointRho'_reFM])
      (by linarith [h_im, ellipticPointRho'_imFM])
  · right
    exact UpperHalfPlane.ext_re_im (by linarith [h_re_right, ellipticPointRhoPlusOne'_reFM])
      (by linarith [h_im, ellipticPointRhoPlusOne'_imFM])

/-! ### Non-elliptic orbit classification -/

/-- Elements of `repCanonFM` map to non-elliptic orbits. -/
theorem orb_repCanon_nonEllFM (p : ℍ) (hp : p ∈ repCanonFM f hf) :
    orbFM p ≠ oiFM ∧ orbFM p ≠ orhoFM := by
  have ⟨hne_i, hne_rho, hne_rho1⟩ := repCanon_ne_ellipticFM f hf p hp
  have hp_fd := repCanon_mem_fdFM f hf hp
  exact ⟨fun h => hne_i (fd_orbit_i_eq_i p hp_fd h),
    fun h => (fd_orbit_rho_eq p hp_fd h).elim hne_rho hne_rho1⟩

/-! ### Finsum equals repCanonFM sum -/

/-- The `∑ᶠ` over non-elliptic orbits equals the `repCanonFM` Finset sum. -/
theorem finsum_nonell_eq_repCanon_sumFM :
    ∑ᶠ (q : NonEllOrbitFM), ordOrbitQFM f q =
    ∑ s ∈ repCanonFM f hf, (orderOfVanishingAt' (⇑f) s : ℂ) := by
  ...
  set R := repCanonFM f hf
  set φ : (p : ℍ) → p ∈ R → NonEllOrbitFM :=
    fun p hp => ⟨orbFM p, orb_repCanon_nonEllFM f hf p hp⟩ with hφ_def
  have h_im : ∀ p (hp : p ∈ R), φ p hp ∈ S := fun p hp => by
    rw [Set.Finite.mem_toFinset]; change ordOrbitFM f (orbFM p) ≠ 0; rw [ordOrbit_mkFM]
    have hp_s₀ := repCanon_mem_s₀ f hf hp
    rw [s₀FM, Set.Finite.mem_toFinset] at hp_s₀; exact hp_s₀.2
  have h_surj : ∀ q ∈ S, ∃ p, ∃ hp : p ∈ R, φ p hp = q := fun q hq => by
    obtain ⟨p, hp_mem, hp_orb⟩ := exists_repCanon_of_nonEllOrbitFM f hf q
      ((Set.Finite.mem_toFinset _).mp hq)
    exact ⟨p, hp_mem, Subtype.ext hp_orb⟩
  exact (Finset.sum_bij φ h_im
    (fun _ h₁ _ h₂ heq => orb_injOn_repCanonFM f hf h₁ h₂ (congr_arg Subtype.val heq))
    h_surj (fun p hp => by
      simp only [ordOrbitQFM, hφ_def]; exact_mod_cast (ordOrbit_mkFM f p).symm)).symm

/-! ### repCanonFM sum decomposition -/

/-- The `repCanonFM` sum splits into strict interior + left vertical + left arc. -/
theorem repCanon_sum_splitFM :
    ∑ s ∈ repCanonFM f hf, (orderOfVanishingAt' (⇑f) s : ℂ) =
    ∑ s ∈ repStrictFM f hf, (orderOfVanishingAt' (⇑f) s : ℂ) +
    ∑ s ∈ repLeftVertFM f hf, (orderOfVanishingAt' (⇑f) s : ℂ) +
    ∑ s ∈ repLeftArcFM f hf, (orderOfVanishingAt' (⇑f) s : ℂ) := by
  simp only [repCanonFM]; rw [Finset.sum_union (disjoint_union_repLeftArcFM f hf),
    Finset.sum_union (disjoint_repStrict_repLeftVertFM f hf)]
...
theorem valence_formula_textbook_orbit_finsum
    ...
    ∑ᶠ (q : NonEllOrbitFM), ordOrbitQFM f q =
    (k : ℂ) / 12 := by
  rw [finsum_nonell_eq_repCanon_sumFM f hf, repCanon_sum_splitFM f hf]
  unfold repStrictFM repLeftVertFM repLeftArcFM
  linear_combination
