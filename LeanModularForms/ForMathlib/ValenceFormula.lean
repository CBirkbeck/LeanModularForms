...
omit f hf in
/-- If `p ∈ 𝒟` and `orb p = oi`, then `p = i`. -/
theorem fd_orbit_i_eq_i (p : ℍ) (hp : p ∈ 𝒟) (horb : orb p = oi) :
    p = ellipticPointI' := by
...
omit f hf in
/-- If `p ∈ 𝒟` and `orb p = orho`, then `p = ρ` or `p = ρ+1`. -/
theorem fd_orbit_rho_eq (p : ℍ) (hp : p ∈ 𝒟) (horb : orb p = orho) :
...
theorem valence_formula_textbook_orbit_finsum
  ...
  linear_combination
    h_core (s₀ f hf) (s₀_mem_fd f hf) (s₀_complete f hf)
