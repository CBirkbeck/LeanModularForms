...
theorem residueAt_eq_laurent_head_coeff (f : ℂ → ℂ) (s : ℂ)
  ...
  have h_eq_on : Set.EqOn f
    ...
    exact hrf_eq ⟨Metric.mem_ball.mpr (Metric.mem_sphere.mp hz ▸ hr_lt_rf),
...
private lemma ae_eq_indicator_diff_cpv_zpow
  ...
  · push_neg at h2
    ...
    ring
  all_goals (push_neg at h1; push_neg at h2 ⊢ <;> skip)
  all_goals
    have h_not_mem :
        ¬(t ∈ ({t | ε < ‖γ.toFun t - s‖} \
          {t | ∀ s' ∈ S0, ε < ‖γ.toFun t - s'‖}) ∩
    ...
    simp only [h_not_mem, ite_false, if_neg (not_lt.mpr h1)]
  · rw [if_pos (show ∃ s' ∈ S0, ‖γ.toFun t - s'‖ ≤ ε from ⟨s, hs, h1⟩)]; ring
  · obtain ⟨s', hs', hs'_le⟩ := h2
    rw [if_pos (show ∃ s' ∈ S0, ‖γ.toFun t - s'‖ ≤ ε from ⟨s', hs', hs'_le⟩)]; ring
...
private lemma ae_not_in_S0_of_finite_crossings
...
private lemma zpow_norm_bound (γ : PiecewiseC1Immersion) (s : ℂ) (m : ℕ)
...
private lemma deriv_continuousWithinAt_off_partition
    (γ : PiecewiseC1Immersion) (t : ℝ)
    (ht_Icc : t ∈ Icc γ.a γ.b) (ht_nP : t ∉ γ.partition) :
    ContinuousWithinAt (deriv γ.toFun) (Icc γ.a γ.b) t := by
  have ht_Ioo : t ∈ Ioo γ.a γ.b :=
    ⟨lt_of_le_of_ne ht_Icc.1 (Ne.symm fun h =>
      ht_nP (h ▸ γ.toPiecewiseC1Curve.endpoints_in_partition.1)),
     lt_of_le_of_ne ht_Icc.2 fun h =>
...
private lemma aesm_deriv_on_Icc (γ : PiecewiseC1Immersion) :
  ...
  aEStronglyMeasurable_of_continuousOn_off_finite (P := γ.partition) (by
    ...
    exact deriv_continuousWithinAt_off_partition γ t ht_Icc ht_nP)

private lemma single_cpv_zpow_intervalIntegrable
    (γ : PiecewiseC1Immersion) (s : ℂ) (m : ℕ)
    (Mγ' : ℝ) (hMγ' : ∀ t ∈ Icc γ.a γ.b, ‖deriv γ.toFun t‖ ≤ Mγ')
    (f_zpow : ℂ → ℂ) (hf_zpow : f_zpow = fun z => (z - s) ^ (-(m : ℤ)))
    (ε : ℝ) (hε : 0 < ε) :
  ...
  rw [h_eq, intervalIntegrable_iff_integrableOn_Ioc_of_le (le_of_lt γ.hab)]
  ...
  · intro t ht
    ...
    split_ifs with h
    · exact zpow_norm_bound γ s m Mγ' hMγ' f_zpow hf_zpow ε hε t ht h
    · simp only [norm_zero]; positivity

private lemma measurableSet_goodSet_Icc'
    (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) (ε : ℝ) :
    MeasurableSet ({t : ℝ | ∀ s' ∈ S0, ε < ‖γ.toFun t - s'‖} ∩ Icc γ.a γ.b) := by
  have h_eq : {t : ℝ | ∀ s' ∈ S0, ε < ‖γ.toFun t - s'‖} ∩ Icc γ.a γ.b =
      Icc γ.a γ.b \
        ({t | ∃ s' ∈ S0, ‖γ.toFun t - s'‖ ≤ ε} ∩ Icc γ.a γ.b) := by
    ext t; constructor
    · intro ⟨h_good, ht⟩
...
private lemma multi_cpv_zpow_aesm
    (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) (s : ℂ) (m : ℕ)
    (hs : s ∈ S0)
    (f_zpow : ℂ → ℂ) (hf_zpow : f_zpow = fun z => (z - s) ^ (-(m : ℤ)))
    (ε : ℝ) (hε : 0 < ε) :
...
private lemma multi_cpv_zpow_intervalIntegrable
    (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) (s : ℂ) (m : ℕ)
    (hs : s ∈ S0)
    (Mγ' : ℝ) (hMγ' : ∀ t ∈ Icc γ.a γ.b, ‖deriv γ.toFun t‖ ≤ Mγ')
    (f_zpow : ℂ → ℂ) (hf_zpow : f_zpow = fun z => (z - s) ^ (-(m : ℤ)))
  ...
  refine IntegrableOn.mono_set ?_ Ioc_subset_Icc_self
  refine integrableOn_of_bounded_aeMeasurable (M := ε⁻¹ ^ m * (|Mγ'| + 1)) ?_ ?_
  · exact (multi_cpv_zpow_aesm S0 γ s m hs f_zpow hf_zpow ε hε).mono_measure
      (Measure.restrict_mono Icc_subset_Icc_self le_rfl)
  · intro t ht
    simp only [cauchyPrincipalValueIntegrandOn]
    split_ifs with h
    · simp only [norm_zero]; positivity
    · push_neg at h; exact zpow_norm_bound γ s m Mγ' hMγ' f_zpow hf_zpow ε hε t ht (h s hs)

private lemma diff_single_multi_dct_bound
    (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) (s : ℂ) (m : ℕ)
    (hs : s ∈ S0)
    (Mγ' : ℝ) (hMγ' : ∀ t ∈ Icc γ.a γ.b, ‖deriv γ.toFun t‖ ≤ Mγ')
    (f_zpow : ℂ → ℂ) (hf_zpow : f_zpow = fun z => (z - s) ^ (-(m : ℤ)))
    ...
    (Bd : ℝ) (hBd : Bd = (δ_sep / 2)⁻¹ ^ m * (|Mγ'| + 1)) :
    ∀ᶠ ε in 𝓝[>] (0 : ℝ),
      ∀ᵐ t ∂volume, t ∈ Ι γ.a γ.b →
        ‖(if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0) -
         cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t‖ ≤ Bd := by
  filter_upwards [Ioo_mem_nhdsGT (show (0 : ℝ) < δ_sep / 2 by linarith)] with ε hε
  ...
  · rw [if_pos h_multi_cut]
    ...
    · push_neg at h_single_cut; rw [if_neg (not_lt.mpr h_single_cut)]
      norm_num; rw [hBd]; positivity
  · rw [if_neg h_multi_cut]
    by_cases h_single_cut : ‖γ.toFun t - s‖ > ε
    · rw [if_pos h_single_cut]; norm_num; rw [hBd]; positivity
    · push_neg at h_single_cut h_multi_cut
      exact absurd (h_multi_cut s hs) (not_lt.mpr h_single_cut)

private lemma multipoint_reduce_to_diff
    (γ : PiecewiseC1Immersion) (S0 : Finset ℂ) (s : ℂ)
    (f_zpow : ℂ → ℂ)
    (h_single : Tendsto (fun ε =>
      ∫ t in γ.a..γ.b,
        (if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0))
      (𝓝[>] 0) (𝓝 0))
    (h_single_int : ∀ ε : ℝ, 0 < ε →
      IntervalIntegrable
        (fun t => if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0)
        volume γ.a γ.b)
    (h_multi_int : ∀ ε : ℝ, 0 < ε →
      IntervalIntegrable
        (fun t => cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t) volume γ.a γ.b)
    ...
    Tendsto (fun ε =>
      ∫ t in γ.a..γ.b,
        cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t) (𝓝[>] 0) (𝓝 0) := by
  have h_eventually_eq : ∀ᶠ ε in 𝓝[>] (0 : ℝ),
    ...
    congr 1; ext t; ring
  exact (h_single.sub h_diff).congr'
    ((by simp : (0 : ℂ) = 0 - 0) ▸ h_eventually_eq.mono fun ε h => h.symm)

private lemma diff_single_multi_ae_convergence
    (S0 : Finset ℂ) (γ : PiecewiseC1Immersion) (s : ℂ)
    (hs : s ∈ S0)
    (f_zpow : ℂ → ℂ) :
    ∀ᵐ t ∂volume, t ∈ Ι γ.a γ.b →
      Tendsto (fun ε =>
  ...
  filter_upwards [ae_not_in_S0_of_finite_crossings S0 γ] with t h_not_cross ht_in
...
theorem multipoint_pv_zpow_tendsto_zero
  ...
  have h_single_int : ∀ ε : ℝ, 0 < ε →
      IntervalIntegrable
        (fun t => if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0)
        volume γ.a γ.b :=
    fun ε hε => single_cpv_zpow_intervalIntegrable γ s m Mγ' hMγ' f_zpow hf_zpow_def ε hε
  have h_multi_int : ∀ ε : ℝ, 0 < ε →
      IntervalIntegrable
        (fun t => cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t)
        volume γ.a γ.b :=
    fun ε hε => multi_cpv_zpow_intervalIntegrable S0 γ s m hs Mγ' hMγ' f_zpow hf_zpow_def ε hε
  suffices h_diff : Tendsto (fun ε =>
      ∫ t in γ.a..γ.b,
        ((if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0) -
         cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t))
      (𝓝[>] 0) (𝓝 0) from
    multipoint_reduce_to_diff γ S0 s f_zpow h_single h_single_int h_multi_int h_diff
  rw [← (show ∫ t in γ.a..γ.b, (0 : ℂ) = 0 by simp)]
  by_cases hS0_single : S0 = {s}
  · apply tendsto_const_nhds.congr'
    filter_upwards [self_mem_nhdsWithin] with ε (hε : (0 : ℝ) < ε)
    simp only [intervalIntegral.integral_zero]; symm
    calc ∫ t in γ.a..γ.b,
          ((if ‖γ.toFun t - s‖ > ε then f_zpow (γ.toFun t) * deriv γ.toFun t else 0) -
           cauchyPrincipalValueIntegrandOn S0 f_zpow γ.toFun ε t)
        = ∫ t in γ.a..γ.b, (0 : ℂ) := by
      ...
      _ = 0 := by simp
  · have hne : (S0.erase s).Nonempty := (Finset.erase_nonempty hs).mpr
        ((Finset.nontrivial_iff_ne_singleton hs).mpr hS0_single)
    set δ_sep := (S0.erase s).inf' hne (fun s' => ‖s - s'‖) with hδ_sep_def
    have hδ_pos : 0 < δ_sep := (Finset.lt_inf'_iff _).mpr
      (fun s' hs' => norm_pos_iff.mpr (sub_ne_zero.mpr (ne_of_mem_erase hs').symm))
    ...
    · exact diff_single_multi_dct_bound S0 γ s m hs Mγ' hMγ' f_zpow hf_zpow_def
        hS0_single δ_sep hδ_pos (fun s' hs' => Finset.inf'_le _ hs') Bd hBd_def
    · exact intervalIntegrable_const
    · exact diff_single_multi_ae_convergence S0 γ s hs f_zpow
private lemma cpvIntegrandOn_ae_tendsto
    (S0 : Finset ℂ) (g : ℂ → ℂ) (γ : PiecewiseC1Immersion) :
    ∀ᵐ t ∂volume, t ∈ Ι γ.a γ.b →
      Tendsto (fun ε => cauchyPrincipalValueIntegrandOn S0 g γ.toFun ε t)
      (𝓝[>] 0) (𝓝 (g (γ.toFun t) * deriv γ.toFun t)) := by
  filter_upwards [ae_not_in_S0_of_finite_crossings S0 γ] with t h_not_in ht_in
  simp only [cauchyPrincipalValueIntegrandOn]
...
private lemma ordinary_integral_zero_by_cauchy
...
theorem holomorphic_cpv_tendsto_zero_on_convex
  ...
  have hg_cont_image : ContinuousOn g (γ.toFun '' Icc γ.a γ.b) :=
    hg.continuousOn.mono (Set.image_subset_iff.mpr (fun t ht => hγ_in_U t ht))
  have hgγ_cont : ContinuousOn (fun t => g (γ.toFun t)) (Set.uIcc γ.a γ.b) := by
    rw [Set.uIcc_of_le (le_of_lt γ.hab)]
    exact hg.continuousOn.comp γ.toPiecewiseC1Curve.continuous_toFun
  ...
  have h_ord_zero := ordinary_integral_zero_by_cauchy U hU hU_convex g hg γ hγ_closed hγ_in_U
  rw [← h_ord_zero]
  exact intervalIntegral.tendsto_integral_filter_of_dominated_convergence
    ...
    (by filter_upwards [self_mem_nhdsWithin] with ε (hε : (0 : ℝ) < ε)
        have h_int := intervalIntegrable_cauchyPrincipalValueIntegrandOn (S0 := S0)
          hε hg_cont_image
        rw [intervalIntegrable_iff] at h_int; exact h_int.aestronglyMeasurable)
    (by filter_upwards [self_mem_nhdsWithin] with ε (_hε : (0 : ℝ) < ε)
        ...
        · exact le_refl _)
    h_ord_int.norm
    (cpvIntegrandOn_ae_tendsto S0 g γ)
...
private lemma aesm_cpvIntegrandOn_of_continuousOn_diff
    ...
    (ε : ℝ) :
    AEStronglyMeasurable
      (cauchyPrincipalValueIntegrandOn S0 g γ.toFun ε)
      (volume.restrict (Icc γ.a γ.b)) := by
  have hγ_cont := γ.toPiecewiseC1Curve.continuous_toFun
  have h_safe_sub :
      (γ.toFun '' Icc γ.a γ.b) ∩
  ...
  have h_prod_meas : AEStronglyMeasurable (fun t => g (γ.toFun t) * deriv γ.toFun t)
    ...
    (hgγ_cont_good.aestronglyMeasurable hGoodSet_meas).mul
      ((aesm_deriv_on_Icc γ).mono_measure
        (Measure.restrict_mono Set.inter_subset_right le_rfl))
  have h_pw := AEStronglyMeasurable.piecewise hGoodSet_meas h_prod_meas
    (aestronglyMeasurable_const (α := ℝ) (β := ℂ))
  have h_ae_eq : cauchyPrincipalValueIntegrandOn S0 g γ.toFun ε
...
private lemma cpvIntegrandOn_norm_bound
    (U : Set ℂ) (S0 : Finset ℂ) (g : ℂ → ℂ)
    (hg_cont : ContinuousOn g (U \ ↑S0))
    (γ : PiecewiseC1Immersion)
    (hγ_in_U : ∀ t ∈ Icc γ.a γ.b, γ.toFun t ∈ U)
    (ε : ℝ) (hε : 0 < ε) :
    ∃ C : ℝ, ∀ t ∈ Icc γ.a γ.b,
      ‖cauchyPrincipalValueIntegrandOn S0 g γ.toFun ε t‖ ≤ C := by
  have hγ_cont := γ.toPiecewiseC1Curve.continuous_toFun
  obtain ⟨Mγ', hMγ'⟩ := piecewiseC1Immersion_deriv_bounded γ
  ...
  have h_safe_sub :
    ...
    exact ⟨hγ_in_U t ht, fun hzS0 => by
      have h1 := hz_safe (γ.toFun t) (Finset.mem_coe.mp hzS0)
      simp only [sub_self, norm_zero] at h1; linarith⟩
  obtain ⟨Mg, hMg⟩ := (h_compact_image.inter_right h_safe_closed).exists_bound_of_continuousOn
    (hg_cont.mono h_safe_sub)
  refine ⟨|Mg| * |Mγ'| + 1, fun t ht => ?_⟩
  simp only [cauchyPrincipalValueIntegrandOn]
  split_ifs with h
  · simp only [norm_zero]; positivity
  · push_neg at h
...
lemma intervalIntegrable_cpvIntegrandOn_of_continuousOn_diff
  ...
  obtain ⟨C, hC⟩ := cpvIntegrandOn_norm_bound U S0 g hg_cont γ hγ_in_U ε hε
  have h_cpv_aesm := aesm_cpvIntegrandOn_of_continuousOn_diff U S0 g hg_cont γ hγ_in_U ε
  have h_int : IntegrableOn
      (cauchyPrincipalValueIntegrandOn S0 g γ.toFun ε) (Icc γ.a γ.b) volume :=
...
theorem tendsto_cpv_of_continuousOn_zero_integral
  ...
  have hgγ_cont : ContinuousOn (fun t => g (γ.toFun t)) (Set.uIcc γ.a γ.b) := by
    rw [Set.uIcc_of_le (le_of_lt γ.hab)]
    exact hg_cont.comp γ.toPiecewiseC1Curve.continuous_toFun
      (fun t ht => Set.mem_image_of_mem _ ht)
  have h_ord_int : IntervalIntegrable (fun t => g (γ.toFun t) * deriv γ.toFun t)
      MeasureTheory.volume γ.a γ.b :=
    (piecewiseC1_deriv_intervalIntegrable γ.toPiecewiseC1Curve
      (piecewiseC1Immersion_deriv_bounded γ)).continuousOn_mul hgγ_cont
...
private lemma circle_integral_constancy_annulus
    (f : ℂ → ℂ) (s : ℂ) (R₀ : ℝ) (rf : ℝ)
  ...
  have hF_diff : ∀ z ∈ (Metric.ball s R₀ \ Metric.closedBall s r) \ (∅ : Set ℂ),
    ...
    have h_ne : z ≠ s := by
      intro heq; subst heq; exact hz_not_cb (Metric.mem_closedBall_self hr_pos.le)
    exact (hF_analytic z (Metric.mem_ball.mpr (lt_trans (Metric.mem_ball.mp hz_ball) hR₀_lt_rf))
      h_ne).differentiableAt
  have h_inv_smul : ∀ ρ (hρ_ne : ρ ≠ 0),
      Set.EqOn (fun z => (z - s)⁻¹ • ((z - s) * f z)) f (Metric.sphere s ρ) := by
    intro ρ hρ_ne z hz
    have h_ne : z ≠ s := by
      intro heq; rw [heq, Metric.mem_sphere, dist_self] at hz; exact hρ_ne hz.symm
...
private lemma circle_integral_residue_sum
    (S0 : Finset ℂ) (f : ℂ → ℂ) (s : ℂ) (hs : s ∈ S0)
    (r : ℝ) (hr_pos : 0 < r) (δ : ℝ) (hr_lt_δ : r < δ)
    (hδ_sep : ∀ s' ∈ S0, s' ≠ s → δ ≤ dist s' s) :
    (∮ z in C(s, r), ∑ s' ∈ S0, residueAt f s' / (z - s')) =
      residueAt f s * (2 * ↑Real.pi * I) := by
  have hr_ne : r ≠ 0 := ne_of_gt hr_pos
  have hs_not : s ∉ Metric.sphere s r := by simp [hr_ne.symm]
  have h_no_pole_on_sphere : ∀ p ∈ S0, ∀ z ∈ Metric.sphere s r, z - ↑p ≠ 0 := by
    intro p hp z hz
    apply sub_ne_zero.mpr; intro heq; rw [heq] at hz
    by_cases h_eq : p = s
    · exact hs_not (h_eq ▸ hz)
    · rw [Metric.mem_sphere] at hz; linarith [hδ_sep p hp h_eq]
  ...
  have h_no_pole_in_cb : ∀ p ∈ S0.erase s, ∀ z ∈ Metric.closedBall s r, z - ↑p ≠ 0 := by
    intro p hp z hz; apply sub_ne_zero.mpr; intro heq; rw [heq] at hz
    rw [Metric.mem_closedBall] at hz
    linarith [hδ_sep p (Finset.mem_of_mem_erase hp) (Finset.ne_of_mem_erase hp)]
  exact Complex.circleIntegral_eq_zero_of_differentiable_on_off_countable hr_pos.le
    Set.countable_empty
    (continuousOn_finset_sum _ fun p hp =>
      ContinuousOn.div continuousOn_const (continuousOn_id.sub continuousOn_const)
        (fun z hz => h_no_pole_in_cb p hp z hz))
    (fun z ⟨hz, _⟩ => DifferentiableAt.fun_sum _ fun p hp =>
      (differentiableAt_const _).div (differentiableAt_id.sub (differentiableAt_const _))
...
private lemma exists_finset_min_sep_dist (S0 : Finset ℂ) (s : ℂ) :
    ∃ δ > 0, ∀ s' ∈ S0, s' ≠ s → δ ≤ dist s' s := by
  by_cases h_other : ∃ s' ∈ S0, s' ≠ s
  · obtain ⟨s', hs', hne⟩ := h_other
    have h_ne : (S0.filter (· ≠ s)).Nonempty := ⟨s', Finset.mem_filter.mpr ⟨hs', hne⟩⟩
    exact ⟨(S0.filter (· ≠ s)).inf' h_ne (fun s' => dist s' s),
      (Finset.lt_inf'_iff h_ne).mpr (fun b hb => dist_pos.mpr (Finset.mem_filter.mp hb).2),
      fun s' hs' hne' => Finset.inf'_le _ (Finset.mem_filter.mpr ⟨hs', hne'⟩)⟩
  · push_neg at h_other
    exact ⟨1, one_pos, fun s' hs' hne' => absurd (h_other s' hs') hne'⟩

private lemma residueAt_eq_circle_integral_of_const_annulus
    (f : ℂ → ℂ) (s : ℂ) (R₀ : ℝ) (hR₀_pos : 0 < R₀)
    (h_const : ∀ r, 0 < r → r ≤ R₀ → (∮ z in C(s, r), f z) = (∮ z in C(s, R₀), f z)) :
    residueAt f s = (2 * ↑Real.pi * I)⁻¹ * ∮ z in C(s, R₀), f z := by
  unfold residueAt; apply Filter.Tendsto.limUnder_eq
  apply tendsto_nhds_of_eventually_eq; rw [eventually_nhdsWithin_iff]
  ...
  rw [h_const r' hr'_pos hr'_lt.le]
...
lemma residueAt_sub_residueSum_eq_zero
  ...
  unfold residueAt; apply Filter.Tendsto.limUnder_eq
  obtain ⟨δ, hδ_pos, hδ_sep⟩ := exists_finset_min_sep_dist S0 s
  obtain ⟨rf, hrf_pos, hrf_analytic⟩ := by
    have := hMero.eventually_analyticAt
    rw [Filter.Eventually, Metric.mem_nhdsWithin_iff] at this; exact this
  set R₀ := min δ rf / 2 with hR₀_def
  have hR₀_pos : 0 < R₀ := by positivity
  have hR₀_lt_δ : R₀ < δ := by linarith [min_le_left δ rf]
  have hR₀_lt_rf : R₀ < rf := by linarith [min_le_right δ rf]
  have h2piI_ne : (2 : ℂ) * ↑Real.pi * I ≠ 0 :=
    mul_ne_zero (mul_ne_zero two_ne_zero
      (Complex.ofReal_ne_zero.mpr Real.pi_ne_zero)) I_ne_zero
  have hf_analytic_at : ∀ z, z ∈ Metric.ball s rf → z ≠ s → AnalyticAt ℂ f z :=
    fun z hz hne => hrf_analytic ⟨hz, Set.mem_compl_singleton_iff.mpr hne⟩
  have hf_cont_sphere : ∀ r, 0 < r → r < rf → ContinuousOn f (Metric.sphere s r) := by
    ...
    exact (hf_analytic_at z (by rwa [Metric.mem_ball, Metric.mem_sphere.mp hz])
      (by intro heq; rw [heq, Metric.mem_sphere, dist_self] at hz;
          linarith)).continuousAt.continuousWithinAt
  have h_const_integral := circle_integral_constancy_annulus f s R₀ rf
    hR₀_pos hR₀_lt_rf hf_analytic_at
  apply tendsto_nhds_of_eventually_eq; rw [eventually_nhdsWithin_iff]
  filter_upwards [Iio_mem_nhds hR₀_pos] with r hr_lt hr_pos
  simp only [Set.mem_Ioi] at hr_pos; simp only [Set.mem_Iio] at hr_lt
  have hf_ci : CircleIntegrable f s r :=
    (hf_cont_sphere r hr_pos (lt_trans hr_lt hR₀_lt_rf)).circleIntegrable hr_pos.le
  have h_no_pole : ∀ p ∈ S0, ∀ z ∈ Metric.sphere s r, z - ↑p ≠ 0 := by
    intro p hp z hz; apply sub_ne_zero.mpr; intro heq; rw [heq] at hz
    by_cases h_eq : p = s
    · exact (by simp [(ne_of_gt hr_pos).symm] : s ∉ Metric.sphere s r) (h_eq ▸ hz)
    · rw [Metric.mem_sphere] at hz; linarith [hδ_sep p hp h_eq, hr_lt, hR₀_lt_δ]
  have hsum_ci : CircleIntegrable (fun z => ∑ s' ∈ S0, residueAt f s' / (z - s')) s r :=
    (ContinuousOn.circleIntegrable hr_pos.le (continuousOn_finset_sum _ fun p hp =>
      ContinuousOn.div continuousOn_const (continuousOn_id.sub continuousOn_const)
        (fun z hz => h_no_pole p hp z hz)))
  simp only [show ∀ s' : ℂ, (limUnder (𝓝[>] (0:ℝ)) fun r =>
    (2 * ↑Real.pi * I)⁻¹ * ∮ z in C(s', r), f z) = residueAt f s' from fun _ => rfl]
  rw [circleIntegral.integral_sub hf_ci hsum_ci,
    circle_integral_residue_sum S0 f s hs r hr_pos δ (lt_trans hr_lt hR₀_lt_δ) hδ_sep,
    mul_sub, mul_comm (residueAt f s) _, ← mul_assoc, inv_mul_cancel₀ h2piI_ne, one_mul,
    h_const_integral r hr_pos hr_lt.le,
    residueAt_eq_circle_integral_of_const_annulus f s R₀ hR₀_pos h_const_integral, sub_self]
