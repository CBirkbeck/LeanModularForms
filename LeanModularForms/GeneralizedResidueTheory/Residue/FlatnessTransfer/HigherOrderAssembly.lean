...
private lemma cpv_polarHigher_tendsto_zero
  ...
  have h_cpv_decomp : ∀ ε t,
    ...
    fun ε t => cpvIntegrandOn_finset_sum S0 Finset.univ
      (fun k z => if k.val ≥ 1 then a_s k / (z - s) ^ (k.val + 1) else 0) γ.toFun ε t
  ...
  have h_sum_tendsto : Tendsto (fun ε => ∑ k : Fin N_s,
      ∫ t in γ.a..γ.b,
        cauchyPrincipalValueIntegrandOn S0
          (fun z => if k.val ≥ 1 then a_s k / (z - s) ^ (k.val + 1) else 0)
          γ.toFun ε t) (𝓝[>] 0) (𝓝 0) := by
    have h0 : (0 : ℂ) = ∑ _k : Fin N_s, (0 : ℂ) := Finset.sum_const_zero.symm
    conv_rhs => rw [h0]
...
private lemma cpv_term_tendsto_zero_of_crossed_positive_order
  ...
  have h_polar_term_tendsto : ∀ (k : Fin N_s), k.val ≥ 1 →
    ...
    exact cpv_single_polar_term_tendsto_zero S0 f s hs γ hγ_closed
      ...
      h_angle h_flat_s h_unique_s hN_s_pos kv hkv hk_ge
...
private lemma residueAt_ppMinusRes_eq_zero
  ...
  have h_ci_agree : ∀ r, 0 < r → r < ρ' →
    ...
    have h_eq_on : ∀ z ∈ Metric.sphere s r,
      ...
      have hfpp := hrp_eq ⟨Metric.mem_ball.mpr
        (by rw [Metric.mem_sphere.mp hz]; exact hr_lt_rp),
        Set.mem_compl_singleton_iff.mpr hne⟩
    ...
    have hg_diff : DifferentiableOn ℂ g_rp (Metric.ball s r) := by
      intro z hz
      exact (hg_ball z (Metric.ball_subset_ball hr_lt_rg.le hz)
        ).differentiableAt.differentiableWithinAt
    ...
    have h_int_eq : (∮ z in C(s, r), (fun z => term_s z + g_rp z) z) =
      ...
      circleIntegral.integral_congr hr_pos.le (fun z hz => by
        have := h_eq_on z hz; linear_combination this)
    calc (∮ z in C(s, r), term_s z)
        = (∮ z in C(s, r), term_s z) + 0 := (add_zero _).symm
      _ = (∮ z in C(s, r), term_s z) + (∮ z in C(s, r), g_rp z) := by rw [hg_ci_zero]
    ...
    have h_pol_tendsto : Tendsto (fun ε => ∫ t in γ.a..γ.b,
      ...
      have h_per_s_int : ∀ s ∈ S0, ∀ ε > 0,
        ...
        exact intervalIntegrable_cpvIntegrandOn_of_continuousOn_diff U S0 _
          (h_per_s_cont s hs) γ hγ_in_U ε hε
      have h_int_sum : ∀ ε > 0,
          ∫ t in γ.a..γ.b,
            cauchyPrincipalValueIntegrandOn S0 h_pol γ.toFun ε t =
          ∑ s ∈ S0, ∫ t in γ.a..γ.b,
      ...
      have h_per_s_tendsto : ∀ s ∈ S0,
        ...
        · obtain ⟨t₁, ht₁, hcross₁⟩ := h_crossed
          ...
          · subst hN_s_zero
            have hf_eq_loc' : ∀ᶠ z in 𝓝[≠] s, f z = g_loc z := by
              filter_upwards [hf_eq_loc] with z hz
              simp only [Finset.univ_eq_empty, Finset.sum_empty, add_zero] at hz
              exact hz
            exact cpv_term_tendsto_zero_of_no_poles S0 f s γ
              (hMero s hs) g_loc hg_loc_an hf_eq_loc'
          · exact cpv_term_tendsto_zero_of_crossed_positive_order U S0 f hf γ
              ...
              s hs t₁ ht₁ hcross₁ ht₁_Ioo hN_s_pos a_s g_loc hg_loc_an
              hf_eq_loc h_angle
