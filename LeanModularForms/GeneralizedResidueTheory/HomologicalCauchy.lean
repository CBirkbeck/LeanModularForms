...
private lemma dixonH1_dslope_expansion (γ : PiecewiseC1Immersion)
...
private lemma ball_avoids_curve_of_infDist_pos (γ : PiecewiseC1Immersion) (w : ℂ)
    (hinfDist_pos : 0 < Metric.infDist w (γ.toFun '' Icc γ.a γ.b)) :
...
private lemma dixonH2_differentiableAt_infDist_pos (f : ℂ → ℂ) (γ : PiecewiseC1Immersion)
  ...
  exact (dixonH2_hasDerivAt f γ hfγ_cont M_f M_d
    ...
    hM_f_spec hM_d_spec
    (le_trans (norm_nonneg _) (hM_f_spec γ.a (left_mem_Icc.mpr hab)))
    (by linarith) w
    (dist_lower_bound_at_center γ w)
    (ball_avoids_curve_of_infDist_pos γ w hinfDist_pos)
...
open GeneralizedResidueTheory in
/-- Null-homologous version of `holomorphic_cpv_tendsto_zero_on_convex`.

For a function holomorphic on an open set `U` containing a null-homologous curve `γ`,
the multi-point CPV integral tends to 0 as ε → 0. Replaces convexity with null-homologous
hypothesis: uses `contourIntegral_eq_zero_of_nullHomologous` (Dixon) instead of FTC. -/
theorem holomorphic_cpv_tendsto_zero_nullHomologous (U : Set ℂ) (hU : IsOpen U)
    (S0 : Finset ℂ) (g : ℂ → ℂ)
  ...
  have hg_cont_image : ContinuousOn g (γ.toFun '' Icc γ.a γ.b) :=
    hg.continuousOn.mono (Set.image_subset_iff.mpr (fun t ht => h_null.image_subset t ht))
  have h_ord_zero : ∫ t in γ.a..γ.b, g (γ.toFun t) * deriv γ.toFun t = 0 :=
    contourIntegral_eq_zero_of_nullHomologous hU hg γ h_null
  exact tendsto_cpv_of_continuousOn_zero_integral S0 g γ hg_cont_image h_ord_zero

open GeneralizedResidueTheory in
set_option maxHeartbeats 800000 in
/-- Null-homologous version of `generalizedResidueTheorem'`.

CPV of `f` along a null-homologous piecewise C¹ immersion equals
`2πi · Σ winding · residue`, even when `γ` crosses simple poles.
Replaces the convexity hypothesis with `IsNullHomologous γ U`. -/
theorem generalizedResidueTheorem'_nullHomologous
    (U : Set ℂ) (hU : IsOpen U)
  ...
  -- Convert hPV_singular to f_sing terms (residueSimplePole f_sing s = residueSimplePole f s)
  have hPV_singular_sing : ∀ s ∈ S0, CauchyPrincipalValueExists'
      (fun z => residueSimplePole f_sing s / (z - s)) γ.toFun γ.a γ.b s := by
    intro s hs
    rw [h_res_sing_eq s hs]; exact hPV_singular s hs
  -- Apply convex generalizedResidueTheorem' to f_sing on (univ, convex_univ)
  have h_sing_thm := generalizedResidueTheorem' Set.univ isOpen_univ convex_univ
    S (fun s _ => Set.mem_univ s) hS_discrete _hS_closed S0 hS0_subset
    f_sing hf_sing_diff γ h_null.closed (fun t _ => Set.mem_univ _)
    _hS_on_curve hSimple_sing hf_sing_ext hPV_singular_sing
  ...
  -- Key: difference ∫ cpv(f,ε) - ∫ cpv(f_sing,ε) → 0 by Dixon
  -- Because cpv(f) - cpv(f_sing) = cpv(g) and g is holomorphic on U.
  have hCancel : Tendsto
      ...
      (𝓝[>] 0) (𝓝 0) := by
    have hg_cpv_zero :=
      holomorphic_cpv_tendsto_zero_nullHomologous U hU S0 g hg_diff γ h_null
  ...
  -- From CPV(f_sing) existence + cancellation, derive CPV(f) existence + formula
  ...
  have h_f_tendsto : Tendsto (fun ε =>
    ...
    have h_eq : (fun ε => ∫ t in γ.a..γ.b,
      ...
      (fun ε =>
        ((∫ t in γ.a..γ.b, cauchyPrincipalValueIntegrandOn S0 f γ.toFun ε t) -
         (∫ t in γ.a..γ.b, cauchyPrincipalValueIntegrandOn S0 f_sing γ.toFun ε t)) +
        (∫ t in γ.a..γ.b, cauchyPrincipalValueIntegrandOn S0 f_sing γ.toFun ε t)) := by
      ext ε; ring
    rw [h_eq, show L_sing = 0 + L_sing from (zero_add _).symm]
    exact hCancel.add hL_sing
  refine ⟨⟨L_sing, h_f_tendsto⟩, ?_⟩
  -- cauchyPrincipalValueOn S0 f = limUnder(cpv(f)) = L_sing = limUnder(cpv(f_sing)) = cpv(f_sing) = formula
...
CPV of `f` along a null-homologous piecewise C¹ immersion equals
`2πi · Σ winding · residue` for simple poles. PV existence is derived
from the immersion structure (closedness + unique crossings) rather than
assumed. Replaces convexity with `IsNullHomologous γ U`. -/
theorem generalizedResidueTheorem_nullHomologous (U : Set ℂ) (hU : IsOpen U)
    (S : Set ℂ) (hS_in_U : ∀ s ∈ S, s ∈ U)
    (hS_discrete : ∀ s ∈ S, ∃ ε > 0, ∀ s' ∈ S, s' ≠ s → ε ≤ ‖s' - s‖)
    (hS_closed : IsClosed S) (S0 : Finset ℂ) (hS0_subset : ∀ s ∈ S0, s ∈ S)
    (f : ℂ → ℂ) (hf : DifferentiableOn ℂ f (U \ S0))
  ...
  -- Derive PV existence from closedness + unique crossings
  have hPV_singular : ∀ s ∈ S0, CauchyPrincipalValueExists'
    ...
    exact cpv_exists_inv_sub_of_closed_unique γ s h_null.closed
      (h_no_endpt_cross s hs) t₀ ht₀_Ioo hcross honly
  exact (generalizedResidueTheorem'_nullHomologous U hU S hS_in_U hS_discrete hS_closed
    S0 hS0_subset f hf γ h_null hS_on_curve hSimplePoles hf_ext hPV_singular).2

open GeneralizedResidueTheory in
set_option maxHeartbeats 800000 in
/-- Null-homologous version of `generalizedResidueTheorem_higher_order`.

For a meromorphic function `f` with poles of arbitrary order on a null-homologous
piecewise C¹ immersion, the Cauchy principal value equals
`2πi · Σ n(γ, s) · Res(f, s)`, provided the higher-order cancellation
hypothesis holds. Replaces convexity with `IsNullHomologous γ U`. -/
theorem generalizedResidueTheorem_higher_order_nullHomologous (U : Set ℂ) (_hU : IsOpen U)
  ...
  set f_res := fun z => ∑ s ∈ S0, residueAt f s / (z - s) with hf_res_def
  -- f_res has simple poles, is holomorphic on univ \ S0
  have hSimple_res : ∀ s ∈ S0, HasSimplePoleAt f_res s :=
    fun s hs => hasSimplePoleAt_sum_div_sub S0 (residueAt f) s hs
  have hf_res_diff_univ : DifferentiableOn ℂ f_res (Set.univ \ ↑S0) :=
    differentiableOn_sum_div_sub S0 (residueAt f) Set.univ
  ...
  have h_res_eq : ∀ s ∈ S0,
      residueSimplePole f_res s = residueAt f s :=
    fun s hs => residueSimplePole_sum_div_sub S0 (residueAt f) s hs
  -- Derive PV of each singular term from closedness + unique crossings
  have hPV_singular_res : ∀ s ∈ S0, CauchyPrincipalValueExists'
    ...
    exact cpv_exists_inv_sub_of_closed_unique γ s h_null.closed
      (h_no_endpt_cross s hs) t₀ ht₀_Ioo hcross honly
  -- Apply convex generalizedResidueTheorem' to f_res on (univ, convex_univ)
  have h_res_thm := generalizedResidueTheorem' Set.univ isOpen_univ convex_univ
    S (fun s _ => Set.mem_univ s) hS_discrete hS_closed S0 hS0_subset
    f_res hf_res_diff_univ γ h_null.closed (fun t _ => Set.mem_univ _)
    hS_on_curve hSimple_res hf_ext_res hPV_singular_res
  -- Formula for f_res with residueAt
  have h_res_formula : cauchyPrincipalValueOn S0 f_res γ.toFun γ.a γ.b =
      2 * Real.pi * I * ∑ s ∈ S0,
        generalizedWindingNumber' γ.toFun γ.a γ.b s * residueAt f s := by
    rw [h_res_thm.2]; congr 1; apply Finset.sum_congr rfl
    intro s hs; rw [h_res_eq s hs]
  -- Limit argument: CPV(f) = CPV(f_res) using hHigherOrderCancel
  obtain ⟨L_res, hL_res⟩ := h_res_thm.1
  have h_cpv_f_tendsto : Tendsto
    ...
    exact hHigherOrderCancel.add hL_res
  have h_cpv_eq : cauchyPrincipalValueOn S0 f γ.toFun γ.a γ.b =
    ...
    rw [h_cpv_f_tendsto.limUnder_eq, hL_res.limUnder_eq]
  rw [h_cpv_eq, h_res_formula]

open GeneralizedResidueTheory in
/-- Null-homologous version of `generalizedResidueTheorem_higher_order_simple`.

When all singularities are simple poles, the higher-order theorem reduces to the
simple-pole theorem: `residueAt = residueSimplePole` and the higher-order
cancellation is automatic. Replaces convexity with `IsNullHomologous γ U`. -/
theorem generalizedResidueTheorem_higher_order_simple_nullHomologous (U : Set ℂ)
    (hU : IsOpen U) (S : Set ℂ)
    (hS_in_U : ∀ s ∈ S, s ∈ U)
    (hS_discrete : ∀ s ∈ S, ∃ ε > 0, ∀ s' ∈ S, s' ≠ s → ε ≤ ‖s' - s‖)
    (hS_closed : IsClosed S) (S0 : Finset ℂ) (hS0_subset : ∀ s ∈ S0, s ∈ S)
    ...
    cauchyPrincipalValueOn S0 f γ.toFun γ.a γ.b =
      2 * Real.pi * I * ∑ s ∈ S0,
        generalizedWindingNumber' γ.toFun γ.a γ.b s * residueAt f s := by
  have h_base := generalizedResidueTheorem_nullHomologous U hU S hS_in_U hS_discrete hS_closed
    S0 hS0_subset f hf γ h_null hS_on_curve hSimplePoles hf_ext
