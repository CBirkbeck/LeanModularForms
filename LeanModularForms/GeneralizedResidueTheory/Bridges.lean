...
def toPath (γ : PiecewiseC1Curve) : Path (γ.toFun γ.a) (γ.toFun γ.b) where
  ...
  source' := by
    show γ.toFun (γ.rescale ⟨0, left_mem_Icc.mpr zero_le_one⟩) = γ.toFun γ.a
    rw [γ.rescale_zero]
  target' := by
    show γ.toFun (γ.rescale ⟨1, right_mem_Icc.mpr zero_le_one⟩) = γ.toFun γ.b
    rw [γ.rescale_one]
