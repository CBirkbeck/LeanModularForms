...
import LeanModularForms.GeneralizedResidueTheory.PVInfrastructure.SingularAnnulus
...
lemma pv_step_bound_ratio_two_uniform
  ...
  have h_sing_int :
    ...
    have h_meas_cond :
      ...
      (measurableSet_lt measurable_const
        (hγ_meas.sub_const (γ t₀)).norm).inter
        (measurableSet_le
          (hγ_meas.sub_const (γ t₀)).norm
          measurable_const)
    ...
    simp only [min_eq_left hab.le,
      max_eq_right hab.le] at ht
    have ht_Icc : t ∈ Set.Icc a b :=
      Set.Ioc_subset_Icc_self ht
    by_cases hcond :
        ε₂ < ‖γ t - γ t₀‖ ∧
          ‖γ t - γ t₀‖ ≤ ε₁
    · rw [if_pos hcond, norm_inv,
      ...
      have h_lt_δ_up : |t - t₀| < δ_up :=
        lt_of_lt_of_le
          (h_localize t ht_Icc hcond.2)
          (le_trans (min_le_right _ _)
            (min_le_right _ _))
      have h_up :=
        h_upper t h_abs_pos h_lt_δ_up
      have h_t_lower :
          ε₂ / (2 * ‖L‖) < |t - t₀| := by
        rw [div_lt_iff₀
          (by positivity : 0 < 2 * ‖L‖)]
  ...
  have h_rem_int :
    ...
    have hf_annulus_int :
      ...
      intro t _
      show (if ε₂ < ‖γ t - γ t₀‖ then f t
            else 0) -
          (if ε₁ < ‖γ t - γ t₀‖ then f t
            else 0) =
        if ε₂ < ‖γ t - γ t₀‖ ∧
          ‖γ t - γ t₀‖ ≤ ε₁
        then f t else 0
      ...
      · rw [if_pos h₂]
        ...
        · rw [if_pos h₁, sub_self,
            if_neg (fun h =>
              absurd h₁ (not_lt.mpr h.2))]
        · push_neg at h₁
          rw [if_neg (not_lt.mpr h₁), sub_zero,
            if_pos ⟨h₂, h₁⟩]
      · rw [if_neg h₂, zero_sub]
        by_cases h₁ : ε₁ < ‖γ t - γ t₀‖
        · exfalso
          exact h₂ (lt_of_le_of_lt hε₂_le h₁)
