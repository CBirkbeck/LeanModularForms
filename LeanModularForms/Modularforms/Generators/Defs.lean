...
theorem levelOne_odd_weight_eq_zero {k : ℤ} (hk : Odd k)
  ...
  have hdenom : denom (Matrix.SpecialLinearGroup.toGL
    ...
    simp
  rw [hdenom, zpow_neg, hk.neg_one_zpow, inv_neg, inv_one,
    SlashInvariantForm.toFun_eq_coe, ModularForm.toSlashInvariantForm_coe] at hmod
...
lemma Ek_q_exp_zero_pow (k : ℕ) (hk : 3 ≤ (k : ℤ)) (hk2 : Even k) (n : ℕ) :
  ...
  rw [Ek_q_exp_zero k hk hk2, one_pow]
...
lemma evalE₄E₆_Delta_poly :
  ...
  simp [Delta_poly, evalE₄E₆_X0, evalE₄E₆_X1]
...
lemma delta_mem_range_evalE₄E₆ :
  ...
  · simp only [DirectSum.smul_apply, DirectSum.sub_apply, DirectSum.of_eq_of_ne _ _ _ hi]
    have he4 : ((DirectSum.of (fun k : ℤ ↦ ModularForm (CongruenceSubgroup.Gamma 1) k) 4 E₄)
      ...
      exact DirectSum.of_eq_of_ne _ _ _ (by lia)
...
lemma evalE₄E₆_whc_eq_single (n : ℕ) (p : MvPolynomial (Fin 2) ℂ)
  ...
  · rw [DirectSum.of_eq_of_ne _ _ _ hk]
    conv_lhs => rw [← MvPolynomial.support_sum_monomial_coeff p, map_sum]
    ...
    rw [hd0, mul_assoc, map_mul, evalE₄E₆_C, Algebra.algebraMap_eq_smul_one, smul_mul_assoc,
      one_mul, DirectSum.smul_apply, evalE₄E₆_monomial, DirectSum.ofPow, DirectSum.ofPow,
      DirectSum.of_mul_of]
    ...
    rw [DirectSum.of_eq_of_ne _ _ _ (by
      ...
      have : Finsupp.weight E₄E₆Weight d = d 0 * 4 + d 1 * 6 := by
        ...
        simp [Fin.sum_univ_two, E₄E₆Weight, mul_comm, smul_eq_mul]
      rw [this] at hweight
      push_cast [← hweight]
      ring), smul_zero]
