...
public import LeanModularForms.Modularforms.Eisensteinqexpansions
...
/-!
# Eisenstein series E₄, E₆ and related q-expansions

This file develops the basic theory of the normalised weight-`4` and weight-`6` Eisenstein
series `E₄` and `E₆` for the full modular group `SL₂(ℤ)`, together with the auxiliary
functions `φ₀`, `φ₂'`, `φ₄'` and their extensions to `ℂ`. It establishes the q-expansions
of `E₄` and `E₆`, the q-expansion of the discriminant `Δ` to first order, reality of
`E₂`, `E₄`, `E₆` on the imaginary axis and boundedness of `E₂` at infinity.

## Main definitions

* `E₄`, `E₆`: the normalised Eisenstein series of weights `4` and `6`.
* `φ₀`, `φ₂'`, `φ₄'`: auxiliary holomorphic functions on `ℍ` used in the dimension argument
  for `M₁₂(Γ(1))`.
* `Delta_E4_E6_aux`: the cusp form realising `Δ` as `(E₄³ - E₆²) / 1728`.

## Main results

* `q_exp_unique`: uniqueness of the q-expansion of a modular form.
* `Ek_q_exp`: explicit q-expansion of `Eₖ` for even `k ≥ 4`.
* `E4_q_exp`, `E6_q_exp`: explicit q-expansion formulae for `E₄` and `E₆`.
* `Delta_q_one_term`: the first q-expansion coefficient of `Δ` is `1`.
* `E_even_imag_axis_real`: `Eₖ(it)` is real for `t > 0` and even `k ≥ 4`.
...
lemma exists_smul_eq_of_rank_one {M : Type*} [AddCommGroup M] [Module ℂ M]
  ...
  let ⟨c, hc⟩ := (finrank_eq_one_iff_of_nonzero' e he).mp
...
lemma E₄_S_transform (z : ℍ) : E₄ (ModularGroup.S • z) = z ^ (4 : ℕ) * E₄ z := by
  have h : (E₄.toFun ∣[(4 : ℤ)] ModularGroup.S) z = E₄.toFun z := by
    refine congrFun (E₄.slash_action_eq' _ ?_) z
    simp only [Subgroup.mem_map, CongruenceSubgroup.mem_Gamma_one]
    exact ⟨ModularGroup.S, trivial, rfl⟩
...
/- `φ₀`, `φ₋₂` and `φ₋₄`, except minus signs cannot appear in subscripts of definitions. -/
/-- The auxiliary function `(E₂ E₄ - E₆)² / Δ`, holomorphic on `ℍ`. -/
def φ₀ (z : ℍ) := (E₂ z * E₄ z - E₆ z) ^ 2 / Δ z

/-- The auxiliary function `E₄ (E₂ E₄ - E₆) / Δ`, holomorphic on `ℍ`. -/
def φ₂' (z : ℍ) := E₄ z * (E₂ z * E₄ z - E₆ z) / Δ z

/-- The auxiliary function `E₄² / Δ`, holomorphic on `ℍ`. -/
def φ₄' (z : ℍ) := E₄ z ^ 2 / Δ z

/-- Extension of `φ₀` to all of `ℂ`, vanishing outside the upper half-plane. -/
...
/-- If a modular form's q-expansion `∑ c m · qᵐ` agrees with `f τ` for every `τ ∈ ℍ`, then on the
unit punctured disc the same series converges to `cuspFunction n f q`. -/
theorem cuspfunc_lim_coef {k : ℤ} {F : Type*} [FunLike F ℍ ℂ] (n : ℕ) (c : ℕ → ℂ) (f : F)
    [ModularFormClass F Γ(n) k] [hn : NeZero n]
    (hf : ∀ τ : ℍ, HasSum (fun m ↦ c m • 𝕢 ↑n ↑τ ^ m) (f τ)) (q : ℂ) (hq : ‖q‖ < 1) (hq1 : q ≠ 0) :
    HasSum (fun m ↦ c m • q ^ m) (cuspFunction n f q) := by
...
lemma tsum_zero_pow (f : ℕ → ℂ) : (∑' m, f m * 0 ^ m) = f 0 := by
  rw [(summable_zero_pow f).tsum_eq_zero_add]
  simp
...
/-- The q-expansion `∑ c m · qᵐ` converges to `cuspFunction n f q` on the open unit disc,
including at `q = 0`. -/
lemma modfom_q_exp_cuspfunc (c : ℕ → ℂ) (f : F) [ModularFormClass F Γ(n) k] [NeZero n]
    (hf : ∀ τ : ℍ, HasSum (fun m : ℕ ↦ c m • 𝕢 n τ ^ m) (f τ)) (q : ℂ) (hq : ‖q‖ < 1) :
    HasSum (fun m : ℕ ↦ c m • q ^ m) (cuspFunction n f q) := by
  by_cases hq1 : q ≠ 0
  · exact cuspfunc_lim_coef n c f hf q hq hq1
  have hlim := cuspfunc_lim_coef n c f hf
  rw [cuspFunction] at hlim
  have h2 : cuspFunction n f 0 = c 0 := by
    ...
    have htt : Tendsto (fun q ↦ ∑' m, c m * q ^ m) (𝓝[≠] 0) (𝓝 (c 0)) := by
      ...
      · simpa [tsum_zero_pow] using hD
      · simpa using
          (hlim (1 / 2) (by norm_num) (one_div_ne_zero (two_ne_zero))).summable.norm
      · intro k
        refine Tendsto.const_mul _ ?_
        exact ((continuous_pow k (M := ℂ)).tendsto 0).mono_left nhdsWithin_le_nhds
      rw [eventually_iff_exists_mem]
      refine ⟨{z | (z : ℂ) ≠ 0 ∧ ‖z‖ < 1 / 2}, ?_, ?_⟩
      · rw [mem_nhdsWithin_iff]
        refine ⟨1/2, by norm_num, fun y hy ↦ ?_⟩
        simp only [mem_inter_iff, mem_ball, dist_zero_right, mem_compl_iff, mem_singleton_iff,
          mem_setOf_eq] at hy ⊢
        exact ⟨hy.2, hy.1⟩
      · intro y hy k
        ...
        rw [← inv_pow]
        gcongr
        simpa only [one_div] using hy.2.le
    refine htt.congr' ?_
    rw [eventuallyEq_nhdsWithin_iff, eventually_nhds_iff_ball]
    refine ⟨1, zero_lt_one, fun y hy hy0 ↦ ?_⟩
    simp only [mem_ball, dist_zero_right, mem_compl_iff, mem_singleton_iff] at hy hy0
    exact (hlim y hy hy0).tsum_eq
  push_neg at hq1
  simp_rw [hq1, h2, smul_eq_mul]
  exact (summable_zero_pow _).hasSum_iff.mpr (tsum_zero_pow _)


/-- The q-parameter `𝕢 n` surjects onto every positive real less than `1` on the upper
half-plane: for `0 < r < 1` there is `z ∈ ℍ` with `‖𝕢 n z‖ = r`. -/
lemma qParam_surj_onto_ball (r : ℝ) (hr : 0 < r) (hr2 : r < 1) [NeZero n] :
  ...
  · rw [Function.Periodic.im_invQParam]
    rw [mul_pos_iff]
    refine .inr ⟨?_, ?_⟩
    · refine div_neg_of_neg_of_pos ?_ two_pi_pos
      simpa using Nat.pos_of_neZero n
    · simpa [Real.norm_of_nonneg hr.le] using (log_neg_iff hr).mpr hr2
  · show ‖𝕢 n (Periodic.invQParam n r : ℂ)‖ = r
    rw [Function.Periodic.qParam_right_inv hn_ne hr_ne]
    simp [hr.le]


/-- Uniqueness of the q-expansion: if a sequence `c` satisfies `∑ c m · qᵐ = f τ` for every
`τ ∈ ℍ`, then `c` agrees with the q-expansion coefficients of `f`. -/
lemma q_exp_unique (c : ℕ → ℂ) (f : ModularForm Γ(n) k) [hn : NeZero n]
  ...
  have hqq2 : ∀ m, ‖qq m‖ = ‖qExpansion2.coeff m‖ := fun m ↦ by
    simp only [qq]
  ...
  have H2 : HasFPowerSeriesOnBall (cuspFunction n f) qq 0 1 := by
    have H21 : 1 ≤ qq.radius := by
      ...
      conv => enter [1, n]; rw [hqq2]
      simp only [PowerSeries.coeff_mk, qExpansion2]
      by_cases hr0 : r = 0
      · rw [hr0]; exact summable_zero_pow _
      obtain ⟨z, hz⟩ := qParam_surj_onto_ball n r (by simpa using pos_iff_ne_zero.mpr hr0)
        (by simpa using hr)
      simpa [← hz] using summable_norm_iff.mpr (hf z).summable
    refine ⟨H21, zero_lt_one, fun {y} hy ↦ ?_⟩
    rw [Metric.mem_eball, edist_zero_right, enorm_eq_nnnorm, ENNReal.coe_lt_one_iff,
      ← NNReal.coe_lt_one, coe_nnnorm] at hy
    simp
    have hms := modfom_q_exp_cuspfunc n c f hf y hy
    apply hms.congr
    intro S
    congr
    ext b
  ...
  have h3 : HasFPowerSeriesAt (cuspFunction n f) qq 0 := ⟨1, H2⟩
  have h4 : HasFPowerSeriesAt (cuspFunction n f) (qExpansionFormalMultilinearSeries n f) 0 :=
    ⟨1, h⟩
  have h5 := (FormalMultilinearSeries.ext_iff.mp
    (HasFPowerSeriesAt.eq_formalMultilinearSeries h3 h4)) m
  ...
  have htv : c m • ContinuousMultilinearMap.mkPiAlgebraFin ℂ m ℂ =
    ...
    rw [h5]; simp [FormalMultilinearSeries.ofScalars]
  have h6 := congrArg
...
/-- Pointwise Leibniz rule: `(fg)' = f'g + fg'` for differentiable `f, g : ℂ → ℂ`. -/
...
/-- The `n`-th coefficient of `(qExpansion E₄) * (qExpansion E₆)` is the convolution
of their coefficients. -/
lemma qExpansion_E4_mul_E6_coeff (n : ℕ) :
    (PowerSeries.coeff n) (qExpansion 1 E₄ * qExpansion 1 E₆) =
      ∑ p ∈ Finset.antidiagonal n,
        (PowerSeries.coeff p.1) (qExpansion 1 E₄) * (PowerSeries.coeff p.2) (qExpansion 1 E₆) :=
  PowerSeries.coeff_mul _ _ _

/-- Crude bound: the divisor sum `σ_k(n)` is bounded by `n^(k+1)`. -/
lemma sigma_bound (k n : ℕ) : σ k n ≤ n ^ (k + 1) := by
  rw [ArithmeticFunction.sigma_apply, pow_add, pow_one, mul_comm]
  have h1 : ∑ d ∈ n.divisors, d ^ k ≤ ∑ d ∈ n.divisors, n ^ k :=
    Finset.sum_le_sum fun i hi ↦ by gcongr; exact Nat.divisor_le hi
  refine h1.trans ?_
  simp only [Finset.sum_const, smul_eq_mul]
  gcongr
  exact Nat.card_divisors_le_self n

/-- The coefficient sequence of the q-expansion of `Eₖ` for even `k ≥ 4`: `1` at index `0` and
`(2πi)^k / ((k-1)! ζ(k)) · σ_{k-1}(m)` at index `m ≥ 1`. -/
def Ek_q (k : ℕ) : ℕ → ℂ := fun m ↦ if m = 0 then 1 else
    (1 / riemannZeta k) * ((-2 * ↑π * Complex.I) ^ k / (k - 1)!) * σ (k-1) m

/-- The series `∑ Ek_q k m · qᵐ` is summable on the open unit disc. -/
...
/-- Explicit q-expansion coefficients of the Eisenstein series `Eₖ` for even `k ≥ 4`: the
constant term is `1` and the `m`-th coefficient (`m ≥ 1`) is `(2πi)^k σ_{k-1}(m) / ((k-1)! ζ(k))`. -/
lemma Ek_q_exp (k : ℕ) (hk : 3 ≤ (k : ℤ)) (hk2 : Even k) :
  ...
  intro z
  have hexp := E_k_q_expansion k hk hk2 z
  rw [Summable.hasSum_iff]
  · simp at hexp
    rw [hexp, tsum_eq_zero_add']
...
/-- The constant term of the q-expansion of `E₄` is `1`. -/
...
theorem Complex.I_pow_six : Complex.I ^ 6 = -1 := by
  rw [show (6 : ℕ) = 2 * 3 by norm_num, pow_mul, I_sq]
  ring
...
theorem bernoulli'_five : bernoulli' 5 = 0 := by
  ...
  rw [bernoulli'_def]
  norm_num [Finset.sum_range_succ, Finset.sum_range_zero, this]
...
theorem bernoulli'_six : bernoulli' 6 = 1 / 42 := by
  ...
  rw [bernoulli'_def]
  norm_num [Finset.sum_range_succ, Finset.sum_range_zero, h1, h2]

/-- Explicit q-expansion of `E₆`: constant term `1`, `m`-th coefficient `-504 σ₅(m)`. -/
lemma E6_q_exp : (fun m ↦ (qExpansion 1 E₆).coeff m) =
    fun m ↦ if m = 0 then 1 else -(504 : ℂ) * σ 5 m := by
  have HH := Ek_q_exp 6 (by norm_num) (by decide)
  rw [E6_eq]
  simp at *
  rw [HH]
...
/-- The constant coefficient of `(E₄³ - E₆²) / 1728` vanishes — this is the q-expansion
identity that lets us realise `Δ` as `(E₄³ - E₆²) / 1728` as an honest cusp form. -/
theorem E4E6_coeff_zero_eq_zero :
    (PowerSeries.coeff 0)
        (qExpansion 1
          ((1 / 1728 : ℂ) • ((DirectSum.of (ModularForm Γ(1)) 4) E₄ ^ 3 -
            (DirectSum.of (ModularForm Γ(1)) 6) E₆ ^ 2) 12)) = 0 := by
...
/-- The cusp function of `Δ` on the disc `‖q‖ < 1/2` is `q · ∏ (1 - qⁿ⁺¹)²⁴`. -/
lemma Delta_cuspFunction_eq : Set.EqOn (cuspFunction 1 Delta)
  ...
  · simp
    ...
    · rw [ofComplex_apply_of_im_pos hz, Delta_apply, Δ]
      ...
      · simp
        have hyrw : cexp (2 * ↑π * Complex.I * Periodic.invQParam 1 y) = y := by
          nth_rw 2 [← hq]; congr 1; simp
        rw [hyrw]
        congr
        ext n
        congr
        have : cexp (2 * ↑π * Complex.I * (↑n + 1) * Periodic.invQParam 1 y) =
          ...
          rw [← Complex.exp_nsmul]; congr; ring
        rw [this]
        congr
...
@[deprecated (since := "2026-05-30")] alias Delta_cuspFuntion_eq := Delta_cuspFunction_eq

/-- `Δ` is not the zero modular form (it is nonzero at `i`). -/
lemma Delta_ne_zero : Delta ≠ 0 :=
  DFunLike.ne_iff.mpr ⟨UpperHalfPlane.I, Δ_ne_zero UpperHalfPlane.I⟩

/-- The finite products `∏ₓ<n (1 - yˣ⁺¹)` converge locally uniformly on `‖y‖ < 1/2` to
`∏'_i (1 - yⁱ⁺¹)`. -/
lemma tendstoLocallyUniformlyOn_partialProducts_disc : TendstoLocallyUniformlyOn
    (fun n : ℕ ↦ fun y : ℂ ↦ ∏ x ∈ Finset.range n, (1 - y ^ (x + 1)))
    (fun x : ℂ ↦ ∏' i, (1 - x ^ (i + 1))) atTop
    (Metric.ball (0 : ℂ) (1/2 : ℝ)) := by
  have hclosed :
...
/-- The infinite product `∏' (1 - yⁿ⁺¹)²⁴` is differentiable within the disc `‖y‖ < 1/2`
at the origin. -/
theorem differentiableWithinAt_tprod_one_sub_pow_disc :
    DifferentiableWithinAt ℂ (fun y : ℂ ↦ ∏' i : ℕ, (1 - y ^ (i + 1)) ^ 24)
      (ball 0 (1 / 2)) 0 := by
  suffices h : DifferentiableWithinAt ℂ
      (fun n : ℂ ↦ (∏' i : ℕ, (1 - n ^ (i + 1))) ^ 24) (ball 0 (1 / 2)) 0 by
    refine h.congr (fun x hx ↦ ?_) (by simp)
    rw [← tprod_pow _ (multipliable_lt_one x (by simp at *; apply lt_trans hx; exact
      two_inv_lt_one))]
  refine DifferentiableWithinAt.pow ?_ _
  have hu := tendstoLocallyUniformlyOn_partialProducts_disc.differentiableOn ?_ isOpen_ball
  · exact hu _ (by simp)
  · simp only [eventually_atTop]
    ...
    simpa [Finset.prod_fn] using
      DifferentiableOn.finset_prod (u := Finset.range b)
        ...
        (s := Metric.ball 0 (1 / 2)) (fun _ _ ↦ by fun_prop)

@[deprecated (since := "2026-05-30")]
alias diffwithinat_prod_1 := differentiableWithinAt_tprod_one_sub_pow_disc


/-- The first q-expansion coefficient of `Δ` is `1`. -/
lemma Delta_q_one_term : (qExpansion 1 Delta).coeff 1 = 1 := by
  rw [qExpansion_coeff]
  simp
  rw [← derivWithin_of_isOpen (s := Metric.ball 0 (1 / 2 : ℝ)) isOpen_ball (by simp),
    derivWithin_congr Delta_cuspFunction_eq]
...
/-- The first q-expansion coefficient of `E₄³` is `3 · 240`. -/
...
/-- Dividing a non-cusp form by its constant q-coefficient normalises the q-expansion so
that the `0`-th coefficient is `1`. -/
lemma modularForm_normalise (f : ModularForm Γ(1) k) (hf : ¬ IsCuspForm Γ(1) k f) :
  ...
  refine inv_mul_cancel₀ fun h ↦ hf ?_
  rwa [← IsCuspForm_iff_coeffZero_eq_zero] at h
...
/-- `Eₖ(it)` is real for all `t > 0` when `k` is even and `k ≥ 4`. -/
theorem E_even_imag_axis_real (k : ℕ) (hk : (3 : ℤ) ≤ k) (hk2 : Even k) :
  ...
  have hterm_im : ∀ n : ℕ+, (↑((ArithmeticFunction.sigma (k - 1)) ↑n) *
      cexp (2 * ↑Real.pi * Complex.I * z * n)).im = 0 := fun n ↦ by
    ...
    · intro n
      calc ‖↑((ArithmeticFunction.sigma (k - 1)) ↑n) * cexp (2 * ↑Real.pi * Complex.I * z * n)‖
        ...
        _ ≤ ‖(↑n : ℂ) ^ k‖ * ‖cexp (2 * ↑Real.pi * Complex.I * z * n)‖ := by
          ...
          rw [Nat.sub_add_cancel (by omega : 1 ≤ k)] at hbound
  ...
  have hzeta_im : (riemannZeta k).im = 0 := by
    rw [show (k : ℂ) = ((k : ℝ) : ℂ) by push_cast; ring]
...
theorem E₂_imag_axis_real : ResToImagAxis.Real E₂ := by
  ...
  have hterm_im : ∀ n : ℕ+, (↑n * cexp (2 * ↑Real.pi * Complex.I * n * z) /
      (1 - cexp (2 * ↑Real.pi * Complex.I * n * z))).im = 0 := fun n ↦ by
  ...
  have hsum : Summable fun n : ℕ+ ↦ ↑n * cexp (2 * ↑Real.pi * Complex.I * n * z) /
    ...
    refine (hs.comp_injective PNat.coe_injective).congr fun n ↦ ?_
  ...
  have hsum_im : (∑' (n : ℕ+), ↑n * cexp (2 * ↑Real.pi * Complex.I * n * z) /
    ...
    rw [Complex.im_tsum hsum]; simp [hterm_im]
...
/-- `E₂` is bounded at infinity. Uses `E₂_eq`: `E₂(z) = 1 - 24·∑ n·qⁿ/(1-qⁿ)` with
`q = exp(2πi z)`; on `im z ≥ 1` we have `‖q‖ ≤ exp(-2π)` so the tail is bounded. -/
