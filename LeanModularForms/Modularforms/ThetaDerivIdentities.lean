...
It also derives the equivalent ordinary-derivative identities `D_H₂`, `D_H₃`, `D_H₄`,
and the identity `E₄ = H₂² + H₂H₄ + H₄²` (as `E₄_eq_H_sum_sq`).

## Strategy

We define error terms `f₂`, `f₃`, `f₄ = LHS - RHS` and prove their transformation rules
under the `S` and `T` generators of `SL(2,ℤ)`: `f₂|S = -f₄`, `f₂|T = -f₂`, `f₄|S = -f₂`,
`f₄|T = f₃`. From these we construct level-1 modular forms `theta_g` (weight 6) and
`theta_h` (weight 8) that vanish at infinity, hence are cusp forms. By dimension
vanishing, weight-`< 12` level-1 cusp forms are zero, so `theta_g = theta_h = 0`, from
which `f₂ = f₃ = f₄ = 0` follows.
-/
...
lemma f₄_decompose :
    f₄ = serre_D (2 : ℤ) H₄ + ((1/6 : ℂ) • (H₄ * ((2 : ℂ) • H₂ + H₄))) := rfl
...
/-- The error terms satisfy `f₂ + f₄ = f₃` (from the Jacobi identity). -/
...
lemma f₄_T_action : (f₄ ∣[(4 : ℤ)] T) = f₃ := by
  ...
  rw [f₄_decompose, add_slash, SL_smul_slash, h_serre_term, h_prod]
  ext z; simp [f₃]
  rw [show H₃ z = H₂ z + H₄ z from (congrFun jacobi_identity z).symm]; ring
...
lemma theta_g_T_action : (theta_g ∣[(6 : ℤ)] T) = theta_g := by
  ...
  have h_term1 : ((((2 : ℂ) • H₂ + H₄) * f₂) ∣[(6 : ℤ)] T) = (-(2 : ℂ) • H₂ + H₃) * (-f₂) := by
    ...
    simpa only [h_2H₂_H₄, f₂_T_action] using hmul
  have h_term2 : (((H₂ + (2 : ℂ) • H₄) * f₄) ∣[(6 : ℤ)] T) = (-H₂ + (2 : ℂ) • H₃) * f₃ := by
    have hmul := mul_slash_SL2 2 4 T (H₂ + (2 : ℂ) • H₄) f₄
    simpa only [h_H₂_2H₄, f₄_T_action] using hmul
  simp only [theta_g, add_slash, h_term1, h_term2]
...
/-- `theta_g` as a `SlashInvariantForm` of level 1 and weight 6. -/
...
/-- `theta_h` as a `SlashInvariantForm` of level 1 and weight 8. -/
...
private lemma H_sum_sq_MDifferentiable : MDiff H_sum_sq := by
  unfold H_sum_sq
  exact ((H₂_SIF_MDifferentiable.pow 2).add (H₂_SIF_MDifferentiable.mul H₄_SIF_MDifferentiable)).add
    (H₄_SIF_MDifferentiable.pow 2)

private lemma H_sum_sq_tendsto : Tendsto H_sum_sq atImInfty (𝓝 1) := by
  unfold H_sum_sq
  simpa [sq] using
...
lemma f₄_sq_mul_eq (z : ℍ) (hg_z : theta_g z = 0) :
  ...
  have h2 : A ^ 2 * (f₂ z * f₄ z) = -(A * B * f₄ z ^ 2) := by
    have h_cross : (A * f₂ z) * (A * f₄ z) = (-(B * f₄ z)) * (A * f₄ z) := by rw [hAf₂]
...
/-- The error term `f₂` vanishes (from `theta_g = theta_h = 0` and `H_sum_sq → 1 ≠ 0`). -/
lemma f₂_eq_zero : f₂ = 0 := by
  ...
  have h_f₄_sq_3H : f₄ ^ 2 * (fun z => 3 * H_sum_sq z) = 0 := by
    ext z; simp only [Pi.mul_apply, Pi.pow_apply, Pi.zero_apply]
...
lemma f₃_eq_zero : f₃ = 0 := by
  rw [← f₂_add_f₄_eq_f₃]; simp [f₂_eq_zero, f₄_eq_zero]
