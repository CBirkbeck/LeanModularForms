...
Define special generators `S`, `T`, `-I` (resp. `α`, `β`, `-I`) for `SL(2, ℤ)` (resp. `Γ(2)`)
and prove that they are indeed generators. As a corollary, we only need to check invariance
under these special elements to check invariance under the whole group. These results are
used to prove that 4-th powers of Jacobi theta functions `Θ_2^4`, `Θ_3^4`, `Θ_4^4` are
modular forms of weight `2` and level `Γ(2)`.
-/
...
/-- The element `α = !![1, 2; 0, 1]` of `Γ(2)`, one of three generators. -/
def α : Γ 2 := ⟨⟨!![1, 2; 0, 1], by simp⟩, by simp; decide⟩

/-- The element `β = !![1, 0; 2, 1]` of `Γ(2)`, one of three generators. -/
def β : Γ 2 := ⟨⟨!![1, 0; 2, 1], by simp⟩, by simp; decide⟩

/-- The element `-I = !![-1, 0; 0, -1]` of `Γ(2)`, one of three generators. -/
def negI : Γ 2 := ⟨⟨!![-1, 0; 0, -1], by simp⟩, by simp⟩

/-- The generator `α` of `Γ(2)` equals `T ^ 2` in `SL(2, ℤ)`. -/
theorem α_eq_T_sq : α = ⟨T ^ 2, by simp [sq, T]; decide⟩ := by ext; simp [α, T, sq]

/-- The generator `β` of `Γ(2)` factors as `negI * S * α⁻¹ * S`. -/
theorem β_eq_negI_mul_S_mul_α_inv_mul_S : β = negI * S * α⁻¹ * S := by ext; simp [β, S, α, negI]

/-- The square of `negI` is the identity in `SL(2, ℤ)`. -/
theorem ModularGroup.modular_negI_sq : negI ^ 2 = 1 := by
  ext i j; fin_cases i <;> fin_cases j <;> rfl

/-- The element `negI` is self-inverse. -/
...
/-- The action of `negI` on `ℍ` is trivial. -/
theorem modular_negI_smul : negI.1 • z = z := by rw [specialLinearGroup_apply]; simp [negI]

/-- For even weight `k`, the slash action by `negI` is the identity. -/
...
/-- The slash action of `S` on `f` at `z`. -/
...
/-- The slash action of `T` on `f` at `z`. -/
...
/-- `SL(2, ℤ)` is generated by the matrices `S` and `T`. -/
...
/-- An element of `Γ(2)` whose `(1, 0)` entry vanishes lies in the subgroup generated by
`{α, β, -I}`. -/
lemma Γ2_c_eq_zero (A : Γ 2) (h : A.1 1 0 = 0) : A ∈ Subgroup.closure {α, β, negI} := by
  ...
  · obtain ⟨val, property⟩ := A
    ...
    | inl h_1 =>
      ...
      have h_alpha_k : val = α ^ k := Subtype.ext <| by
        rw [α_zpow_val]; ext i j; fin_cases i <;> fin_cases j <;> simp_all
      exact h_alpha_k.symm ▸ Subgroup.zpow_mem _ hα_mem _
    | inr h_2 =>
      have h11 : val.val 1 1 = -1 := by
        have := val.2; rw [Matrix.det_fin_two] at this; grind
      have h_val : val = negI * α ^ (-k) := Subtype.ext <| by
      ...
      exact h_val ▸ Subgroup.mul_mem _ hnegI_mem (Subgroup.zpow_mem _ hα_mem (-k))
  · exfalso
    have := A.1.2; rw [Matrix.det_fin_two] at this
    simp_all [Int.mul_eq_one_iff_eq_one_or_neg_one]

/-- Given an odd `a` and a nonzero even `c`, some multiple `a + 2nc` has absolute value
...
/-- Given a nonzero odd `a` and an even `c`, some multiple `c + 2ma` has absolute value
...
private lemma Γ2_even_10 (A : Γ 2) : Even (A.val.val 1 0) := by
  have := A.2; simp only [Gamma_mem] at this
  exact_mod_cast even_iff_two_dvd.mpr ((ZMod.intCast_zmod_eq_zero_iff_dvd _ 2).mp this.2.2.1)

/-- For any `A ∈ Γ(2)` with `A 1 0 ≠ 0`, there is an element `M` of the subgroup generated by
`{α, β, -I}` such that `|(M * A) 1 0| < |A 1 0|`. -/
lemma Γ2_descent (A : Γ 2) (h : A.1 1 0 ≠ 0) :
  ...
  obtain ⟨n, hn⟩ := Γ2_reduce_row (A.val.val 0 0) (A.val.val 1 0) h_odd h_even h
...
/-- `Γ(2)` is generated by the three matrices `α`, `β`, `-I`. -/
theorem Γ2_generate : (⊤ : Subgroup (Γ 2)) = Subgroup.closure {α, β, negI} := by
  ...
  obtain ⟨M, hM₁, hM₂⟩ :
    ...
    obtain ⟨M, hM₁, hM₂⟩ := Nat.sInf_mem h_nonempty
    exact ⟨M, hM₁, hM₂.symm⟩
  by_cases hd_pos : d > 0
...
/-- If `f` is invariant under the slash action of `S` and `T`, then it is invariant under the
slash action of every element of `SL(2, ℤ)`. -/
...
/-- If `f` is invariant under the slash action of `S` and `T`, then it is invariant under the
slash action of every element of the image of `Γ(1) = SL(2, ℤ)` in `GL(2, ℝ)`. -/
...
/-- If `f` is invariant under the slash action of `α`, `β`, and `-I`, then it is invariant
under the slash action of every element of the image of `Γ(2)` in `GL(2, ℝ)`. -/
