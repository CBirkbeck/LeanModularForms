...
public import LeanModularForms.Modularforms.SerreDerivativeSlash
...
lemma tendsto_exp_neg_mul_atTop {c : ℝ} (hc : 0 < c) :
    Filter.Tendsto (fun y : ℝ ↦ Real.exp (-c * y)) Filter.atTop (nhds 0) :=
  Real.tendsto_exp_comp_nhds_zero.mpr <|
    Filter.tendsto_id.const_mul_atTop_of_neg (neg_neg_of_pos hc)
...
lemma E₂_sub_one_isBigO_exp : (fun z : ℍ ↦ E₂ z - 1) =O[atImInfty]
  ...
  have hexp_lt_half : Real.exp (-2 * π) < 1 / 2 := by
    have h1 : 1 < 2 * π := by nlinarith [pi_gt_three]
  ...
  have hsum_le_8q : ‖q‖ / (1 - ‖q‖) ^ 3 ≤ 8 * ‖q‖ := by
    ...
    calc ‖q‖ / (1 - ‖q‖) ^ 3 ≤ ‖q‖ / (1 / 8) :=
          div_le_div_of_nonneg_left (norm_nonneg _) (by positivity) h1
      _ = 8 * ‖q‖ := by ring
...
/-- General limit: if `f → c` at i∞ and `f` is holomorphic and bounded, then
`serre_D k f → -k * c / 12`. -/
lemma serre_D_tendsto_of_tendsto (k : ℤ) (f : ℍ → ℂ) (c : ℂ) (hf_holo : MDiff f)
    (hf_bdd : IsBoundedAtImInfty f) (hf_lim : Filter.Tendsto f atImInfty (nhds c)) :
...
lemma serre_D_tendsto_neg_k_div_12 (k : ℤ) (f : ℍ → ℂ) (hf_holo : MDiff f)
    (hf_bdd : IsBoundedAtImInfty f) (hf_lim : Filter.Tendsto f atImInfty (nhds 1)) :
...
lemma serre_D_tendsto_zero_of_tendsto_zero (k : ℤ) (f : ℍ → ℂ) (hf_holo : MDiff f)
    (hf_bdd : IsBoundedAtImInfty f) (hf_lim : Filter.Tendsto f atImInfty (nhds 0)) :
...
/-- `serre_D 1 E₂` is a weight-4 modular form (even though `E₂` itself is not). -/
def serre_DE₂_ModularForm : ModularForm (CongruenceSubgroup.Gamma 1) 4 where
  toSlashInvariantForm :=
    { toFun := serre_D 1 E₂
      slash_action_eq' γ hγ := by
        obtain ⟨γ', _, rfl⟩ := Subgroup.mem_map.mp hγ
        exact serre_DE₂_slash_invariant γ' }
  holo' := serre_D_differentiable E₂_holo'
  bdd_at_cusps' hc := bounded_at_cusps_of_bounded_at_infty hc fun _ hA ↦ by
...
lemma summable_pow_shift (k : ℕ) :
  ...
  have h_eq : ∀ m : ℕ, (m + 1 : ℝ) ^ k * rexp (-2 * π * m) =
    ...
    rw [show rexp (-2 * π * m) = rexp (2 * π) * rexp (-2 * π * (m + 1)) by
      rw [← Real.exp_add]; ring_nf]
    ring
  simp_rw [h_eq]
  refine Summable.mul_left _ <| .congr (h.comp_injective Nat.succ_injective) fun m ↦ by simp

/-- Derivative bounds for q-expansion coefficients: given `‖a n‖ ≤ n^k`, produces bounds
`‖a n * 2πin * exp(2πin z)‖ ≤ 2π * n^(k+1) * exp(-2πn * y_min)`
on compact `K ⊆ {z : 0 < z.im}`. -/
lemma qexp_deriv_bound_of_coeff_bound {a : ℕ+ → ℂ} {k : ℕ}
  ...
  · obtain ⟨k_min, hk_min_mem, hk_min_le⟩ := hK_compact.exists_isMinOn hK_nonempty
    ...
    have hconv : Summable (fun n : ℕ+ ↦
      ...
      ext n; ring
    refine ⟨fun n ↦ 2 * π * (n : ℝ)^(k + 1) * rexp (-2 * π * ↑n * k_min.im), ?_, ?_⟩
    · refine hconv.of_nonneg_of_le (fun n ↦ by positivity) fun n ↦ ?_
      exact (show -2 * π * ↑↑n * k_min.im = -(2 * π * k_min.im) * ↑↑n by ring) ▸ le_rfl
    · intro n ⟨z, hz_mem⟩
      have hz_im : k_min.im ≤ z.im := hk_min_le hz_mem
      have hn_pos : (0 : ℝ) < n := by exact_mod_cast n.pos
      have h_norm_2pin : ‖(2 : ℂ) * π * I * ↑↑n‖ = 2 * π * n := by
        simp [Real.norm_of_nonneg pi_pos.le]
      calc ‖a n * (2 * π * I * ↑↑n) * cexp (2 * π * I * ↑↑n * z)‖
        ...
        _ ≤ (n : ℝ)^k * (2 * π * n) * rexp (-2 * π * n * z.im) := by
            ...
            have hexp : ‖cexp (2 * π * I * ↑↑n * z)‖ ≤ rexp (-2 * π * n * z.im) := by
              rw [Complex.norm_exp]
              have hre : (2 * π * I * ↑↑n * z).re = -2 * π * n * z.im := by simp
              rw [hre]
            gcongr; exact ha n
        _ ≤ (n : ℝ)^k * (2 * π * n) * rexp (-2 * π * n * k_min.im) := by
            refine mul_le_mul_of_nonneg_left ?_ (by positivity)
            exact Real.exp_le_exp_of_le (by
              refine mul_le_mul_of_nonpos_left hz_im ?_
              nlinarith [pi_pos, hn_pos])
        _ = 2 * π * (n : ℝ)^(k + 1) * rexp (-2 * π * n * k_min.im) := by ring
  · exact ⟨fun _ ↦ 0, summable_zero, fun _ ⟨z, hz_mem⟩ ↦ absurd ⟨z, hz_mem⟩ hK_nonempty⟩
