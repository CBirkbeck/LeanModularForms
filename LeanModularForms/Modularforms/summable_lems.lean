...
/-!
# Summability lemmas for modular forms

This file collects summability and convergence results used in the theory of modular forms,
including summability of lattice sums, iterated derivatives of Eisenstein series, and
tsum reorganisation lemmas for modular forms.
-/
...
theorem summable_neg {α : Type*} [TopologicalSpace α] [AddCommMonoid α] (f : ℤ → α)
  ...
  (negEquiv.summable_iff (f := f)).mpr hf
...
theorem nat_pos_tsum2 {α : Type*} [TopologicalSpace α] [AddCommMonoid α]
    (f : ℕ → α) (hf : f 0 = 0) : (Summable fun x : ℕ+ ↦ f x) ↔ Summable f := by
  refine PNat.coe_injective.summable_iff fun x hx ↦ ?_
  simp only [mem_range, not_exists] at hx
  obtain (rfl | h) := Nat.eq_zero_or_pos x
  · exact hf
  simpa using hx ⟨x, h⟩

theorem tsum_pNat {α : Type*} [AddCommGroup α] [UniformSpace α] [IsUniformAddGroup α] [T2Space α]
...
lemma tsum_pnat_eq_tsum_succ4 {α : Type*} [TopologicalSpace α] [AddCommGroup α]
  ...
  rw [Summable.tsum_eq_zero_add hf, add_right_inj, tsum_pnat_eq_tsum_succ]
...
theorem int_nat_sum {α : Type*} [AddCommGroup α] [UniformSpace α] [IsUniformAddGroup α]
    [CompleteSpace α] (f : ℤ → α) : Summable f → Summable fun x : ℕ ↦ f x :=
  fun h ↦ (summable_int_iff_summable_nat_and_neg.mp h).1
...
private lemma aus (a b : ℂ) : a + b ≠ 0 ↔ a ≠ -b :=
...
theorem upbnd (z : ℍ) (d : ℤ) : (d ^ 2 : ℝ) * r z ^ 2 ≤ ‖((z : ℂ) ^ 2 - d ^ 2)‖ := by
  ...
  · have h1 : (z ^ 2 : ℂ) - d ^ 2 = d ^ 2 * (1 / d * z - 1) * (1 / d * z + 1) := by
    ...
    · have h1 := auxbound2 z ((d : ℝ)⁻¹) (d := -1) (by norm_num)
      exact h1.trans_eq (by congr; simp only [ofReal_inv, ofReal_intCast, ofReal_neg, ofReal_one]; ring)
    · have h1 := auxbound2 z ((d : ℝ)⁻¹) (d := 1) (by norm_num)
      exact h1.trans_eq (by congr; simp [ofReal_inv, ofReal_intCast])
...
theorem lhs_summable (z : ℍ) : Summable fun n : ℕ+ ↦ 1 / ((z : ℂ) - n) + 1 / (z + n) := by
  ...
  apply mul_pos
  · exact_mod_cast pow_pos (r_pos z) _
  have hb := b.2
  exact_mod_cast pow_pos (by simpa using hb) _
...
private lemma pow_max (x y : ℕ) : (max x y) ^ 2 = max (x ^ 2) (y ^ 2) := by
  rcases le_total x y with h | h <;> simp [Nat.pow_le_pow_left, h]
...
private lemma aux (a b c : ℝ) (ha : 0 < a) (hb : 0 < b) (_hc : 0 < c) :
    a⁻¹ ≤ c * b⁻¹ ↔ b ≤ c * a := by
...
theorem summable_diff_denom (z : ℍ) (i : ℤ) :
  ...
  exact summable_inv_of_isBigO_rpow_inv one_lt_two
    (((linear_bigO' i z).mul (linear_bigO' (i+1) z)).congr
      (fun n ↦ by rw [mul_comm]; simp; ring)
      (fun n ↦ by norm_cast; rw [pow_two, ← mul_inv]; simp))
...
lemma summable_pain (z : ℍ) (i : ℤ) :
  ...
  have h := summable_diff_denom z i
  rwa [← Finset.summable_compl_iff (s := {0})] at h
...
lemma G_2_alt_summable (z : ℍ) : Summable fun (m : Fin 2 → ℤ) ↦
  ...
  · exact mul_nonneg (Real.rpow_nonneg (r_pos z).le (-1))
      (Real.rpow_nonneg (norm_nonneg _) (-1))
...
lemma G_2_alt_summable_δ (z : ℍ) : Summable fun (m : Fin 2 → ℤ) ↦
  ...
  · exact absurd (by nth_rw 1 [← h1.1, ← h1.2]; exact List.ofFn_inj.mp rfl) hb1
  · exact absurd (by nth_rw 1 [← h2.1, ← h2.2]; exact List.ofFn_inj.mp rfl) hb2
  rfl
...
theorem summable_1 (k : ℕ) (z : ℍ) (hk : 1 ≤ k) :
  ...
  exact hl2.congr_left fun n ↦ by rw [@neg_inv]; congr; ring
...
theorem summable_3 (m : ℕ) (y : {z : ℂ | 0 < z.im}) :
  ...
  · refine Summable.add ?_ ?_
    · have h0 := summable_1 m (⟨y, y.2⟩ : ℍ) hm1
      simp only [mem_setOf_eq, one_div] at *
      exact h0.subtype _
    have h1 := summable_2 m (⟨y, y.2⟩ : ℍ) hm1
    simp only [mem_setOf_eq, one_div] at *
    exact h1.subtype _
  simp [Nat.factorial_ne_zero]

theorem summable_iter_derv' (k : ℕ) (y : ℍ') :
    Summable fun n : ℕ ↦ (2 * ↑π * Complex.I * n) ^ k * Complex.exp (2 * ↑π * Complex.I * n * y) := by
  ...
  conv =>
    ...
    rw [← norm_pow, ← norm_mul,
      show cexp (2 * (↑π * (Complex.I * (↑n * ↑y)))) = cexp (2 * (↑π * (Complex.I * (↑y)))) ^ n from
...
theorem sigma_eq_sum_div' (k n : ℕ) : sigma k n = ∑ d ∈ Nat.divisors n, (n / d) ^ k := by
  simp only [sigma, ArithmeticFunction.coe_mk]; rw [← Nat.sum_div_divisors]
...
lemma hsum (k : ℕ) (z : ℍ) : Summable fun b : ℕ+ ↦ ∑ _ ∈ (b : ℕ).divisors, b ^ k *
  ...
  rw [← mul_assoc]; gcongr; rw [pow_add, mul_comm]; gcongr
...
theorem summable_auxil_1 (k : ℕ) (z : ℍ) :
  ...
  · simp only [Complex.norm_mul, norm_pow, RCLike.norm_natCast, tsum_fintype, Finset.univ_eq_attach]
    ...
    · intro b
      ...
      exact le_of_eq (by rw [mul_assoc]; norm_cast; rw [Nat.mul_div_cancel' hi])
  · intro i
    simp
...
theorem exp_series_ite_deriv_uexp2 (k : ℕ) (x : {z : ℂ | 0 < z.im}) :
  ...
  | succ k IH =>
    ...
    · exact x.2
    · intro y hy
      exact (summable_iter_derv' k ⟨y, hy⟩).congr fun b ↦ (exp_iter_deriv_within k b hy).symm
    · intro K hK1 hK2
...
theorem iter_der_within_add (k : ℕ+) (x : {z : ℂ | 0 < z.im}) :
  ...
  · congr
    rw [exp_series_ite_deriv_uexp2 k x]
    exact tsum_congr fun b ↦ by simpa using exp_iter_deriv_within k b x.2
...
lemma sub_bound (s : ℍ) (A B : ℝ) (hB : 0 < B) (hs : s ∈ verticalStrip A B) (k : ℕ)
  ...
  gcongr
  have := summand_bound_of_mem_verticalStrip (k := (k + 2)) (by norm_cast; lia) ![1,-n] hB hs
  ...
  · have H := UpperHalfPlane.subset_verticalStrip_of_isCompact hk2
    ...
    · apply Summable.mul_left
      ...
      have : Summable fun (i : ℕ) ↦ ((i : ℝ) ^ ((k : ℤ) + 2))⁻¹ := by
        exact ((Real.summable_nat_rpow_inv (p := k + 2)).mpr (by linarith)).congr (by intro n; norm_cast)
      apply this.subtype
    intro n s
    rw [← iteratedDerivWithin_succ]
...
theorem tsum_aexp_contDiffOn (k : ℕ) :
  ...
  · intro K hK1 hK2
    ...
    simpa using h21
  intro n r
  apply DifferentiableOn.differentiableAt _ (IsOpen.mem_nhds (isOpen_lt (by fun_prop) (by fun_prop)) r.2)
  simpa using diff_on_aux m n
...
theorem aux_iter_der_tsum (k : ℕ) (hk : 1 ≤ k) (x : ℍ) :
  ...
  · apply tsum_aexp_contDiffOn k
    exact x.2
  · exact x.2
  · exact (isOpen_lt (by fun_prop) (by fun_prop)).uniqueDiffOn
...
theorem q_exp_iden (k : ℕ) (hk : 2 ≤ k) (z : ℍ) :
  ...
  have h3 : (-1) ^ ((k - 1)) * ↑(k - 1)! * ((-(2 * ↑π * Complex.I)) ^ k / ↑(k - 1)!) = -(2 * ↑π *
    ...
    have h2 : (↑(k - 1)! : ℂ) * (↑(k - 1)!)⁻¹ = 1 := by
      rw [mul_inv_cancel₀]; norm_cast; exact Nat.factorial_ne_zero _
    have hkk : (k - 1) + k = 2 * k - 1 := by
      rw [add_comm, ← Nat.add_sub_assoc (by linarith), two_mul]
    ...
    rw [hj, h2, mul_one, mul_comm, neg_pow, mul_comm, ← mul_assoc, ← pow_add]
    rw [Odd.neg_one_pow]
    · ring
    rw [hkk]
  ...
  have hee :
    ∑' n : ℕ+, (2 * ↑π * Complex.I * ((n : ℕ) : ℂ)) ^ ((k - 1) : ℕ) *
      ...
      (2 * ↑π * Complex.I) ^ (k - 1) * ∑' n : ℕ+, n ^ (k - 1) * exp (2 * ↑π * Complex.I * ↑z * n) := by
    rw [← tsum_mul_left]
    apply tsum_congr
    intro b
    rw [← mul_assoc]
  ...
  have he2 : 2 * ↑π * Complex.I * (2 * ↑π * Complex.I) ^ (k - 1) = (2 * ↑π * Complex.I) ^ k := by
    ...
    nth_rw 2 [hke]; norm_cast; rw [pow_add]; simp
  rw [he2]
...
theorem summable_exp_pow (z : ℍ) : Summable fun i : ℕ ↦
  ...
  exact exp_upperHalfPlane_lt_one z

theorem a1 (k : ℕ) (e : ℕ+) (z : ℍ) :
    Summable fun c : ℕ ↦ (e : ℂ) ^ (k - 1) * exp (2 * ↑π * Complex.I * ↑z * e * c) := by
  apply Summable.mul_left
  apply Summable.of_norm
  simp_rw [show ∀ a : ℕ, cexp (2 * ↑π * Complex.I * ↑z * ↑↑e * ↑↑a) = cexp (2 * ↑π * Complex.I * ↑z * ↑↑e) ^ (a : ℕ)
    from fun a ↦ by rw [← Complex.exp_nat_mul]; ring_nf]
  have h1 : ‖cexp (2 * ↑π * Complex.I * ↑z * ↑↑e)‖ < 1 := by
    ...
    positivity
  exact summable_norm_iff.mpr (summable_geometric_iff_norm_lt_one.mpr h1)
...
lemma t9 (z : ℍ) : ∑' m : ℕ,
  ...
  · have := (a4 2 z).prod_symm
    ...
    ring_nf
  · intro e
    simpa only [pow_one] using (a33 (k := 1) e z).congr (by intro b; ring_nf)
  · intro e
    simpa [Nat.add_one_sub_one, pow_one] using (a1 2 e z).subtype _
  exact (a4 2 z).congr (by intro b; simp [uncurry]; congr 1; ring)

lemma summable_pnats (f : ℕ → ℂ) : Summable (fun n : ℕ+ ↦ f n) ↔ Summable f :=
