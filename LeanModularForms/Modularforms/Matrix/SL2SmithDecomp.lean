...
lemma isUnit_of_dvd_entries_of_primitive2
    ...
    {d : ℤ} (h00 : d ∣ A 0 0) (h01 : d ∣ A 0 1)
    (h10 : d ∣ A 1 0) (h11 : d ∣ A 1 1) :
    IsUnit d := by
  -- Transport each ℤ-divisibility to ℕ-divisibility via `Int.natAbs_dvd_natAbs`.
  have hn00 : d.natAbs ∣ (A 0 0).natAbs := Int.natAbs_dvd_natAbs.mpr h00
  have hn01 : d.natAbs ∣ (A 0 1).natAbs := Int.natAbs_dvd_natAbs.mpr h01
  have hn10 : d.natAbs ∣ (A 1 0).natAbs := Int.natAbs_dvd_natAbs.mpr h10
  have hn11 : d.natAbs ∣ (A 1 1).natAbs := Int.natAbs_dvd_natAbs.mpr h11
  -- Combine divisibilities via `Nat.dvd_gcd` to reach the nested gcd.
  have h_row0 : d.natAbs ∣ Nat.gcd (A 0 0).natAbs (A 0 1).natAbs :=
    Nat.dvd_gcd hn00 hn01
  have h_row1 : d.natAbs ∣ Nat.gcd (A 1 0).natAbs (A 1 1).natAbs :=
    Nat.dvd_gcd hn10 hn11
  have h_all : d.natAbs ∣ Nat.gcd (Nat.gcd (A 0 0).natAbs (A 0 1).natAbs)
                                  (Nat.gcd (A 1 0).natAbs (A 1 1).natAbs) :=
    Nat.dvd_gcd h_row0 h_row1
  rw [hprim] at h_all
  -- `d.natAbs ∣ 1 ⇒ d.natAbs = 1 ⇒ IsUnit d`.
...
lemma dvd_entries_of_mul_diag_mul_of_dvd
  ...
  -- Expand the product entry to the explicit 2-term sum.
  have h_prod :
      (U * !![d₀, 0; 0, d₁] * Q) i j =
    ...
    simp [Matrix.mul_apply, Fin.sum_univ_two, Matrix.cons_val_zero,
          Matrix.cons_val_one]
  rw [h_prod]
  -- First summand is visibly divisible by `d₀`; second uses `d₀ ∣ d₁`.
  refine dvd_add ⟨U i 0 * Q 0 j, ?_⟩ ?_
  · ring
  · exact (h.mul_left (U i 1)).mul_right (Q 1 j)

/-! ### Sign-flip matrix and GL → SL sign normalisation -/
...
@[simp]
lemma signFlip2_mul_self : signFlip2 * signFlip2 = 1 := by
  ext i j
  fin_cases i <;> fin_cases j <;>
    simp [signFlip2, Matrix.mul_apply, Fin.sum_univ_two]
...
/-! ### Bezout row/column reduction over `SL(2, ℤ)` -/
...
lemma sl2Bezout_col_zero_out (a b : ℤ) (h_ne : a ≠ 0 ∨ b ≠ 0) :
  ...
  have hd_ne : (Int.gcd a b : ℤ) ≠ 0 := by
    ...
    rcases h_ne with ha' | hb' <;> [exact ha' ha; exact hb' hb]
  ...
  · -- Both entries of the resulting column are explicit ℤ expressions.
...
lemma sl2Bezout_row_zero_out (a b : ℤ) (h_ne : a ≠ 0 ∨ b ≠ 0) :
  ...
  -- `(a, b) ᵥ* Bᵀ = (B ᵥ* (a, b)ᵀ)ᵀ`.  Cʹs role is `Bᵀ`; we transpose.
...
/-! ### CRT existence of coprime row combination -/
...
lemma exists_good_residue_mod_prime_zmod
  ...
  have ha : a = 0 := h0.1
  have hb : b = 0 := h0.2
  simp only [ha, hb, zero_add, one_mul] at h1
  exact hne ⟨ha, hb, h1.1, h1.2⟩
...
lemma exists_good_residue_mod_prime {a b c d : ℤ} {p : ℕ} [Fact p.Prime]
  ...
  -- Translate the non-all-divisible hypothesis to the ZMod level.
  ...
  -- Convert the ZMod-level "not both zero" back to ℤ-divisibility.
  intro ⟨h_ac, h_bd⟩
  apply hr
  refine ⟨?_, ?_⟩
  · have : ((a + (r.val : ℤ) * c : ℤ) : ZMod p) = 0 :=
      (ZMod.intCast_zmod_eq_zero_iff_dvd _ _).mpr h_ac
    simpa [Int.cast_add, Int.cast_mul, Int.cast_natCast, ZMod.natCast_val,
           ZMod.cast_id] using this
  · have : ((b + (r.val : ℤ) * d : ℤ) : ZMod p) = 0 :=
      (ZMod.intCast_zmod_eq_zero_iff_dvd _ _).mpr h_bd
    simpa [Int.cast_add, Int.cast_mul, Int.cast_natCast, ZMod.natCast_val,
           ZMod.cast_id] using this

/-! ### Main CRT existence lemma -/
...
lemma exists_k_coprime_of_primitive
  ...
  have hP_not_all_dvd : ∀ p ∈ M.primeFactors,
    ...
    have hp_nat : p ∣ a.natAbs := Int.natAbs_dvd_natAbs.mpr h_a
    have hp_nat' : p ∣ b.natAbs := Int.natAbs_dvd_natAbs.mpr h_b
    have hp_nat'' : p ∣ c.natAbs := Int.natAbs_dvd_natAbs.mpr h_c
    have hp_nat''' : p ∣ d.natAbs := Int.natAbs_dvd_natAbs.mpr h_d
    have h_ab : p ∣ Nat.gcd a.natAbs b.natAbs := Nat.dvd_gcd hp_nat hp_nat'
    have h_cd : p ∣ Nat.gcd c.natAbs d.natAbs := Nat.dvd_gcd hp_nat'' hp_nat'''
    have h_all : p ∣ Nat.gcd (Nat.gcd a.natAbs b.natAbs) (Nat.gcd c.natAbs d.natAbs) :=
      Nat.dvd_gcd h_ab h_cd
    rw [hprim] at h_all
    exact (hP_prime p hp).one_lt.ne' (Nat.dvd_one.mp h_all)
  -- Extract good residue per prime via classical choice.
  classical
  ...
  -- The integer witness is `k_nat : ℤ`.
  refine ⟨(k_nat : ℤ), ?_⟩
  -- Suppose `gcd(a + k_nat·c, b + k_nat·d) ≠ 1`; derive contradiction via CRT.
  rw [Int.isCoprime_iff_gcd_eq_one]
  by_contra h_not_one
  -- Some prime `q` divides the gcd, so divides both entries.
  set G := Int.gcd (a + (k_nat : ℤ) * c) (b + (k_nat : ℤ) * d) with hG_def
  have hG_ne_one : G ≠ 1 := h_not_one
  -- Handle G = 0 separately: then a + k_nat*c = 0 and b + k_nat*d = 0, so det A = 0.
  rcases eq_or_ne G 0 with hG_zero | hG_ne
  · rw [hG_def] at hG_zero
    have h_both_zero : a + (k_nat : ℤ) * c = 0 ∧ b + (k_nat : ℤ) * d = 0 := by
      rcases Int.gcd_eq_zero_iff.mp hG_zero with ⟨h1, h2⟩
      exact ⟨h1, h2⟩
    have h_det_zero : a * d - b * c = 0 := by
      have : (a + (k_nat : ℤ) * c) * d - (b + (k_nat : ℤ) * d) * c = 0 := by
        rw [h_both_zero.1, h_both_zero.2]; ring
      linarith [this, show (a + (k_nat : ℤ) * c) * d - (b + (k_nat : ℤ) * d) * c =
        a * d - b * c from by ring]
    exact hdet h_det_zero
  -- G ≠ 0, G ≠ 1, so G ≥ 2, and some prime q divides G.
  have hG_gt_one : 1 < G := by
    rcases Nat.lt_or_ge G 1 with h_lt | h_ge
    · interval_cases G
      · exact (hG_ne rfl).elim
    · omega
  obtain ⟨q, hq_prime, hq_dvd_G⟩ := Nat.exists_prime_and_dvd hG_gt_one.ne'
  have hq_pos : (q : ℤ) ≠ 0 := by exact_mod_cast hq_prime.pos.ne'
  -- q divides G, hence q divides both a + k_nat*c and b + k_nat*d as ℤ values.
  have hq_dvd_ac : (q : ℤ) ∣ a + (k_nat : ℤ) * c := by
    have h_nat : (q : ℤ) ∣ (G : ℤ) := by exact_mod_cast hq_dvd_G
    exact h_nat.trans (Int.gcd_dvd_left _ _)
  have hq_dvd_bd : (q : ℤ) ∣ b + (k_nat : ℤ) * d := by
    have h_nat : (q : ℤ) ∣ (G : ℤ) := by exact_mod_cast hq_dvd_G
    exact h_nat.trans (Int.gcd_dvd_right _ _)
  -- Hence q divides det A, so q ∈ M.primeFactors.
  have hq_dvd_det : (q : ℤ) ∣ a * d - b * c :=
    dvd_det_of_dvd_row_combination hq_dvd_ac hq_dvd_bd
  have hq_dvd_M : q ∣ M := by
    rw [hM_def]
    exact_mod_cast (Int.natAbs_dvd_natAbs.mpr hq_dvd_det)
  have hq_mem : q ∈ M.primeFactors :=
    Nat.mem_primeFactors.mpr ⟨hq_prime, hq_dvd_M, hM_ne⟩
  -- By CRT, k_nat ≡ r_nat q at prime q. So the "good residue" property at q
  -- is inherited by k_nat, contradicting that q divides both row entries.
  have h_cong : k_nat ≡ r_nat q [MOD q] := hk_cong q hq_mem
  have : (q : ℤ) ∣ (k_nat : ℤ) - (r_nat q : ℤ) := by
  ...
  -- So (q : ℤ) | (a + k_nat*c) - (a + r_nat q * c) = (k_nat - r_nat q) * c.
  have hq_dvd_ac' : (q : ℤ) ∣ a + (r_nat q : ℤ) * c := by
    have h_eq : a + (k_nat : ℤ) * c - (a + (r_nat q : ℤ) * c) =
        ((k_nat : ℤ) - (r_nat q : ℤ)) * c := by ring
    have h_diff : (q : ℤ) ∣ ((k_nat : ℤ) - (r_nat q : ℤ)) * c :=
      this.mul_right c
    have : (q : ℤ) ∣ (a + (k_nat : ℤ) * c) - (a + (r_nat q : ℤ) * c) := by
      rw [h_eq]; exact h_diff
    exact (dvd_sub_right hq_dvd_ac).mp this
  have hq_dvd_bd' : (q : ℤ) ∣ b + (r_nat q : ℤ) * d := by
    have h_eq : b + (k_nat : ℤ) * d - (b + (r_nat q : ℤ) * d) =
        ((k_nat : ℤ) - (r_nat q : ℤ)) * d := by ring
    have h_diff : (q : ℤ) ∣ ((k_nat : ℤ) - (r_nat q : ℤ)) * d :=
      this.mul_right d
    have : (q : ℤ) ∣ (b + (k_nat : ℤ) * d) - (b + (r_nat q : ℤ) * d) := by
      rw [h_eq]; exact h_diff
    exact (dvd_sub_right hq_dvd_bd).mp this
  exact hr_good q hq_mem ⟨hq_dvd_ac', hq_dvd_bd'⟩

/-! ### Smith decomposition for 2×2 primitive matrices with positive determinant -/
...
theorem smith_decomp_of_primitive_posDet {A : Matrix (Fin 2) (Fin 2) ℤ}
    ...
    ∃ U V : Matrix.SpecialLinearGroup (Fin 2) ℤ,
      (U : Matrix (Fin 2) (Fin 2) ℤ) * A * (V : Matrix (Fin 2) (Fin 2) ℤ) =
        !![1, 0; 0, A.det] := by
  -- Determinant in entry form, and `≠ 0`.
  have hdet_ne : A 0 0 * A 1 1 - A 0 1 * A 1 0 ≠ 0 := by
    have heq : A.det = A 0 0 * A 1 1 - A 0 1 * A 1 0 := Matrix.det_fin_two A
    rw [heq] at hdet_pos; exact hdet_pos.ne'
  -- CRT produces `k : ℤ` making first-row combination coprime.
  ...
  have hne : (A 0 0 + k * A 1 0) ≠ 0 ∨ (A 0 1 + k * A 1 1) ≠ 0 := by
    rcases eq_or_ne (A 0 0 + k * A 1 0) 0 with h1 | h1
    · rcases eq_or_ne (A 0 1 + k * A 1 1) 0 with h2 | h2
      · have : Int.gcd (A 0 0 + k * A 1 0) (A 0 1 + k * A 1 1) = 0 := by
          rw [h1, h2]; rfl
        omega
      · exact Or.inr h2
    · exact Or.inl h1
  -- Upper-triangular elementary `E(k) = !![1, k; 0, 1] ∈ SL(2, ℤ)`.
  let Ek : Matrix.SpecialLinearGroup (Fin 2) ℤ :=
    ⟨!![1, k; 0, 1], by rw [Matrix.det_fin_two_of]; ring⟩
  -- Bezout row reduction.
  ...
  -- Entry-wise verification `↑U₂ * B = !![1, 0; 0, A.det]`.
  show (!![1, 0; -(B 1 0), 1] : Matrix (Fin 2) (Fin 2) ℤ) * B = !![1, 0; 0, A.det]
  ext i j
  fin_cases i <;> fin_cases j
  · show ((!![1, 0; -(B 1 0), 1] : Matrix (Fin 2) (Fin 2) ℤ) * B) 0 0 = 1
    simp [Matrix.mul_apply, Fin.sum_univ_two, hB_00]
  · show ((!![1, 0; -(B 1 0), 1] : Matrix (Fin 2) (Fin 2) ℤ) * B) 0 1 = 0
    simp [Matrix.mul_apply, Fin.sum_univ_two, hB_01]
  · show ((!![1, 0; -(B 1 0), 1] : Matrix (Fin 2) (Fin 2) ℤ) * B) 1 0 = 0
    simp [Matrix.mul_apply, Fin.sum_univ_two, hB_00]
  · show ((!![1, 0; -(B 1 0), 1] : Matrix (Fin 2) (Fin 2) ℤ) * B) 1 1 = A.det
    simp [Matrix.mul_apply, Fin.sum_univ_two, hB_01, hB_11]
