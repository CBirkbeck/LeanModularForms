...
/-!
# Big-O estimates for linear functions over the upper half-plane

This file collects `IsBigO` estimates for linear maps `n ↦ (m * z + n)⁻¹` and related
expressions, used in proving uniform convergence of Eisenstein series.
-/
...
lemma linear_bigO (m : ℤ) (z : ℍ) :
    (fun (n : ℤ) => ((m : ℂ) * z + n)⁻¹) =O[cofinite] fun n => |(n : ℝ)|⁻¹ := by
  have h1 : (fun (n : ℤ) => ((m : ℂ) * z + n)⁻¹) =O[cofinite]
    ...
    have hr : (r z)⁻¹ = |r z|⁻¹ := by
      simp only [inv_inj]
      exact (abs_eq_self.mpr (r_pos z).le).symm
...
lemma Asymptotics.IsBigO.of_neg {α β : Type*} [Norm α] [Norm β] {f : ℤ → α} {g : ℤ → β}
  ...
  rw [← Equiv.neg_apply]
  exact hf.comp_tendsto <| Injective.tendsto_cofinite (Equiv.injective (Equiv.neg ℤ))

lemma linear_bigO_nat (m : ℤ) (z : ℍ) :
    (fun (n : ℕ) => ((m : ℂ) * z + n)⁻¹) =O[cofinite] fun n => |(n : ℝ)|⁻¹ :=
  (linear_bigO m z).zify

lemma linear_bigO' (m : ℤ) (z : ℍ) :
    (fun (n : ℤ) => ((n : ℂ) * z + m)⁻¹) =O[cofinite] fun n => |(n : ℝ)|⁻¹ := by
