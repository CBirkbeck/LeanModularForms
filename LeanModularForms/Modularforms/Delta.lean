...
lemma DiscriminantProductFormula (z : ℍ) : Δ z = cexp (2 * π * Complex.I * z) * ∏' (n : ℕ+),
  ...
  simp [Δ]
  conv => enter [1,1]; ext n; rw [show (n : ℂ) + 1 = ((n + 1) : ℕ) by simp]
  exact (tprod_pnat_eq_tprod_succ (f := fun n => (1 - cexp (2 * π * Complex.I * (n) * z)) ^ 24)).symm
...
lemma Delta_eq_eta_pow (z : ℍ) : Δ z = (η z) ^ 24 := by
  ...
  · rw [Periodic.qParam, ← Complex.exp_nat_mul]
    congr 1
    simp [field]
  · ext n; simp [ModularForm.eta_q_eq_cexp]

lemma Δ_ne_zero (z : UpperHalfPlane) : Δ z ≠ 0 := by
  rw [Delta_eq_eta_pow]
  exact pow_ne_zero _ (by simpa [ModularForm.eta] using ModularForm.eta_ne_zero z.2)
...
lemma Discriminant_S_invariant : (Δ ∣[(12 : ℤ)] ModularGroup.S) = Δ := by
  ...
  have hi : -1 / (z.1 : ℂ) = (-(z : ℂ))⁻¹ := by rw [neg_div, ← neg_inv]; simp
  rw [hi] at he
...
lemma I_in_atImInfty (A : ℝ) : {z : ℍ | A ≤ z.im} ∈ atImInfty :=
  (atImInfty_mem _).mpr ⟨A, fun _ h => h⟩
...
lemma atImInfy_pnat_mono (S : Set ℍ) (hS : S ∈ atImInfty) (B : ℝ) : ∃ A : ℝ,
  ...
  obtain ⟨A, hA⟩ := (atImInfty_mem _).mp hS
  refine ⟨A, fun n s hs => ?_, ?_⟩
  · simp only [mem_inter_iff, mem_setOf_eq] at *
    have K : max A B ≤ (n • s).im := by
      ...
      exact ⟨hs2.1.trans <| (le_mul_iff_one_le_left s.2).mpr hn,
        hs2.2.trans <| (le_mul_iff_one_le_left s.2).mpr hn⟩
    refine ⟨?_, K⟩
    simp at K
    exact hA _ K.1
  · simp only [inter_mem_iff, hS, true_and]; exact I_in_atImInfty _
...
theorem tendsto_neg_cexp_atImInfty (k : ℕ) :
  ...
  rw [show (0 : ℂ) = -0 from neg_zero.symm]
  refine Tendsto.neg <| tendsto_exp_nhds_zero_iff.mpr ?_
  simp
  exact Filter.Tendsto.const_mul_atTop (by positivity) tendsto_comap
...
theorem log_one_neg_cexp_tendto_zero (k : ℕ) :
  ...
  rw [show (fun x : ℍ ↦ Complex.log ((1 - cexp (2 * ↑π * Complex.I * (↑k + 1) * ↑x)) ^ 24)) =
    Complex.log ∘ (fun x => x ^ 24) ∘ (fun x : ℍ ↦ 1 - cexp (2 * π * Complex.I * (k + 1) * x))
    from by ext x; simp]
  refine Tendsto.comp (y := 𝓝 1) ?_ (Tendsto.comp (y := 𝓝 1)
    ((continuous_pow 24).tendsto' (1 : ℂ) (1 : ℂ) (by simp)) ?_)
  · nth_rw 1 [← Complex.log_one]
    exact (continuousAt_clog (by simp)).tendsto
  · simp_rw [sub_eq_add_neg]
    nth_rw 3 [show (1 : ℂ) = 1 + 0 by ring]
    exact tendsto_const_nhds.add (tendsto_neg_cexp_atImInfty k)
...
lemma Complex.cexp_tsum_eq_tprod_func (f : ι → α → ℂ) (hfn : ∀ x n, f n x ≠ 0)
  ...
  ext a
  refine (HasProd.tprod_eq <| ((hf a).hasSum.cexp).congr fun _ => ?_).symm
  congr
...
theorem Delta_boundedfactor :
  ...
  · refine Tendsto.comp (y := 𝓝 0)
      (Complex.continuous_exp.tendsto' 0 1 Complex.exp_zero) ?_
    have := tendsto_tsum_of_dominated_convergence (𝓕 := atImInfty) (g := fun (_ : ℕ) => (0 : ℂ))
        (f := fun x : ℍ ↦ fun (n : ℕ) => Complex.log ((1 - cexp (2 * ↑π * Complex.I * (↑n + 1) *
          (x : ℂ))) ^ 24))
        (bound := fun k => ‖24 * ((3/2) * cexp (2 * ↑π * Complex.I * (↑k + 1) * Complex.I))‖)
    simp at this
    apply this
    · exact (Summable.mul_left _ <| Summable.mul_left _ <|
        by simpa using summable_exp_pow UpperHalfPlane.I)
    · exact log_one_neg_cexp_tendto_zero
    · have h0 := tendsto_neg_cexp_atImInfty 0
      ...
      obtain ⟨A, hA, hAmem⟩ := atImInfy_pnat_mono (min a a2) hminmem 1
      refine ⟨(a ⊓ a2) ∩ {z | A ⊔ 1 ≤ z.im}, hAmem, fun b hb k => ?_⟩
  ...
  · intro x n
    ...
    exact term_ne_zero x n
  · intro x
    refine (log_summable_pow (fun n => -cexp (2 * ↑π * Complex.I * (↑n + 1) * x)) ?_ 24).congr
...
def Delta : CuspForm (CongruenceSubgroup.Gamma 1) 12 where
  ...
  holo' := by
    ...
    have he2 : DifferentiableOn ℂ (fun z => (η z) ^ 24) {z | 0 < z.im} :=
      DifferentiableOn.pow (fun x hx => (ModularForm.differentiableAt_eta_of_mem_upperHalfPlaneSet
        (z := x) hx).differentiableWithinAt.congr (by simp [ModularForm.eta])
        (by simp [ModularForm.eta])) _
    rw [Discriminant_SIF]
    simp only [SlashInvariantForm.coe_mk]
...
lemma Delta_isTheta_rexp : Delta =Θ[atImInfty] (fun τ => Real.exp (-2 * π * τ.im)) := by
  ...
  have hb := Delta_boundedfactor.norm
  simp only [norm_one] at hb
  have hl := Filter.Tendsto.eventually_const_le one_half_lt_one hb
  ...
  have hA3 := hA2 z hz
  conv => enter [2,2,1]; rw [Complex.norm_exp]
  ...
  conv at h4 => enter [1]; rw [mul_comm, ← mul_assoc]; simp
...
theorem div_Delta_is_SIF (k : ℤ) (f : CuspForm (CongruenceSubgroup.Gamma 1) k) (γ : GL (Fin 2) ℝ)
  ...
  have hmem := Subgroup.mem_map_of_mem (Matrix.SpecialLinearGroup.mapGL ℝ) hA₁
  have h0 : (⇑f / ⇑Delta) z = (⇑f z / ⇑Delta z) := rfl
  have h1 : (⇑f / ⇑Delta) (γ • z) = (⇑f (γ • z) / ⇑Delta (γ • z)) := rfl
  have h2 : (f.toFun ∣[k] γ) z = f.toFun z := congrFun (f.slash_action_eq' _ hmem) z
  have h3 : (Delta.toFun ∣[(12 : ℤ)] γ) z = Delta.toFun z :=
    congrFun (Delta.slash_action_eq' _ hmem) z
...
def CuspForm_div_Discriminant (k : ℤ) (f : CuspForm (CongruenceSubgroup.Gamma 1) k) :
  ...
  holo' := by
    rw [mdifferentiable_iff]
    simp only [SlashInvariantForm.coe_mk]
    show DifferentiableOn ℂ ((⇑f ∘ ↑ofComplex) / (Delta ∘ ↑ofComplex)) _
    refine DifferentiableOn.div ?_ ?_ fun x hx => ?_
  ...
  bdd_at_cusps' {c} hc := by
    ...
    exact div_le_div₀ (by positivity) (hA2 _ hz.1) (by positivity) (hB3 _ hz.2)
...
lemma cexp_aux1 (t : ℝ) : cexp (2 * ↑π * Complex.I * (Complex.I * t)) = rexp (-2 * π * t) :=
  calc
...
lemma Complex.im_finset_prod_eq_zero_of_im_eq_zero {ι : Type*} (s : Finset ι)
    (f : ι → ℂ) (h : ∀ i ∈ s, (f i).im = 0) : (∏ i ∈ s, f i).im = 0 := by
  classical
...
lemma Delta_imag_axis_real : ResToImagAxis.Real Δ := by
  ...
  have him_g : ∀ n, (g n).im = 0 := fun n => by
    have h1 : (cexp (-(2 * (π : ℂ) * ((n + 1) : ℂ) * t))).im = 0 := by
      simpa [mul_comm, mul_left_comm, mul_assoc] using cexp_aux4 t n
    have h2 : ((1 - cexp (2 * (π : ℂ) * Complex.I * (n + 1) * (Complex.I * t))) : ℂ).im = 0 := by
      simpa [Complex.sub_im, hArg n] using h1
    simpa [g] using Complex.im_pow_eq_zero_of_im_eq_zero h2 24
...
lemma re_ResToImagAxis_Delta_eq_real_prod (t : ℝ) (ht : 0 < t) :
    (Δ.resToImagAxis t).re = Real.exp (-2 * π * t) *
  ...
  have hMap' : Complex.ofReal (∏' n : ℕ, fR n) = ∏' n : ℕ, ((fR n : ℝ) : ℂ) := by
    simpa using Function.LeftInverse.map_tprod (f := fR) (g := Complex.ofRealHom.toMonoidHom)
      (hg := by simpa using Complex.continuous_ofReal)
      (hg' := Complex.continuous_re) (hgg' := fun _ => by simp)
...
lemma tprod_pos_nat_im (z : ℍ) :
  ...
  have hsum_log : Summable
      (fun n : ℕ => Real.log ((1 - Real.exp (-(2 * π * ((n + 1) : ℝ) * z.im))) ^ 24)) := by
