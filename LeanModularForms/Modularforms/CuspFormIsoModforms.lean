...
/-- **Direct cusp-form constructor from a modular form plus per-cusp vanishing.**
Given `g : ModularForm Γ k` (where `Γ : Subgroup (GL (Fin 2) ℝ)`) and a hypothesis
that `g` vanishes at every cusp of `Γ`, package `g` as a `CuspForm Γ k` whose
underlying function is `⇑g`. -/
public def CuspForm.ofModularForm {Γ : Subgroup (GL (Fin 2) ℝ)} {k : ℤ}
    (g : ModularForm Γ k)
...
@[simp] lemma CuspForm.ofModularForm_coe {Γ : Subgroup (GL (Fin 2) ℝ)} {k : ℤ}
    (g : ModularForm Γ k)
    (h : ∀ {c : OnePoint ℝ}, IsCusp c Γ → c.IsZeroAt g.toFun k) :
    ⇑(CuspForm.ofModularForm g h) = ⇑g := rfl

@[simp] lemma CuspForm.ofModularForm_apply {Γ : Subgroup (GL (Fin 2) ℝ)} {k : ℤ}
    (g : ModularForm Γ k)
    (h : ∀ {c : OnePoint ℝ}, IsCusp c Γ → c.IsZeroAt g.toFun k) (z : ℍ) :
    CuspForm.ofModularForm g h z = g z := rfl
