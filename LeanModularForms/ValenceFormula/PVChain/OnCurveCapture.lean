...
theorem oncurve_vert_capture
  ...
  have h_im_val : z.im = H' - t * (H' - Real.sqrt 3 / 2) := by
    rw [hz_seg]; simp [fdBoundary_seg1_H, add_im, mul_im, I_re, I_im, ofReal_re, ofReal_im,
      div_ofNat]
  ...
  let p : ℍ := ⟨z, h_im_pos⟩
  have hp_fd : p ∈ 𝒟 := by
    refine ⟨?_, by show |z.re| ≤ 1/2; rw [h_re]; norm_num⟩
    show 1 ≤ Complex.normSq z
...
theorem oncurve_seg4_capture
  ...
  have hp_fd : p ∈ 𝒟 := by
    refine ⟨?_, by show |(fdBoundary_seg1_H H s).re| ≤ 1/2; rw [h_re]; norm_num⟩
    show 1 ≤ Complex.normSq (fdBoundary_seg1_H H s)
    rw [Complex.normSq_apply]
    nlinarith [mul_self_le_mul_self (by positivity : 0 ≤ Real.sqrt 3 / 2) h_im_gt.le,
      Real.sq_sqrt (show (0:ℝ) ≤ 3 by norm_num)]
  have hp_zero : f p = 0 := by
