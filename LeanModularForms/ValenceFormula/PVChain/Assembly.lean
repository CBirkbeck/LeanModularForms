...
private lemma norm_shift_neg_inv_eq {z s : ℂ} (hz_re : z.re = 1/2) (hs_unit : ‖s‖ = 1) :
  ...
  have h2_re : (z - s).re = z.re - s.re := Complex.sub_re z s
  have h2_im : (z - s).im = z.im - s.im := Complex.sub_im z s
  ...
  have h_sq2 : ‖z - s‖ ^ 2 = (z.re - s.re) ^ 2 + (z.im - s.im) ^ 2 := by
    rw [Complex.sq_norm, Complex.normSq_apply, h2_re, h2_im]; ring
  have h_eq : ‖(z - 1) - (-(1 : ℂ) / s)‖ ^ 2 = ‖z - s‖ ^ 2 := by
    rw [h_sq1, h_sq2, hz_re]; ring
  have ha := norm_nonneg ((z - 1) - (-(1 : ℂ) / s))
  have hb := norm_nonneg (z - s)
...
private lemma truncation_iff_shift_union
  ...
  · rintro ⟨s, hs, h_le⟩
    ...
    · refine ⟨-(1 : ℂ) / s, Finset.mem_union_left _ (sArcOfS_closed S s h_arc), ?_⟩
      have h_eq := norm_shift_neg_inv_eq hz_re (sArcOfS_unit S s h_arc)
      linarith
...
private lemma logDeriv_modFormComp_periodic :
  ...
  have h1 : (modularFormCompOfComplex f) (z + 1) = (modularFormCompOfComplex f) z := h_per z
...
private lemma deriv_fdBoundary_H_on_seg4 (H : ℝ) (t : ℝ) (ht : t ∈ Set.Ioo (3:ℝ) 4) :
  ...
  have h_eq : fdBoundary_H H =ᶠ[𝓝 t] fdBoundary_seg4_H H := by
    have h1 : Ioi (3 : ℝ) ∈ 𝓝 t := Ioi_mem_nhds ht.1
    have h2 : Iio (4 : ℝ) ∈ 𝓝 t := Iio_mem_nhds ht.2
...
private theorem pvIntegral_vertical_cancel
  ...
  have h_pw : ∀ u ∈ Set.Ioo (0:ℝ) 1,
      pvIntegrand f (fdBoundary_H H) (sVertOfS S) ε (4 - u) =
      -(pvIntegrand f (fdBoundary_H H) (sVertOfS S) ε u) := by
    intro u hu
    have hu_le1 : u ≤ 1 := le_of_lt hu.2
    have h4u_gt3 : (3:ℝ) < 4 - u := by linarith [hu.2]
    have h4u_le4 : 4 - u ≤ 4 := by linarith [hu.1]
    have h4u_lt4 : 4 - u < 4 := by linarith [hu.1]
    have h_seg1 : fdBoundary_H H u = fdBoundary_seg1_H H u :=
      fdBoundary_H_eq_seg1_H hu_le1
    have h_seg4 : fdBoundary_H H (4 - u) = fdBoundary_seg4_H H (4 - u) :=
      fdBoundary_H_eq_seg4_H h4u_gt3 h4u_le4
    have h_shift : fdBoundary_H H (4 - u) = fdBoundary_H H u - 1 := by
      rw [h_seg4, seg4_eq_seg1_minus_one_H H u ⟨le_of_lt hu.1, hu_le1⟩, h_seg1]
    have h_re_u : (fdBoundary_H H u).re = 1/2 := by
      rw [h_seg1]; simp [fdBoundary_seg1_H, add_re, ofReal_re, mul_re, I_re, I_im, ofReal_im]
    have h_trunc : (∃ s ∈ sVertOfS S, ‖fdBoundary_H H (4 - u) - s‖ ≤ ε) ↔
    ...
    by_cases h_trunc_u : ∃ s ∈ sVertOfS S, ‖fdBoundary_H H u - s‖ ≤ ε
    ...
    · rw [if_neg h_trunc_u, if_neg (mt h_trunc.mp h_trunc_u)]
      ...
      ring
  have h_neg : ∫ u in (0:ℝ)..1, pvIntegrand f (fdBoundary_H H) (sVertOfS S) ε (4 - u) =
      ∫ u in (0:ℝ)..1, -(pvIntegrand f (fdBoundary_H H) (sVertOfS S) ε u) := by
  ...
  rw [h_neg, intervalIntegral.integral_neg]
  ring
...
private theorem pvIntegral_vertical_cancel_union
  ...
  rw [integral_seg4_cov]
  have h_pw : ∀ u ∈ Set.Ioo (0:ℝ) 1,
      pvIntegrand f (fdBoundary_H H) (sArcOfS S ∪ sVertOfS S) ε (4 - u) =
      -(pvIntegrand f (fdBoundary_H H) (sArcOfS S ∪ sVertOfS S) ε u) := by
    intro u hu
    have hu_le1 : u ≤ 1 := le_of_lt hu.2
    have h4u_gt3 : (3:ℝ) < 4 - u := by linarith [hu.2]
    have h4u_le4 : 4 - u ≤ 4 := by linarith [hu.1]
    ...
    simp only [pvIntegrand, cauchyPrincipalValueIntegrandOn]
    by_cases h_trunc_u :
        ∃ s ∈ sArcOfS S ∪ sVertOfS S, ‖fdBoundary_H H u - s‖ ≤ ε
    · rw [if_pos h_trunc_u, if_pos (h_trunc.mpr h_trunc_u)]; simp
    · rw [if_neg h_trunc_u, if_neg (mt h_trunc.mp h_trunc_u)]
      have h_logDeriv : logDeriv (modularFormCompOfComplex f) (fdBoundary_H H (4 - u)) =
          logDeriv (modularFormCompOfComplex f) (fdBoundary_H H u) := by
...
private lemma pvIntegrand_intervalIntegrable
  ...
  have h_cont : ContinuousOn (fun t => logDeriv g (γ t)) K' := by
    ...
    · have h_im : 0 < (γ t).im := fdBoundary_H_im_pos H hH t ht.1
      have h_diffOn : DifferentiableOn ℂ g {z | 0 < z.im} :=
        UpperHalfPlane.mdifferentiable_iff.mp f.holo'
      have h_analytic : AnalyticAt ℂ g (γ t) :=
        h_diffOn.analyticAt (UpperHalfPlane.isOpen_upperHalfPlaneSet.mem_nhds h_im)
  ...
  have h_compl_zero : EqOn F 0 (uIoc a b \ K) := by
    ...
    exact if_pos h_near
  have h_int_K' : IntegrableOn (fun t => logDeriv g (γ t) * deriv γ t) K' := by
    ...
    · obtain ⟨t_max, ht_max_mem, ht_max⟩ := hK'_compact.exists_isMaxOn hK'_ne
      ...
      rw [norm_mul]
      apply mul_le_mul (ht_max ht) ?_ (norm_nonneg _) (norm_nonneg _)
      have ht_Icc := ht.1
      by_cases h1 : t < 1
...
private theorem tendsto_pvIntegral_split
  ...
  have mem5 : (5:ℝ) ∈ Icc (0:ℝ) 5 := ⟨by norm_num, le_refl _⟩
  ...
  have h3 := (intervalIntegral.integral_add_adjacent_intervals
    (hi mem0 mem4) (hi mem4 mem5)).symm
  rw [h3, h2, h1]
...
private lemma modFormComp_ne_zero_at_height
  ...
  intro h_zero
  have h_fp : f ⟨z, hz_pos⟩ = 0 := h_bridge ▸ h_zero
  have h_eq := SlashInvariantFormClass.eq_cuspFunction (1 : ℕ) f ⟨z, hz_pos⟩
  ...
  exact absurd (h_eq.trans h_fp) (hcusp _ h_qmem h_qne)
...
theorem cpv_modular_side_tendsto
  ...
  have h_vert_below : ∀ s ∈ sVertOfS S, s.im < H := by
    intro s hs
    have h_im := sVertOfS_im_lt_height_bound S s hs hH₁_bound
    calc s.im < H₁ := h_im
  ...
  have h_arc_below : ∀ s ∈ sArcOfS S, s.im < H := by
    intro s hs
    have h_unit := sArcOfS_unit S s hs
    have : s.im ≤ 1 := by
        ...
        · by_cases h4' : t = 4
          ...
          · have h_lt4 : t < 4 := lt_of_le_of_ne h4 h4'
            ...
            have h_F_per : Function.Periodic
              ...
              have := SlashInvariantFormClass.periodic_comp_ofComplex 1 f
              simpa only [Nat.cast_one] using this
            have h_u_range : 4 - t ∈ Set.Ioo (0:ℝ) 1 :=
              ⟨by linarith, by linarith⟩
            ...
            have h_vert_mem := h_oncurve_vert (4 - t) h_u_range h_F_zero_shifted
            ...
            exact Finset.mem_coe.mpr (Finset.mem_union_right _
              (sVertOfS_pair_left S _ (Finset.mem_coe.mp h_vert_mem) h_re))
        · exfalso
          have h_im : (fdBoundary_H H t).im = H := by
            rw [fdBoundary_H_eq_seg5_H (by linarith : (4:ℝ) < t)]
            simp [fdBoundary_seg5_H, add_im, ofReal_im, mul_im, I_re, I_im]
          exact modFormComp_ne_zero_at_height f hH_pos hcusp h_im h_zero
  have h_split := tendsto_pvIntegral_split f S hH_sqrt3 h_capture
  ...
  have h_vert := pvIntegral_vertical_cancel_union f S hH_sqrt3 h_oncurve_vert
...
theorem cpv_residue_side_tendsto
  ...
  have hH₀_exists : ∃ H₀ : ℝ, Real.sqrt 3 / 2 < H₀ ∧ 1 ≤ H₀ ∧
      ∀ s ∈ S, (s : ℂ).im < H₀ := by
    ...
    · refine ⟨max 1 (S.sup' hne (fun s => (s : ℂ).im) + 1), ?_, ?_, ?_⟩
      ...
      · intro s hs
        have h1 : (s : ℂ).im ≤ S.sup' hne (fun p => (p : ℂ).im) :=
          Finset.le_sup' (fun p : ℍ => (p : ℂ).im) hs
        exact lt_of_le_of_lt h1
          (lt_of_lt_of_le (lt_add_one _) (le_max_right _ _))
  obtain ⟨H₀, hH₀_sqrt3, hH₀_ge1, hH₀_bound⟩ := hH₀_exists
  ...
  have hSimplePoles : ∀ s ∈ S0, HasSimplePoleAt F s := by
    ...
    · have h_im : 0 < s.im := fdBox_im_pos ((mem_allZerosInFdBox_iff f hf hM_half).mp h_box).1
  ...
  have h_capture : ∀ t ∈ Icc (0:ℝ) 5,
    ...
    left; rw [mem_allZerosInFdBox_iff]
    exact ⟨fdBoundary_H_mem_fdBox' hH_ge1 (by linarith : H < M) t ht, h_zero⟩
  ...
  have hγ_closed : γ_imm.toPiecewiseC1Curve.IsClosed :=
    fdBoundary_HCurve_closed H
  have hγ_in_U : ∀ t ∈ Icc γ_imm.a γ_imm.b, γ_imm.toFun t ∈ fdBox M :=
    fun t ht => fdBoundary_H_mem_fdBox' hH_ge1 (by linarith : H < M) t ht
  have hFp_diff : DifferentiableOn ℂ Fp (fdBox M \ ↑S0) := by
    intro z hz
    ...
    have hz_not_zero : modularFormCompOfComplex f z ≠ 0 := by
      intro h_zero
      exact hz_not_S0 (Finset.mem_union_left S_on
        ((mem_allZerosInFdBox_iff f hf hM_half).mpr ⟨hz.1, h_zero⟩))
  ...
  have hS0_in_U : ∀ s ∈ (↑S0 : Set ℂ), s ∈ fdBox M := by
    ...
    rcases hs with h | h
    · exact ((mem_allZerosInFdBox_iff f hf hM_half).mp h).1
    · exact fdBox_of_on_curve S hS hH_sqrt3 (by linarith : H < M) hH_ge1 hH_bound s h
  have hS0_discrete := finset_discrete S0
  ...
  have h_grt := generalizedResidueTheorem' (fdBox M) (fdBox_isOpen M) (fdBox_convex M)
    (↑S0) hS0_in_U hS0_discrete S0.finite_toSet.isClosed
    S0 (fun s hs => Finset.mem_coe.mpr hs)
    ...
    (logDerivPatched_hf_ext F S0 hSimplePoles) hPV
  obtain ⟨⟨L, hL_tendsto⟩, h_val⟩ := h_grt
  have h_integrand_eq : ∀ ε > 0, ∀ t,
      cauchyPrincipalValueIntegrandOn S0 Fp γ ε t =
      cauchyPrincipalValueIntegrandOn S0 F γ ε t := by
    intro ε hε t
    simp only [cauchyPrincipalValueIntegrandOn]
    split_ifs with h
    · rfl
    · push_neg at h
      have : γ t ∉ S0 := by
        intro habs; have := h (γ t) habs; simp [sub_self] at this; linarith
      have : Fp (γ t) = F (γ t) := logDerivPatched_eq_raw_off F S0 hSimplePoles this
      rw [this]
  rw [show γ_imm.toFun = γ from rfl,
      show γ_imm.a = (0:ℝ) from rfl,
  ...
  have hL_tendsto_F : Tendsto (fun ε =>
    ...
    exact h_integrand_eq ε (mem_Ioi.mp hε) t
  have h_off_curve_min_dist : ∀ s ∈ Sbox \ S_on,
      ∃ δ > 0, ∀ t ∈ Icc (0:ℝ) 5, δ ≤ ‖γ t - s‖ := by
    intro s hs
    have h_off : ∀ t ∈ Icc (0:ℝ) 5, γ t ≠ s := by
      intro t ht heq
      have h_box := (Finset.mem_sdiff.mp hs).1
      have h_narc := (Finset.mem_sdiff.mp hs).2
      exact h_narc (Finset.mem_coe.mp (heq ▸ h_capture_S_on t ht (by
        rw [heq]; exact ((mem_allZerosInFdBox_iff f hf hM_half).mp h_box).2)))
  ...
  have h_eventually_eq : ∀ᶠ ε in 𝓝[>] (0:ℝ),
    ...
    have h_finite_family : ∀ᶠ ε in 𝓝[>] (0:ℝ),
      ...
      have : ∀ s ∈ Sbox \ S_on, ∀ᶠ ε in 𝓝[>] (0:ℝ),
        ...
        intro s hs
        obtain ⟨δ, hδ_pos, hδ_bound⟩ := h_off_curve_min_dist s hs
        filter_upwards [Ioo_mem_nhdsGT hδ_pos] with ε hε
    ...
    have h_iff :
      ...
      · rintro ⟨s, hs, h_norm⟩
        ...
        · by_cases h_on2 : s ∈ S_on
          ...
          · exact absurd h_norm (not_le.mpr (hε s (Finset.mem_sdiff.mpr ⟨h_box, h_on2⟩) t ht))
        · exact ⟨s, h_on, h_norm⟩
      · rintro ⟨s, hs, h_norm⟩
  ...
  have hL_tendsto_S_on : Tendsto (fun ε =>
      ...
      (𝓝[>] 0) (𝓝 L) := by
    apply hL_tendsto_F.congr'
    filter_upwards [h_eventually_eq] with ε hε
    exact intervalIntegral.integral_congr (fun t ht => hε t ht)
  have h_sum_convert : L = 2 * ↑Real.pi * I *
    ...
    have h_rhs1 : ∑ s ∈ S0, generalizedWindingNumber' γ 0 5 s *
        residueSimplePole Fp s =
      ∑ s ∈ S0, generalizedWindingNumber' γ 0 5 s *
        residueSimplePole F s :=
      Finset.sum_congr rfl (fun s hs => by
        congr 1; exact residue_logDerivPatched_eq_raw F S0 hSimplePoles s hs)
    rw [h_rhs1]
    rw [show S0 = Sbox ∪ (S_on \ Sbox) from by
    ...
    have h_sarc_zero : ∑ s ∈ S_on \ Sbox,
      ...
      have h_nz : modularFormCompOfComplex f s ≠ 0 := by
        ...
        exact hs_nbox ((mem_allZerosInFdBox_iff f hf hM_half).mpr
          ⟨fdBox_of_on_curve S hS hH_sqrt3
            (by linarith : H < M) hH_ge1 hH_bound s hs_on, h_zero⟩)
      have h_im : 0 < s.im := by
        rcases Finset.mem_union.mp hs_on with h | h
        · exact sArcOfS_im_pos S s h
        · exact sVertOfS_im_pos S s h
      have h_res0 : residueSimplePole F s = 0 := by
        rw [hF_def]; exact residueSimplePole_logDeriv_eq_zero_at_nonzero f s h_im h_nz
      rw [h_res0, mul_zero]
    rw [h_sarc_zero, add_zero]
    have h_mfcc_eq : ∀ (p : ℍ), modularFormCompOfComplex f (↑p : ℂ) = f p := fun p => by
      simp only [modularFormCompOfComplex, Function.comp_apply]
      congr 1; rw [UpperHalfPlane.ofComplex_apply_of_im_pos p.im_pos]; ext; rfl
    set S_zeros := S.filter (fun p => f p = 0) with hS_zeros_def
    have h_S_zeros_in_Sbox : ∀ p ∈ S_zeros, (↑p : ℂ) ∈ Sbox := by
      intro p hp
      rw [Finset.mem_filter] at hp
      rw [hSbox_def, mem_allZerosInFdBox_iff]
      refine ⟨?_, ?_⟩
      · have h_fd := hS p hp.1
        have h_im_bound := hH_bound p hp.1
      ...
      · rw [h_mfcc_eq]; exact hp.2
    have h_image_sub : S_zeros.image (↑· : ℍ → ℂ) ⊆ Sbox :=
      Finset.image_subset_iff.mpr h_S_zeros_in_Sbox
    have h_complement_zero : ∀ s ∈ Sbox,
      ...
      have h_gWN := winding_zero_for_non_fd_point_H_geo f hf S hS_complete hH_ge1 s hs h_not_S
      rw [h_gWN, zero_mul]
    have h_coe_inj : ∀ a ∈ S_zeros, ∀ b ∈ S_zeros,
        (↑a : ℂ) = (↑b : ℂ) → a = b :=
      fun _ _ _ _ h => Subtype.val_injective h
    calc ∑ s ∈ Sbox, generalizedWindingNumber' γ 0 5 s * residueSimplePole F s
        = ∑ s ∈ S_zeros.image (↑· : ℍ → ℂ),
            generalizedWindingNumber' γ 0 5 s * residueSimplePole F s :=
          (Finset.sum_subset h_image_sub h_complement_zero).symm
      _ = ∑ p ∈ S_zeros,
            generalizedWindingNumber' γ 0 5 (↑p : ℂ) * residueSimplePole F (↑p : ℂ) :=
          Finset.sum_image h_coe_inj
      _ = ∑ p ∈ S_zeros,
