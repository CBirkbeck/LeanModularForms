...
ref_p₀ avoids the fdPolygon boundary, derives slitPlane membership for the
...
* `fdPolygon_avoids_line_to_ref` — line from p to ref_p₀ avoids fdPolygon
* `rc_sub_ref_p₀_mem_slitPlane` — radial circle minus ref_p₀ is in slitPlane
* `fdPolygon_sub_ref_p₀_mem_slitPlane` — fdPolygon minus ref_p₀ is in slitPlane
...
lemma continuous_w : Continuous (fun t => fdPolygon t - ref_p₀) :=
...
lemma w_tL_im_zero : (fdPolygon (tL ref_p₀) - ref_p₀).im = 0 :=
