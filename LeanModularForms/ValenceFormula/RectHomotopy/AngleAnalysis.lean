...
* `angleOnCircle`, `fdPolygonRadialCircle_angle` — angle of radial projection
* `tL` — time when vector crosses negative real axis on seg4
* `fdPolygonRadialCircle_angle_lifted` — lifted angle accounting for branch cut
* `ref_p₀` — reference point on imaginary axis
...
noncomputable def tL (p : ℂ) : ℝ :=
  3 + (p.im - Real.sqrt 3 / 2) / (H_height - Real.sqrt 3 / 2)
...
lemma tL_mem_Ioo (p : ℂ) (hp_norm : ‖p‖ > 1) (hp_re : |p.re| < 1/2)
  ...
  have hnum_lt : p.im - Real.sqrt 3 / 2 < H_height - Real.sqrt 3 / 2 := by
    linarith
...
lemma fdPolygonRadialCircle_angle_periodic (p : ℂ) :
    fdPolygonRadialCircle_angle p 5 =
      fdPolygonRadialCircle_angle p 0 := by
  simp only [fdPolygonRadialCircle_angle, angleOnCircle,
    fdPolygonRadialCircle, polygonToCircleRadial]
...
lemma circleParamCW_wrapCount :
    circleParamCW_angle 0 = 2 * Real.pi ∧
      circleParamCW_angle 5 = 0 := by
  constructor
  · simp only [circleParamCW_angle]; norm_num
  · simp only [circleParamCW_angle]; norm_num
