...
lemma polygonToCircleRadial_closed (p : ℂ) (_hp_norm : ‖p‖ > 1) (_hp_re : |p.re| < 1/2)
    (_hp_im : p.im < H_height) (s : ℝ) (_hs : s ∈ Icc 0 1) :
...
lemma polygonToCircleRadial_differentiable_off_partition (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  have h_coeff_diff : DifferentiableAt ℝ
      (fun t' => (1 - s) * ‖fdPolygon t' - p‖ + s) t :=
...
lemma polygonToCircleRadial_deriv_cont_on_piece (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  have h_dir_joint : ContDiffAt ℝ 1
      (fun q : ℝ × ℝ => fdPolygon q.1 - p) (t, s) :=
    h_fd_joint.sub contDiffAt_const
  have h_norm_joint : ContDiffAt ℝ 1
      (fun q : ℝ × ℝ => ‖fdPolygon q.1 - p‖) (t, s) :=
    h_dir_joint.norm ℝ hdir_ne
  have h_norm_C_joint : ContDiffAt ℝ 1
      (fun q : ℝ × ℝ => (‖fdPolygon q.1 - p‖ : ℂ)) (t, s) :=
  ...
  have h_inv_norm_C : ContDiffAt ℝ 1
      (fun q : ℝ × ℝ => ((‖fdPolygon q.1 - p‖ : ℂ))⁻¹) (t, s) :=
    h_norm_C_joint.inv h_norm_C_ne
  have h_unit_joint : ContDiffAt ℝ 1
      (fun q : ℝ × ℝ => (fdPolygon q.1 - p) *
  ...
  have h_fderiv_cont : ContinuousAt
      (fderiv ℝ (polygonToCircleRadial p)) (t, s) :=
...
lemma norm_normalize_sub_le {w₁ w₂ : ℂ} {δ : ℝ} (hδ : 0 < δ)
  ...
  have hterm2 : ‖w₂ * ((‖w₂‖ : ℂ) - (‖w₁‖ : ℂ)) /
    ...
    exact le_trans
      (div_le_div_of_nonneg_right h_norm_sub_bound (le_of_lt h1_pos))
...
lemma fdPolygon_hasDerivWithinAt_Ici (x : ℝ) :
  ...
  · have heq : fdPolygon =ᶠ[𝓝[Ici x] x] fdPolygon_seg1 := by
    ...
    exact (fdPolygon_seg1_differentiable.differentiableAt.hasDerivAt.hasDerivWithinAt
      ...
      (by simp only [fdPolygon, fdPolygon_seg1, show x ≤ 1 from le_of_lt h1, ite_true])
  · push_neg at h1
    ...
    exact (fdPolygon_seg2_differentiable.differentiableAt.hasDerivAt.hasDerivWithinAt
      ...
      (by simp only [fdPolygon, fdPolygon_seg2]
          split_ifs with h'₁ h'₂ h'₃ h'₄
  ...
  · push_neg at h1 h2
    have heq : fdPolygon =ᶠ[𝓝[Ici x] x] fdPolygon_seg3 := by
      ...
      · have : t = 2 := le_antisymm h'₂ (h2.trans ht_ge)
        subst this; simp [chordSegment, rho, i_point]; ring
      · rfl
      all_goals linarith
    exact (fdPolygon_seg3_differentiable.differentiableAt.hasDerivAt.hasDerivWithinAt
      ).congr_of_eventuallyEq heq
      (by simp only [fdPolygon, fdPolygon_seg3]
          split_ifs with h'₁ h'₂ h'₃ h'₄
          · linarith
          · have : x = 2 := le_antisymm h'₂ h2; subst this
            simp [chordSegment, rho, i_point]; ring
          · rfl
          all_goals linarith)
  · push_neg at h1 h2 h3
    have heq : fdPolygon =ᶠ[𝓝[Ici x] x] fdPolygon_seg4 := by
      ...
      · have : t = 3 := le_antisymm h'₃ (h3.trans ht_ge)
        subst this
        simp [chordSegment, rho, i_point, H_height]; ring
      · rfl
      all_goals linarith
    exact (fdPolygon_seg4_differentiable.differentiableAt.hasDerivAt.hasDerivWithinAt
      ).congr_of_eventuallyEq heq
      (by simp only [fdPolygon, fdPolygon_seg4]
          split_ifs with h'₁ h'₂ h'₃ h'₄
          · linarith
          · linarith
...
lemma polygonToCircleRadial_deriv_bounded (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  have h_dist_pos : ∀ t ∈ Icc (0:ℝ) 5, 0 < ‖fdPolygon t - p‖ := by
    ...
    exact norm_pos_iff.mpr
      (sub_ne_zero.mpr (fdPolygon_avoids_interior p hp_norm hp_re hp_im t ht))
  obtain ⟨t_min, ht_min_mem, ht_min_le⟩ :=
    isCompact_Icc.exists_isMinOn
      (Set.nonempty_Icc.mpr (by norm_num : (0:ℝ) ≤ 5))
  ...
  · apply norm_deriv_le_of_lip'
      (by positivity : 0 ≤ (3 + 4 / δ) * 3)
    ...
    calc ‖(fun t' => polygonToCircleRadial p (t', s)) t' -
      ...
      _ = ‖(1 - s) • (fdPolygon t' - fdPolygon t) +
          s • ((fdPolygon t' - p) / (‖fdPolygon t' - p‖ : ℂ) -
               (fdPolygon t - p) /
                (‖fdPolygon t - p‖ : ℂ))‖ := by rw [hg_diff t']
      _ ≤ ‖(1 - s) • (fdPolygon t' - fdPolygon t)‖ +
          ‖s • ((fdPolygon t' - p) / (‖fdPolygon t' - p‖ : ℂ) -
               (fdPolygon t - p) /
                (‖fdPolygon t - p‖ : ℂ))‖ := norm_add_le _ _
      _ = |1 - s| * ‖fdPolygon t' - fdPolygon t‖ +
          |s| * ‖(fdPolygon t' - p) / (‖fdPolygon t' - p‖ : ℂ) -
               (fdPolygon t - p) /
                (‖fdPolygon t - p‖ : ℂ)‖ := by
...
lemma fdPolygon_piecewise_homotopic_to_radialCircle (p : ℂ)
    ...
    (hp_im : p.im < H_height) :
    PiecewiseCurvesHomotopicAvoiding fdPolygon
      (fdPolygonRadialCircle p) 0 5 p
      ({1, 2, 3, 4} : Finset ℝ) := by
...
lemma winding_fdPolygon_eq_radialCircle (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  exact windingNumber_eq_of_piecewise_homotopic fdPolygon
    (fdPolygonRadialCircle p) 0 5 p
    ({1, 2, 3, 4} : Finset ℝ) hab
    (fdPolygon_piecewise_homotopic_to_radialCircle p hp_norm hp_re
      hp_im)
