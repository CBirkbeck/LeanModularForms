...
* `RectHomotopyProof.rho`, `rho'`, `i_point` — elliptic points
...
* `RectHomotopyProof.H_height` — height parameter (= `heightCutoff`)
-/
...
def arc1 (t : ℝ) : ℂ :=
  Complex.exp (I * (θ_rho' + t * (θ_i - θ_rho')))

def arc2 (t : ℝ) : ℂ :=
  Complex.exp (I * (θ_i + t * (θ_rho - θ_i)))

lemma arc1_on_unit_circle (t : ℝ) : ‖arc1 t‖ = 1 := by
  simp only [arc1]
  have h : I * (↑θ_rho' + ↑t * (↑θ_i - ↑θ_rho')) =
      I * ↑(θ_rho' + t * (θ_i - θ_rho')) := by
...
lemma arc2_on_unit_circle (t : ℝ) : ‖arc2 t‖ = 1 := by
  simp only [arc2]
  have h : I * (↑θ_i + ↑t * (↑θ_rho - ↑θ_i)) =
      I * ↑(θ_i + t * (θ_rho - θ_i)) := by
    simp only [ofReal_add, ofReal_mul, ofReal_sub]
  rw [h, mul_comm]
  exact Complex.norm_exp_ofReal_mul_I _
...
lemma fdBoundary_at_five :
    fdBoundary 5 = (1/2 : ℂ) + H_height * I := by
  simp only [fdBoundary,
