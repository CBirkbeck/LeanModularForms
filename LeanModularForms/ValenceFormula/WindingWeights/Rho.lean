...
theorem pv_integral_at_rho_tendsto (H : ℝ) (hH : Real.sqrt 3 / 2 < H) :
  ...
  rw [show dist (∫ t in (0:ℝ)..5,
      if ‖fdBoundary_H H t - (ellipticPointRho : ℂ)‖ > ε
      then (fdBoundary_H H t - (ellipticPointRho : ℂ))⁻¹ *
           deriv (fun s => fdBoundary_H H s - (ellipticPointRho : ℂ)) t
      else 0)
    (-(I * ↑Real.pi / 3)) =
    ‖(∫ t in (0:ℝ)..5,
      if ‖fdBoundary_H H t - (ellipticPointRho : ℂ)‖ > ε
  ...
  set zL := fdBoundary_H H (3 - δ_L) - (ellipticPointRho : ℂ)
  set zR := fdBoundary_H H (3 + δ_R) - (ellipticPointRho : ℂ)
