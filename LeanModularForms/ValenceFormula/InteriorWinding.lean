...
theorem fdBoundary_H_avoids_interior (p : ℂ) (hp_norm : ‖p‖ > 1) (hp_re : |p.re| < 1 / 2)
  ...
  · push_neg at h1
    by_cases h3 : t ≤ 3
    · have hnorm : ‖fdBoundary_H H t‖ = 1 := by
  ...
  have hhom : PiecewiseCurvesHomotopicAvoiding
    ...
    · intro t _ ht_not_P s _
      ...
      have : t ∉ fdBoundary_H_partition := by
        ...
        push_neg at ht_not_P ⊢; exact ⟨ht_not_P.2.1, ht_not_P.2.2.2.1, ht_not_P.2.2.2.2.1⟩
    ...
    · intro p₁ p₂ _ hfree hsub
      ...
      exact (hc_base.comp continuousOn_fst (fun r hr => hr.1)).congr (fun r hr => by
        have ht_not_part : r.1 ∉ fdBoundary_H_partition := by
          simp only [fdBoundaryFullPartition, fdBoundary_H_partition,
            Finset.mem_insert, Finset.mem_singleton] at hfree ⊢
          push_neg; have := hfree r.1 hr.1; push_neg at this
