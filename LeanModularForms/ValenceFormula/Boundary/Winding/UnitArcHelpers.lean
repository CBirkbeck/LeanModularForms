...
Helper lemmas for proving `generalizedWindingNumber' (fdBoundary_H H) 0 5 s = -1/2`
    ...
    ·
      ...
      · have ht1 : 1 < t := lt_of_le_of_ne (le_of_lt h1) (Ne.symm h1')
        have ht3 : t < 3 := lt_of_le_of_ne h3 (by
          ...
          have hre := congr_arg Complex.re hs_eq
          have h_angle :
              (↑Real.pi / 2 + (↑(3:ℝ) - 2) * (2 * ↑Real.pi / 3 - ↑Real.pi / 2) : ℂ) =
...
lemma unitArc_min_dist_from_non_arc (H : ℝ) (hH : 1 < H) (s : ℂ)
  ...
  ·
    ...
    have hre :
        (exp ((↑Real.pi / 2 + (↑(3:ℝ) - 2) * (2 * ↑Real.pi / 3 - ↑Real.pi / 2)) * I)).re =
        -1/2 := by
...
lemma unitArc_min_dist_from_seg1 (H : ℝ) (s : ℂ)
...
lemma unitArc_ftc_value (H : ℝ) (hH : 1 < H) (s : ℂ)
    ...
    IntervalIntegrable
      (fun t => deriv (fun u => fdBoundary_H H u - s) t / (fdBoundary_H H t - s))
      volume 0 (t₀ - δ) ∧
    IntervalIntegrable
      (fun t => deriv (fun u => fdBoundary_H H u - s) t / (fdBoundary_H H t - s))
      volume (t₀ + δ) 5 ∧
  ...
  have piece₄ := ftc_log_neg (by norm_num : (4:ℝ) ≤ 5)
    ...
    (by rw [show deriv h₅ = fun _ => (1 : ℂ) from funext fun t => (hd₅ t).deriv]
        exact continuousOn_const)
    hslit_seg5
  have h_ae_01 : ∀ᵐ t ∂volume,
      t ∈ Set.uIoc 0 1 → deriv h₀ t / h₀ t = deriv g t / g t := by
    have h_excl : ({1} : Set ℝ)ᶜ ∈ ae volume :=
      mem_ae_iff.mpr
        (by rw [compl_compl]; exact (Set.toFinite ({1} : Set ℝ)).measure_zero volume)
  ...
  have h_ae_1_td : ∀ᵐ t ∂volume, t ∈ Set.uIoc 1 (t₀ - δ) →
    ...
    have h_excl : ({1, t₀ - δ} : Set ℝ)ᶜ ∈ ae volume :=
      mem_ae_iff.mpr
        (by rw [compl_compl]; exact (Set.toFinite ({1, t₀ - δ} : Set ℝ)).measure_zero volume)
  ...
  have h_ae_arc_after : ∀ᵐ t ∂volume, t ∈ Set.uIoc (t₀ + δ) 3 →
    ...
    have h_excl : ({t₀ + δ, 3} : Set ℝ)ᶜ ∈ ae volume :=
      mem_ae_iff.mpr
        (by rw [compl_compl]; exact (Set.toFinite ({t₀ + δ, 3} : Set ℝ)).measure_zero volume)
    filter_upwards [h_excl] with t ht_ne ht_mem
    ...
    have ht3 : t < 3 := by
      ...
      · exact h
    rw [hg_arc t ht1 ht3, hderiv_arc t ⟨ht1, ht3⟩]
  have h_ae_34 : ∀ᵐ t ∂volume,
      t ∈ Set.uIoc 3 4 → deriv h₃ t / h₃ t = deriv g t / g t := by
    have h_excl : ({3, 4} : Set ℝ)ᶜ ∈ ae volume :=
      mem_ae_iff.mpr
        (by rw [compl_compl]; exact (Set.toFinite ({3, 4} : Set ℝ)).measure_zero volume)
    filter_upwards [h_excl] with t ht_ne ht_mem
  ...
  have h_ae_45 : ∀ᵐ t ∂volume,
      t ∈ Set.uIoc 4 5 → deriv h₅ t / h₅ t = deriv g t / g t := by
  ...
  have h_split_left : ∫ t in (0:ℝ)..(t₀ - δ), deriv g t / g t =
      (∫ t in (0:ℝ)..1, deriv g t / g t) +
      (∫ t in (1:ℝ)..(t₀ - δ), deriv g t / g t) := by
