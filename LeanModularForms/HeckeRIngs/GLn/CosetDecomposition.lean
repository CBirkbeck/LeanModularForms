...
private lemma coset_sum_eq {a : Fin n → ℕ} {hdiv : DivChain n a} {B₂ : UpperTriRep n a hdiv}
  ...
  · simp only [hij, ite_true]
    rw [Finset.sum_eq_single_of_mem i
        (Finset.mem_erase.mpr ⟨hij.ne, Finset.mem_univ i⟩)]
    · rw [h_sum_rest i hij.ne]; simp [hij]
    · intro k hk hki
...
private lemma coset_entry_zero_of_lt {a : Fin n → ℕ} {hpos : ∀ i, 0 < a i}
  ...
  have h_dvd : a i ∣ a j := divChain_dvd n hdiv hij.le
  set q := a j / a i
  have hq_pos : 0 < q := divChain_div_pos n hpos hdiv hij.le
  have h_aj_eq : (a j : ℤ) = (a i : ℤ) * (q : ℤ) := by
    have h : (q : ℤ) * (a i : ℤ) = (a j : ℤ) := mod_cast Nat.div_mul_cancel h_dvd
    linarith
  have h_ai_ne : (a i : ℤ) ≠ 0 := mod_cast (hpos i).ne'
  have h_cancel : σ.val i j * (q : ℤ) =
    ...
    linarith
  have h1 : ((B₁ ⟨(i, j), hij⟩ : ℕ) : ℤ) < (q : ℤ) := mod_cast (B₁ ⟨(i, j), hij⟩).isLt
  have h2 : ((B₂ ⟨(i, j), hij⟩ : ℕ) : ℤ) < (q : ℤ) := mod_cast (B₂ ⟨(i, j), hij⟩).isLt
  by_contra hσ_ne
