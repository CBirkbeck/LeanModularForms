...
private lemma slSuccEmbed_mul {k : ℕ} (A B : SpecialLinearGroup (Fin (k + 1)) ℤ) :
  ...
  set e : Fin (k + 2) ≃ Fin 1 ⊕ Fin (k + 1) :=
    (Fin.castOrderIso (show k + 1 + 1 = 1 + (k + 1) by omega)
      |>.toEquiv.trans finSumFinEquiv.symm)
  show (slSuccEmbed (A * B)).1 = (slSuccEmbed A).1 * (slSuccEmbed B).1
  rw [slSuccEmbed_val_eq, slSuccEmbed_val_eq, slSuccEmbed_val_eq, SpecialLinearGroup.coe_mul,
    show fromBlocks (1 : Matrix (Fin 1) (Fin 1) ℤ) 0 0 (A.1 * B.1) =
      fromBlocks 1 0 0 A.1 * fromBlocks 1 0 0 B.1 by rw [fromBlocks_multiply]; simp]
...
private lemma slSuccEmbed_one {k : ℕ} :
  ...
  simp only [SpecialLinearGroup.coe_one, fromBlocks_one]
  ext i j
  simp only [submatrix_apply, one_apply, Equiv.apply_eq_iff_eq]
...
private lemma mapGL_injective (n : ℕ) :
    Function.Injective (mapGL ℚ : SpecialLinearGroup (Fin n) ℤ →* GL (Fin n) ℚ) := by
  intro a b h
  refine Subtype.ext ?_
  ext i j
  have hij := congr_arg (fun g ↦ (g : Matrix (Fin n) (Fin n) ℚ) i j) (congr_arg Units.val h)
...
private lemma slSuccEmbed_H_mul {k : ℕ} (σ₁ σ₂ : (GL_pair (k + 1)).H) :
  ...
  have htoSL : toSL (σ₁ * σ₂) = toSL σ₁ * toSL σ₂ :=
    mapGL_injective (k + 1) <| by
      rw [map_mul, toSL_spec, toSL_spec, toSL_spec]; exact Subgroup.coe_mul _ _ _
...
private lemma slSuccEmbed_H_one {k : ℕ} :
  ...
  rw [mapGL_injective (k + 1) (by rw [toSL_spec]; simp : mapGL ℚ (toSL 1) = mapGL ℚ 1),
...
private lemma slSuccEmbed_H_injective {k : ℕ} :
  ...
  have hval : (slSuccEmbed_H σ₁ : GL (Fin (k + 2)) ℚ) =
      (slSuccEmbed_H σ₂ : GL (Fin (k + 2)) ℚ) := congr_arg (·.val) h
...
private lemma block_conj_identity {k : ℕ}
  ...
  apply Units.ext
  ext i j
  simp only [Units.val_mul, mul_apply, hD'_def, diagMat_val _ _ (cons_one_pos hd), diagonal_apply,
...
lemma slSuccEmbed_H_stab_diagMat {k : ℕ}
    (a : Fin (k + 1) → ℕ) (ha : ∀ i, 0 < a i) (σ : (GL_pair (k + 1)).H)
  ...
  exact ⟨slSuccEmbed ν, (block_conj_identity a ha (toSL σ) ν
    (by rw [toSL_spec σ]; exact hν.symm)).symm⟩
...
private lemma diagConjWitnessMat_entry_cast {n : ℕ} (c : Fin n → ℕ)
    ...
    (diagConjWitnessMat c N i j : ℚ) * (c i : ℤ) = (N.1 i j : ℚ) * (c j : ℤ) :=
  mod_cast congr_arg (fun z : ℤ ↦ (z : ℚ)) (Int.ediv_mul_cancel (h_dvd i j))
...
private lemma diagConjWitnessMat_det_one {n : ℕ} (c : Fin n → ℕ) (hc : ∀ i, 0 < c i)
  ...
  have hD_det_ne : (D.val).det ≠ 0 := by
    ...
    exact Finset.prod_ne_zero_iff.mpr fun i _ ↦ mod_cast (hc i).ne'
  ...
  have hN1 : ((mapGL ℚ N).val).det = 1 := by
    rw [show ((mapGL ℚ N).val).det = (N.val.map (Int.cast : ℤ → ℚ)).det by
      rw [mapGL_coe_matrix]; simp [algebraMap_int_eq], ← Int.cast_det, N.2]
    simp
  rw [hN1, one_mul] at hdet
  have h1 : (((diagConjWitnessMat c N).det : ℤ) : ℚ) = (1 : ℚ) := by
    rw [Int.cast_det _]
    exact mul_left_cancel₀ hD_det_ne (by rw [mul_one]; linarith)
  exact_mod_cast h1
...
lemma diagMat_cons_one_conj_mapGL_mem_H_of_entry_dvd {k : ℕ}
    (a : Fin (k + 1) → ℕ) (ha : ∀ i, 0 < a i) (N : SpecialLinearGroup (Fin (k + 2)) ℤ)
    (h_dvd : ∀ i j : Fin (k + 2), ((Fin.cons 1 a : Fin (k + 2) → ℕ) i : ℤ) ∣
        N.1 i j * ((Fin.cons 1 a : Fin (k + 2) → ℕ) j : ℤ)) :
    (diagMat (k + 2) (Fin.cons 1 a))⁻¹ * (mapGL ℚ N : GL (Fin (k + 2)) ℚ) *
      diagMat (k + 2) (Fin.cons 1 a) ∈ (GL_pair (k + 2)).H := by
...
noncomputable def decompQuot_slSuccEmbed_diagMat {k : ℕ}
  ...
  rw [QuotientGroup.leftRel_apply, Subgroup.mem_subgroupOf,
    Subgroup.mem_pointwise_smul_iff_inv_smul_mem, ConjAct.smul_def] at h_rel
  rw [QuotientGroup.leftRel_apply,
    show (slSuccEmbed_H σ₁)⁻¹ * slSuccEmbed_H σ₂ = slSuccEmbed_H (σ₁⁻¹ * σ₂) by
  ...
  rw [diagMat_delta_val (k + 1) a ha] at h_rel
  rw [diagMat_delta_val (k + 2) (Fin.cons 1 a) (cons_one_pos ha)]
  exact slSuccEmbed_H_stab_diagMat a ha (σ₁⁻¹ * σ₂) h_rel

lemma slSuccEmbed_val_zero_zero {k : ℕ} (τ : SpecialLinearGroup (Fin (k + 1)) ℤ) :
...
private lemma slSuccEmbed_conj_flip {k : ℕ} (a : Fin (k + 1) → ℕ)
    (σ : (GL_pair (k + 1)).H) (ν : SpecialLinearGroup (Fin (k + 2)) ℤ)
...
private lemma slSuccEmbed_conj_entry {k : ℕ}
  ...
  simpa only [Units.val_mul, Matrix.mul_apply, slSuccEmbed_H_val,
    ...
    SpecialLinearGroup.map_apply_coe, RingHom.mapMatrix_apply, Matrix.map_apply] using
...
private lemma slSuccEmbed_conj_ν_succ_zero {k : ℕ}
  ...
  have hai : (0 : ℚ) < (a i : ℚ) := mod_cast ha i
...
private lemma ν_bottomBlock_det {k : ℕ}
  ...
  rw [show (∑ x : Fin (k + 1), (-1 : ℤ) ^ (x.succ : Fin (k + 2)).val * ν.val x.succ 0 *
      (ν.val.submatrix x.succ.succAbove (Fin.succAbove 0)).det) = 0 by
    refine Finset.sum_eq_zero fun x _ ↦ ?_
    rw [slSuccEmbed_conj_ν_succ_zero a ha σ ν hflip]; ring, add_zero, ν.prop] at h_expand
  rw [show (fun i j : Fin (k + 1) ↦ ν.val i.succ j.succ :
      Matrix (Fin (k + 1)) (Fin (k + 1)) ℤ) =
      ν.val.submatrix (Fin.succAbove 0) (Fin.succAbove 0) by
    ext i j; simp only [Matrix.submatrix_apply, Fin.succAbove_zero]]
  exact h_expand.symm
...
private lemma ν_bottomBlock_mapGL_eq {k : ℕ}
  ...
  have h_flip_k1 : (σ : GL (Fin (k + 1)) ℚ) * diagMat (k + 1) a =
    ...
    rw [show (σ : GL (Fin (k + 1)) ℚ).val i j = ((toSL σ).val i j : ℚ) by
...
lemma decompQuot_slSuccEmbed_diagMat_injective {k : ℕ}
  ...
  rw [show (slSuccEmbed_H σ₁)⁻¹ * slSuccEmbed_H σ₂ = slSuccEmbed_H (σ₁⁻¹ * σ₂) by
      rw [← slSuccEmbed_H_inv, ← slSuccEmbed_H_mul],
  ...
  rw [diagMat_delta_val (k + 2) (Fin.cons 1 a) (cons_one_pos ha)] at h
...
private noncomputable def blockEmbedGL {k : ℕ} (X : GL (Fin (k + 1)) ℚ) :
  ...
  have h_det_prod : X.val.det * (X⁻¹ : GL (Fin (k + 1)) ℚ).val.det = 1 := by
    rw [← Matrix.det_mul, show X.val * (X⁻¹ : GL _ ℚ).val = 1 by exact_mod_cast X.mul_inv]
    exact Matrix.det_one
  intro h
  rw [h, zero_mul] at h_det_prod
  exact one_ne_zero h_det_prod.symm
...
private lemma blockEmbedGL_mul {k : ℕ} (X Y : GL (Fin (k + 1)) ℚ) :
  ...
  set e : Fin (k + 2) ≃ Fin 1 ⊕ Fin (k + 1) :=
    (Fin.castOrderIso (show k + 1 + 1 = 1 + (k + 1) by omega)).toEquiv.trans finSumFinEquiv.symm
  rw [Units.val_mul, blockEmbedGL_val_eq, blockEmbedGL_val_eq, blockEmbedGL_val_eq,
    Units.val_mul,
    show Matrix.fromBlocks (1 : Matrix (Fin 1) (Fin 1) ℚ) 0 0 (X.val * Y.val) =
        Matrix.fromBlocks (1 : Matrix (Fin 1) (Fin 1) ℚ) 0 0 X.val *
        Matrix.fromBlocks (1 : Matrix (Fin 1) (Fin 1) ℚ) 0 0 Y.val by
...
private lemma blockEmbedGL_one {k : ℕ} :
  ...
  rw [blockEmbedGL_val_eq,
    show ((1 : GL (Fin (k + 1)) ℚ).val) = (1 : Matrix (Fin (k + 1)) (Fin (k + 1)) ℚ) from rfl,
...
private noncomputable def slPredEmbed {k : ℕ}
    (M : SpecialLinearGroup (Fin (k + 2)) ℤ) (h_diag : M.1 0 0 = 1)
  ...
  ⟨M.1.submatrix Fin.succ Fin.succ, by
    ...
    rw [show (∑ i : Fin (k + 1), (-1 : ℤ) ^ ((i.succ : Fin (k + 2)) : ℕ) * M.1 i.succ 0 *
        (M.1.submatrix (i.succ).succAbove Fin.succ).det) = 0 from
      Finset.sum_eq_zero fun i _ ↦ by rw [h_col i]; ring, add_zero, M.2] at h_laplace
...
private lemma slSuccEmbed_H_slPredEmbed_H_eq {k : ℕ} (σ : (GL_pair (k + 2)).H)
    (h_diag : (toSL σ).1 0 0 = 1) (h_col : ∀ i : Fin (k + 1), (toSL σ).1 i.succ 0 = 0)
  ...
  rw [show toSL (slPredEmbed_H σ h_diag h_col) = slPredEmbed (toSL σ) h_diag h_col from
    mapGL_injective (k + 1) (by
      rw [toSL_spec (slPredEmbed_H σ h_diag h_col), slPredEmbed_H_val]),
    slSuccEmbed_slPredEmbed_eq _ h_diag h_col h_row]
  exact toSL_spec σ
...
private lemma slPredEmbed_H_stab_diagMat {k : ℕ}
    ...
    (diagMat (k + 1) a)⁻¹ * (slPredEmbed_H σ h_diag h_col : GL (Fin (k + 1)) ℚ) *
        diagMat (k + 1) a ∈ (GL_pair (k + 1)).H := by
  rw [show (σ : GL (Fin (k + 2)) ℚ) =
      (slSuccEmbed_H (slPredEmbed_H σ h_diag h_col) : GL (Fin (k + 2)) ℚ) by
    rw [slSuccEmbed_H_slPredEmbed_H_eq σ h_diag h_col h_row]] at hσ
  exact slSuccEmbed_H_stab_diagMat_converse a ha (slPredEmbed_H σ h_diag h_col) hσ

private lemma blockEmbedGL_unfold_succ {k : ℕ} (X : GL (Fin (k + 1)) ℚ) (i j : Fin (k + 1)) :
    (blockEmbedGL X).val i.succ j.succ = X.val i j := by
  rw [blockEmbedGL_val_eq]
  simp [Matrix.submatrix_apply, Matrix.fromBlocks, Matrix.of_apply,
    Fin.castOrderIso, finSumFinEquiv, Fin.addCases, Fin.subNat]
...
private lemma blockEmbedGL_injective {k : ℕ} :
  ...
  have h_val : (blockEmbedGL X).val i.succ j.succ = (blockEmbedGL Y).val i.succ j.succ :=
    congr_arg (·.val i.succ j.succ) h
  rwa [blockEmbedGL_unfold_succ, blockEmbedGL_unfold_succ] at h_val

private lemma blockEmbedGL_mem_H_imp {k : ℕ} (h : GL (Fin (k + 1)) ℚ)
    (hh : blockEmbedGL h ∈ (GL_pair (k + 2)).H) :
    h ∈ (GL_pair (k + 1)).H := by
  obtain ⟨ν, hν⟩ := hh
  have hν_val : ∀ p q : Fin (k + 2), ((ν.1 p q : ℤ) : ℚ) = (blockEmbedGL h).val p q := fun p q ↦ by
    simpa [mapGL_coe_matrix, algebraMap_int_eq, Matrix.map_apply] using
      congr_arg (·.val p q) hν
...
lemma slSuccEmbed_H_fiber_transfer_converse {k : ℕ}
  ...
  apply blockEmbedGL_mem_H_imp
  rwa [blockEmbedGL_mul, blockEmbedGL_mul, blockEmbedGL_mul, blockEmbedGL_mul,
...
lemma fiber_diagMat_iff_mem_H {n : ℕ} [NeZero n]
  ...
  · have hmem : (σ : GL (Fin n) ℚ) * diagMat n a *
    ...
    rw [show (diagMat n c)⁻¹ * (σ : GL (Fin n) ℚ) * diagMat n a *
      ...
      show (diagMat n c)⁻¹ * ((σ : GL (Fin n) ℚ) * diagMat n a *
          ((τ : GL (Fin n) ℚ) * diagMat n b)) = h by rw [← hprod]; group]
    exact hh
...
noncomputable def decompQuot_rep_to_diagMat {n : ℕ} [NeZero n]
  ...
  exact (decompQuot_val_equiv _ ⟨_, hmem⟩ hLR).trans
    ((Subtype.ext (diagMat_delta_val n a ha) :
        diagMat_delta n a = ⟨diagMat n a, diagMat_mem_posDetInt n a ha⟩) ▸
      decompQuot_double_H_equiv (GL_pair n)
