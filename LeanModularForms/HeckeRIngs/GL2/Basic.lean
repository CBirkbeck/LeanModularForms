...
private lemma mulMap_right_scalar_eq (b : Fin 2 → ℕ)
    ...
    (p : decompQuot (GL_pair 2) (T_diag b) ×
         decompQuot (GL_pair 2) (T_diag (fun _ : Fin 2 => c))) :
    mulMap (GL_pair 2) (T_diag b) (T_diag (fun _ : Fin 2 => c)) p =
      T_diag (b * (fun _ => c)) := by
  ...
  exact HeckeRing.HeckeCoset_ext (GL_pair 2) _ _ (doubleCoset_eq_of_mem' _ _ h_mem)

private lemma scalar_coset_rep_normalizes (c : ℕ) (hc : 0 < c) :
    let D_c := T_diag (fun _ : Fin 2 => c)
    let H' := (GL_pair 2).H
    let δ_c := (D_c.doubleCoset_eq.choose : GL (Fin 2) ℚ)
  ...
  have hδc_simp : δ_c = (h₁c * h₂c) * diagMat 2 (fun _ => c) := by
    have : δ_c = ↑(T_diag (fun _ : Fin 2 => c)).doubleCoset_eq.choose := rfl
...
private lemma heckeMultiplicity_right_scalar_eq_one (b : Fin 2 → ℕ)
    ...
    HeckeRing.heckeMultiplicity (GL_pair 2) D_b D_c D_bc = 1 := by
  have h_card : Fintype.card (decompQuot (GL_pair 2) (T_diag (fun _ : Fin 2 => c))) = 1 := by
    have := HeckeCoset_deg_scalar 2 c hc; simp only [HeckeRing.HeckeCoset_deg] at this; exact_mod_cast this
  haveI : Subsingleton (decompQuot (GL_pair 2) (T_diag (fun _ : Fin 2 => c))) :=
    Fintype.card_le_one_iff_subsingleton.mp (le_of_eq h_card)
  have h_le : HeckeRing.heckeMultiplicity (GL_pair 2) (T_diag b) (T_diag (fun _ : Fin 2 => c))
      (T_diag (b * (fun _ => c))) ≤ 1 := by
    classical
    ...
    have hi : i₁ = i₂ := by
      by_contra hne
      apply HeckeRing.decompQuot_coset_diff (GL_pair 2) (T_diag b) i₁ i₂ hne
      let δ_c := ((T_diag (fun _ : Fin 2 => c)).doubleCoset_eq.choose : GL (Fin 2) ℚ)
      ...
      have h12' : ({(i₁.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
            (((GL_pair 2).H : Set _) * {δ_c}) =
          ({(i₂.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
            (((GL_pair 2).H : Set _) * {δ_c}) := by
        have lhs_eq : ({(i₁.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
            {(j₁.out : GL (Fin 2) ℚ) * δ_c} * ((GL_pair 2).H : Set _) =
            ({(i₁.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
              (((GL_pair 2).H : Set _) * {δ_c}) := by rw [mul_assoc, h_coset]
        have rhs_eq : ({(i₂.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
            {(j₁.out : GL (Fin 2) ℚ) * δ_c} * ((GL_pair 2).H : Set _) =
            ({(i₂.out : GL (Fin 2) ℚ) * (T_diag b).doubleCoset_eq.choose} : Set _) *
              (((GL_pair 2).H : Set _) * {δ_c}) := by rw [mul_assoc, h_coset]
    ...
    subst hi; rfl
  have h_pos : 0 < HeckeRing.heckeMultiplicity (GL_pair 2) (T_diag b) (T_diag (fun _ : Fin 2 => c))
      (T_diag (b * (fun _ => c))) :=
  ...
  exact HeckeRing.heckeMultiplicity_eq_one_of_le_one_and_pos (GL_pair 2) D_b D_c D_bc h_le h_pos
...
private lemma heckeMultiplicity_right_scalar_eq_zero (b : Fin 2 → ℕ) (hb_pos : ∀ i, 0 < b i)
    ...
    HeckeRing.heckeMultiplicity (GL_pair 2) (T_diag b) (T_diag (fun _ : Fin 2 => c)) A = 0 := by
