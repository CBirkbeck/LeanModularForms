...
private lemma T_pp_comm_T_sum_ppow (k : ℕ) :
    T_pp p * T_sum ⟨p ^ k, pow_pos hp.pos k⟩ =
...
private lemma T_pp_pow_comm_T_sum_ppow (i k : ℕ) :
    ...
    T_sum ⟨p ^ k, pow_pos hp.pos k⟩ * T_pp p ^ i := by
  induction i with
  | zero => simp
  | succ i ih =>
...
private lemma T_sum_p_comm_T_pp_pow_T_sum (i k : ℕ) :
    T_sum ⟨p, hp.pos⟩ * (T_pp p ^ i * T_sum ⟨p ^ k, pow_pos hp.pos k⟩) =
...
private lemma thm324_4_lhs2_shift (r s : ℕ) : (p : ℤ) • (T_pp p *
      ∑ i ∈ Finset.range (r + 1),
        (p : ℤ) ^ i • (T_pp p ^ i *
...
private lemma deg_ppow_term_lt' (i k : ℕ) (h2i : 2 * i < k) :
  ...
  rw [deg_T_ad (p ^ i) (p ^ (k - i)) (pow_pos hp.pos i) (pow_pos hp.pos (k - i))
    (Nat.pow_dvd_pow p (by omega))]
...
private lemma deg_ppow_term_eq' (i k : ℕ) (h2i : 2 * i = k) :
  ...
  rw [show k - i = i from by omega,
    deg_T_ad (p ^ i) (p ^ i) (pow_pos hp.pos i) (pow_pos hp.pos i) (dvd_refl _)]
...
private lemma deg_T_sum_one : deg (GL_pair 2) (T_sum 1) = 1 := by
  ...
  simp only [Nat.divisors_one, Finset.sum_singleton, Nat.div_self one_pos]
  rw [deg_T_ad 1 1 one_pos one_pos (dvd_refl 1)]
