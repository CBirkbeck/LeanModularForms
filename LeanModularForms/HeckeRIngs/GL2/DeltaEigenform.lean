...
lemma Delta_lvl1_ne_zero : Delta_lvl1 ≠ 0 := fun h ↦
  Δ_ne_zero UpperHalfPlane.I <| by
    have hI : Delta_lvl1 UpperHalfPlane.I = 0 := by rw [h]; rfl
    rwa [Delta_lvl1_apply] at hI
...
private noncomputable def cuspForm_Gamma_one_equiv_Gamma1_one (k : ℤ) :
  ...
  toFun f :=
    { toFun := f
      slash_action_eq' γ hγ :=
        f.slash_action_eq' γ (map_Gamma_one_eq_map_Gamma1_one ▸ hγ)
      holo' := f.holo'
      zero_at_cusps' {_} hc :=
        f.zero_at_cusps' (map_Gamma_one_eq_map_Gamma1_one ▸ hc) }
  invFun f :=
    { toFun := f
      slash_action_eq' γ hγ :=
        f.slash_action_eq' γ (map_Gamma_one_eq_map_Gamma1_one ▸ hγ)
      ...
      zero_at_cusps' {_} hc :=
        f.zero_at_cusps' (map_Gamma_one_eq_map_Gamma1_one ▸ hc) }
  left_inv _ := by ext _; rfl
  right_inv _ := by ext _; rfl
  map_add' _ _ := by ext _; rfl
...
theorem Delta_lvl1_isEigenform : IsEigenform (N := 1) (k := 12) Delta_lvl1 := by
  ...
  · haveI : NeZero n.val := ⟨n.pos.ne'⟩
    exact ((finrank_eq_one_iff_of_nonzero' Delta_lvl1 Delta_lvl1_ne_zero).mp
      finrank_cuspForm_lvl1_weight12 (heckeT_n_cusp 12 n.val Delta_lvl1)).choose_spec.symm
