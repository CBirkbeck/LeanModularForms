...
noncomputable def adjointOp (A : GoodHeckeFamilyScalarAdjoint (N := N) F) (n : GoodIndex N) :
    Module.End ℂ V :=
  A.adjointScalar n • F.op n

@[simp] lemma adjointOp_apply (A : GoodHeckeFamilyScalarAdjoint (N := N) F) (n : GoodIndex N)
    (f : V) : A.adjointOp n f = A.adjointScalar n • F.op n f := rfl

lemma smul_right (A : GoodHeckeFamilyScalarAdjoint (N := N) F) (c : ℂ) (f g : V) :
    A.pairing f (c • g) = c * A.pairing f g :=
  A.smul_right' c f g

lemma adjoint (A : GoodHeckeFamilyScalarAdjoint (N := N) F) (n : GoodIndex N) (f g : V) :
...
noncomputable def cuspFormCharSpacePetPairing (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) :
...
noncomputable def cuspGamma0NebentypusPetPairing (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) :
...
private lemma petN_heckeT_n_cusp_adjoint_scalar (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) (n : ℕ) [NeZero n]
  ...
  have hTg := heckeT_n_cusp_preserves_cuspFormCharSpace (N := N) k n hn χ hg
...
noncomputable def cuspFormCharSpaceScalarAdjoint (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) :
    GoodHeckeFamilyScalarAdjoint (N := N) (cuspFormCharSpaceFamily (N := N) k χ) where
...
`Γ₀(N), χ` cusp space. -/
noncomputable def cuspGamma0NebentypusScalarAdjoint (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) :
    GoodHeckeFamilyScalarAdjoint (N := N) (cuspGamma0NebentypusFamily (N := N) k χ) where
  ...
  adjoint' n f g := by
    ...
    exact petN_heckeT_n_cusp_adjoint_scalar (N := N) k χ (n : ℕ) n.property.2
      ...
      ((mem_cuspFormCharSpace_iff_mem_cuspGamma0NebentypusSubmodule
        (N := N) k χ _).mpr g.property)

@[simp] lemma cuspGamma0NebentypusScalarAdjoint_adjointScalar (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
    (n : GoodIndex N) : (cuspGamma0NebentypusScalarAdjoint (N := N) k χ).adjointScalar n =
