...
theorem ambientHeckeOfGoodIndex_commute_from_mulFormula (k : ℤ) (m n : GoodIndex N) :
  ...
  refine Finset.sum_bij (fun d _ ↦ ⟨d.1, by simp [Nat.gcd_comm]⟩)
    ...
    (fun _ _ _ _ h ↦ Subtype.ext (congrArg (fun z ↦ z.1) h))
...
theorem ambientHeckeOfGoodIndex_commute_apply_from_mulFormula (k : ℤ) (m n : GoodIndex N)
    ...
    ambientHeckeOfGoodIndex (N := N) k m (ambientHeckeOfGoodIndex (N := N) k n f) =
      ambientHeckeOfGoodIndex (N := N) k n (ambientHeckeOfGoodIndex (N := N) k m f) :=
  LinearMap.congr_fun (ambientHeckeOfGoodIndex_commute_from_mulFormula (N := N) k m n).eq f

/-- The restricted `Γ₁(N), χ` good-index operators commute by restricting the
ambient multiplication-table proof source. -/
theorem heckeT_coprimeRestrict_commute_from_mulFormula (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
    (m n : GoodIndex N) :
    Commute (heckeT_coprimeRestrict k χ m) (heckeT_coprimeRestrict k χ n) :=
  LinearMap.ext fun f ↦ Subtype.ext <|
    ambientHeckeOfGoodIndex_commute_apply_from_mulFormula (N := N) k m n
      (f : ModularForm ((Gamma1 N).map (mapGL ℝ)) k)
...
@[simp] lemma modFormCharSpaceFamily_coe (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ) (n : GoodIndex N)
  ...
  show ((heckeT_n_charRestrict k (n : ℕ) n.property.2 χ f :
      modFormCharSpace k χ) : ModularForm ((Gamma1 N).map (mapGL ℝ)) k) = _
