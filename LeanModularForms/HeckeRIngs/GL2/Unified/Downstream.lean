...
`Γ₀(p^r), χ` cusp space. -/
theorem mainLemma_gamma0Nebentypus_primePower {p : ℕ} [hp : Fact p.Prime] {r : ℕ} (hr : 0 < r)
    {k : ℤ} (χ : DirichletCharacter ℂ (p ^ r)) (f : CuspForm ((Gamma1 (p ^ r)).map (mapGL ℝ)) k)
  ...
  HeckeRing.GL2.AtkinLehner.mainLemma_charSpace_primePower hr χ f
...
theorem mainLemma_gamma0Nebentypus_of_prime_decomposition {N : ℕ} [NeZero N] {k : ℤ}
    (χ : DirichletCharacter ℂ N) (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k)
    (S : Finset ℕ) (hS : S ⊆ N.primeFactors)
    (f_p : ℕ → CuspForm ((Gamma1 N).map (mapGL ℝ)) k) (h_decomp : f = ∑ p ∈ S, f_p p)
    (h_char : ∀ p ∈ S, f_p p ∈ cuspGamma0NebentypusSubmodule (N := N) k χ.toUnitHom)
    (h_supp : ∀ p ∈ S, f_p p ∈ HeckeRing.GL2.AtkinLehner.qSupportedOnDvdSubmodule N k p) :
    f ∈ cuspFormsOld N k :=
  HeckeRing.GL2.AtkinLehner.mainLemma_charSpace_of_prime_decomposition χ f S hS f_p h_decomp
    (fun p hp ↦ (mem_cuspFormCharSpace_iff_mem_cuspGamma0NebentypusSubmodule
      (N := N) k χ.toUnitHom (f_p p)).mpr (h_char p hp)) h_supp

/-- The full-prime-factor Atkin–Lehner main lemma, restated for the
experimental `Γ₀(N), χ` cusp space. -/
theorem mainLemma_gamma0Nebentypus_of_primeFactors_decomposition {N : ℕ} [NeZero N] {k : ℤ}
    (χ : DirichletCharacter ℂ N) (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k)
    (f_p : ℕ → CuspForm ((Gamma1 N).map (mapGL ℝ)) k)
    (h_decomp : f = ∑ p ∈ N.primeFactors, f_p p)
    (h_char : ∀ p ∈ N.primeFactors,
      f_p p ∈ cuspGamma0NebentypusSubmodule (N := N) k χ.toUnitHom)
    (h_supp : ∀ p ∈ N.primeFactors,
      f_p p ∈ HeckeRing.GL2.AtkinLehner.qSupportedOnDvdSubmodule N k p) :
    f ∈ cuspFormsOld N k :=
  HeckeRing.GL2.AtkinLehner.mainLemma_charSpace_of_primeFactors_decomposition χ f f_p h_decomp
    (fun p hp ↦ (mem_cuspFormCharSpace_iff_mem_cuspGamma0NebentypusSubmodule
      (N := N) k χ.toUnitHom (f_p p)).mpr (h_char p hp)) h_supp

/-- Atkin–Lehner uniqueness, restated using the experimental `Γ₀(N), χ`
modular-form subspace. -/
theorem newform_unique_of_gamma0Nebentypus {N : ℕ} [NeZero N] {k : ℤ}
...
modular-form subspace. -/
theorem strongMultiplicityOne_of_gamma0Nebentypus {N : ℕ} [NeZero N] {k : ℤ}
    (f g : Newform N k) (χ : (ZMod N)ˣ →* ℂˣ)
    (hfχ : f.toCuspForm.toModularForm' ∈ gamma0NebentypusSubmodule (N := N) k χ)
    (hgχ : g.toCuspForm.toModularForm' ∈ gamma0NebentypusSubmodule (N := N) k χ) (S : Finset ℕ)
    (h : ∀ n : ℕ+, Nat.Coprime n.val N → n.val ∉ S → f.eigenvalue n = g.eigenvalue n) :
    f.toCuspForm = g.toCuspForm :=
  strongMultiplicityOne f g χ
    ((mem_modFormCharSpace_iff_mem_gamma0NebentypusSubmodule
      (N := N) k χ f.toCuspForm.toModularForm').mpr hfχ)
    ((mem_modFormCharSpace_iff_mem_gamma0NebentypusSubmodule
      (N := N) k χ g.toCuspForm.toModularForm').mpr hgχ) S h
