...
noncomputable def ambientCuspHeckeOfGoodIndex (k : ℤ) (n : GoodIndex N) :
  ...
  · intro f g
    ext z
    show (heckeT_n k (n : ℕ) (f + g).toModularForm').toFun z =
  ...
  · intro c f
    ext z
    show (heckeT_n k (n : ℕ) (c • f).toModularForm').toFun z =
...
theorem ambientCuspHeckeOfGoodIndex_commute_from_mulFormula
    (k : ℤ) (m n : GoodIndex N) :
    Commute (ambientCuspHeckeOfGoodIndex (N := N) k m)
      (ambientCuspHeckeOfGoodIndex (N := N) k n) := by
  apply LinearMap.ext
  intro f
  apply CuspForm.ext
  intro z
...
theorem ambientCuspHeckeOfGoodIndex_commute_apply_from_mulFormula
    (k : ℤ) (m n : GoodIndex N)
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    ambientCuspHeckeOfGoodIndex (N := N) k m
        (ambientCuspHeckeOfGoodIndex (N := N) k n f) =
      ambientCuspHeckeOfGoodIndex (N := N) k n
        (ambientCuspHeckeOfGoodIndex (N := N) k m f) := by
  exact LinearMap.congr_fun
    (ambientCuspHeckeOfGoodIndex_commute_from_mulFormula (N := N) k m n).eq f
...
theorem heckeT_n_cusp_charRestrict_commute_from_mulFormula
    (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
    (m n : GoodIndex N) :
    Commute (cuspCharHeckeOfGoodIndex (N := N) k χ m)
      (cuspCharHeckeOfGoodIndex (N := N) k χ n) := by
  apply LinearMap.ext
  intro f
  apply Subtype.ext
  exact ambientCuspHeckeOfGoodIndex_commute_apply_from_mulFormula
...
noncomputable def cuspFormCharSpaceFamily
  ...
  map_one' := by
    ...
    apply CuspForm.ext
    intro z
    show (heckeT_n k 1 (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k).toModularForm').toFun z =
...
@[simp] lemma cuspCharHeckeOfGoodIndex_coe
  ...
  @[simp] lemma cuspFormCharSpaceFamily_coe
      (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
      (n : GoodIndex N) (f : cuspFormCharSpace k χ) :
      (((cuspFormCharSpaceFamily (N := N) k χ).op n f : cuspFormCharSpace k χ) :
        ...
        ambientCuspHeckeOfGoodIndex (N := N) k n
          (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
  simpa [cuspFormCharSpaceFamily] using
    cuspCharHeckeOfGoodIndex_coe (N := N) k χ n f
...
noncomputable def cuspFormCharSpace_equiv_gamma0Nebentypus
  ...
  toFun f := ⟨f.1, by
    ...
    exact (isNebentypus_iff k (gamma0NebentypusChar (N := N) χ) (f.1 : UpperHalfPlane → ℂ)).2
      ((cuspFormCharSpace_iff_nebentypus (N := N) k χ f.1).1 f.2)⟩
  invFun f := ⟨f.1, by
    exact (cuspFormCharSpace_iff_nebentypus (N := N) k χ f.1).2
...
@[simp] lemma mem_cuspFormCharSpace_iff_mem_cuspGamma0NebentypusSubmodule
    (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
    (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) :
    f ∈ cuspFormCharSpace k χ ↔ f ∈ cuspGamma0NebentypusSubmodule (N := N) k χ := by
  constructor
  · intro hf
    exact (cuspFormCharSpace_equiv_gamma0Nebentypus (N := N) k χ ⟨f, hf⟩).property
  · intro hf
    exact ((cuspFormCharSpace_equiv_gamma0Nebentypus (N := N) k χ).symm ⟨f, hf⟩).property
...
@[simp] lemma mem_cuspGamma0NebentypusSubmodule_iff_toModularForm'_mem_gamma0NebentypusSubmodule
  ...
  rw [mem_cuspGamma0NebentypusSubmodule_iff, mem_gamma0NebentypusSubmodule_iff]
  change IsNebentypus k (gamma0NebentypusChar (N := N) χ) (f : UpperHalfPlane → ℂ) ↔
    IsNebentypus k (gamma0NebentypusChar (N := N) χ) (f : UpperHalfPlane → ℂ)
  rfl
...
@[simp] lemma cuspGamma0NebentypusFamily_coe
    (k : ℤ) (χ : (ZMod N)ˣ →* ℂˣ)
    (n : GoodIndex N) (f : cuspGamma0NebentypusSubmodule (N := N) k χ) :
    (((cuspGamma0NebentypusFamily (N := N) k χ).op n f :
        cuspGamma0NebentypusSubmodule (N := N) k χ) :
        CuspForm ((Gamma1 N).map (mapGL ℝ)) k) =
      ambientCuspHeckeOfGoodIndex (N := N) k n
        (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
  let e := cuspFormCharSpace_equiv_gamma0Nebentypus (N := N) k χ
  calc
    (((cuspGamma0NebentypusFamily (N := N) k χ).op n f :
        cuspGamma0NebentypusSubmodule (N := N) k χ) :
        CuspForm ((Gamma1 N).map (mapGL ℝ)) k)
      = ((e ((cuspFormCharSpaceFamily (N := N) k χ).op n (e.symm f)) :
            cuspGamma0NebentypusSubmodule (N := N) k χ) :
            CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
          rfl
    _ = (((cuspFormCharSpaceFamily (N := N) k χ).op n (e.symm f) :
            cuspFormCharSpace k χ) :
            CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
          rfl
    _ = ambientCuspHeckeOfGoodIndex (N := N) k n
          ((e.symm f : cuspFormCharSpace k χ) :
            CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
          rw [cuspFormCharSpaceFamily_coe]
    _ = ambientCuspHeckeOfGoodIndex (N := N) k n
          (f : CuspForm ((Gamma1 N).map (mapGL ℝ)) k) := by
          rfl
