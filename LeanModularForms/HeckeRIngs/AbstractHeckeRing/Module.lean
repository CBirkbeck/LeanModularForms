...
lemma smulOrbit_nonempty (t : HeckeCoset P) (m : HeckeLeftCoset P) :
    (smulOrbit P t m).Nonempty := by rw [smulOrbit]; simp
...
noncomputable instance instSMulHeckeModule : SMul (𝕋 P Z) (HeckeModule P Z) where
  smul t mm := Finsupp.sum t fun D1 b₁ => mm.sum fun m b₂ =>
    (∑ i ∈ smulOrbit P D1 m, Finsupp.single i (b₁ * b₂ : Z) : (HeckeLeftCoset P) →₀ Z)
...
lemma eq_of_smul_eq_smul_𝕋 (T1 T2 : (𝕋 P Z))
  ...
  Finsupp.ext fun D => by
    obtain ⟨m, hm⟩ := smulOrbit_nonempty P D (HeckeLeftCoset.one P)
