...
This project request had uuid: cfa38f91-e01e-4c30-97aa-3c6ab73d14b8
...
- lemma polygonToCircleRadial_differentiable_off_partition (p : ℂ) (hp_norm : ‖p‖ > 1)
    ...
    DifferentiableAt ℝ (fun t' => polygonToCircleRadial p (t', s)) t
...
Received a different number of the word 'sorry' (1) than the number of goals (4)-/
...
lemma fdPolygonRadialCircle_dist (p : ℂ) (hp_norm : ‖p‖ > 1) (hp_re : |p.re| < 1/2)
  ...
  convert polygonToCircleRadial_avoids p hp_norm hp_re hp_im t ht 1 ( by norm_num ) using 1;
  unfold fdPolygonRadialCircle polygonToCircleRadial; aesop;

-- Technical: norm of normalized vector = 1
...
lemma polygonToCircleRadial_continuous (p : ℂ) (hp_norm : ‖p‖ > 1) (hp_re : |p.re| < 1/2)
  ...
  apply_rules [ Continuous.add, Continuous.smul, continuous_const, continuous_id ];
  · apply_rules [ Continuous.mul, Continuous.sub, continuous_const, continuous_fst, continuous_snd, Continuous.norm ];
    exact fdPolygon_continuous.comp continuous_fst;
  · exact continuous_snd;
  · refine' Continuous.div _ _ _;
    · exact Continuous.sub ( fdPolygon_continuous.comp continuous_fst ) continuous_const;
    · exact Complex.continuous_ofReal.comp <| continuous_norm.comp <| Continuous.sub ( fdPolygon_continuous.comp continuous_fst ) continuous_const;
    · intro x hx; have := fdPolygon_avoids_interior p hp_norm hp_re hp_im x.1; simp_all +decide [ sub_eq_iff_eq_add ] ;
      unfold fdPolygon at hx; split_ifs at hx <;> norm_num at *;
      · norm_num [ Complex.ext_iff ] at hx;
...
-- Technical: continuity of radial projection (division by nonzero norm)
...
lemma polygonToCircleRadial_at_s_zero (p : ℂ) (hp_norm : ‖p‖ > 1) (hp_re : |p.re| < 1/2)
  ...
  -- By definition of $polygonToCircleRadial$, we have $polygonToCircleRadial p (t, 0) = p + (1 - 0) * ‖fdPolygon t - p‖ • (fdPolygon t - p) / ‖fdPolygon t - p‖$.
  simp [polygonToCircleRadial];
  by_cases h : fdPolygon t - p = 0 <;> simp_all +decide [ mul_div_cancel₀ ];
  exact Eq.symm ( sub_eq_zero.mp h )

-- Technical: radial homotopy at s=0 equals original curve
...
lemma polygonToCircleRadial_differentiable_off_partition (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  apply_rules [ DifferentiableAt.add, DifferentiableAt.smul, differentiableAt_id, differentiableAt_const ];
  · apply_rules [ DifferentiableAt.mul, DifferentiableAt.sqrt ] <;> norm_num;
    · apply_rules [ DifferentiableAt.add, DifferentiableAt.mul, differentiableAt_id, differentiableAt_const ];
      · have h_diff : DifferentiableAt ℝ fdPolygon t := by
          apply_rules [ fdPolygon_differentiableAt_off_partition ];
        exact Complex.reCLM.differentiableAt.comp t h_diff;
      · have h_diff : DifferentiableAt ℝ fdPolygon t := by
          apply_rules [ fdPolygon_differentiableAt_off_partition ];
        exact Complex.reCLM.differentiableAt.comp t h_diff;
      · have h_diff : DifferentiableAt ℝ fdPolygon t := by
          apply_rules [ fdPolygon_differentiableAt_off_partition ];
        exact Complex.imCLM.differentiableAt.comp t h_diff;
      · have h_diff : DifferentiableAt ℝ fdPolygon t := by
          apply_rules [ fdPolygon_differentiableAt_off_partition ];
        exact Complex.imCLM.differentiableAt.comp t h_diff;
    · exact sub_ne_zero_of_ne <| by intro h; have := fdPolygon_avoids_interior p hp_norm hp_re hp_im t ⟨ ht.1.le, ht.2.le ⟩ ; aesop;
  · refine' DifferentiableAt.div _ _ _;
    · exact DifferentiableAt.sub ( fdPolygon_differentiableAt_off_partition t ht ht_not_P ) ( differentiableAt_const _ );
    · have h_diff : DifferentiableAt ℝ (fun t' => ‖fdPolygon t' - p‖) t := by
        have h_diff : DifferentiableAt ℝ (fun t' => fdPolygon t' - p) t := by
          exact DifferentiableAt.sub ( fdPolygon_differentiableAt_off_partition t ht ht_not_P ) ( differentiableAt_const _ );
        have h_diff_norm : DifferentiableAt ℝ (fun t' => ‖fdPolygon t' - p‖^2) t := by
          simpa only [ sq ] using h_diff.norm_sq ℝ;
        have h_diff_norm : DifferentiableAt ℝ (fun t' => Real.sqrt (‖fdPolygon t' - p‖^2)) t := by
          refine' DifferentiableAt.sqrt h_diff_norm _;
          have h_norm_ne_zero : fdPolygon t ≠ p := by
            apply fdPolygon_avoids_interior p hp_norm hp_re hp_im t (Set.Ioo_subset_Icc_self ht);
          exact ne_of_gt ( sq_pos_of_pos ( norm_pos_iff.mpr ( sub_ne_zero.mpr h_norm_ne_zero ) ) );
        simpa only [ Real.sqrt_sq ( norm_nonneg _ ) ] using h_diff_norm;
      exact Complex.ofRealCLM.differentiableAt.comp _ h_diff;
    · apply_fun Complex.re at * ; norm_num at *;
      exact sub_ne_zero_of_ne <| by exact fun h => by have := fdPolygon_avoids_interior p hp_norm hp_re ( by linarith ) t ⟨ by linarith, by linarith ⟩ ; aesop;

-- Technical: composition of differentiable functions

/- Condition 7: t-derivative is continuous on each piece. -/
noncomputable section AristotleLemmas

/-
The derivative of fdPolygon is continuous on any interval avoiding the partition points.
-/
lemma deriv_fdPolygon_continuousOn_piece (p₁ p₂ : ℝ) (hp₁p₂ : p₁ < p₂)
    (hpiece : ∀ t ∈ Ioo p₁ p₂, t ∉ ({1, 2, 3, 4} : Finset ℝ))
    (h_sub : Ioo p₁ p₂ ⊆ Ioo 0 5) :
    ContinuousOn (deriv fdPolygon) (Ioo p₁ p₂) := by
      -- Since the interval (p₁, p₂) is a subset of one of the intervals (0,1), (1,2), (2,3), (3,4), or (4,5), on each of these intervals, `fdPolygon` is define as an affine function (either a vertical/horizontal line or a chord segment).
      by_cases h_cases : Set.Ioo p₁ p₂ ⊆ Set.Ioo 0 1 ∨ Set.Ioo p₁ p₂ ⊆ Set.Ioo 1 2 ∨ Set.Ioo p₁ p₂ ⊆ Set.Ioo 2 3 ∨ Set.Ioo p₁ p₂ ⊆ Set.Ioo 3 4 ∨ Set.Ioo p₁ p₂ ⊆ Set.Ioo 4 5;
      · -- Since `fdPolygon` is affine on each of these intervals, its derivative is constant.
        have h_affine_derivative : ∀ t ∈ Set.Ioo 0 1, deriv fdPolygon t = -(H_height - Real.sqrt 3 / 2) * I := by
          intro t ht; rw [ show deriv fdPolygon t = deriv ( fun t : ℝ => 1 / 2 + ( H_height - t * ( H_height - Real.sqrt 3 / 2 ) ) * Complex.I ) t by exact Filter.EventuallyEq.deriv_eq <| Filter.eventuallyEq_of_mem ( Ioo_mem_nhds ht.1 ht.2 ) fun x hx => if_pos hx.2.le ] ; norm_num [ Complex.ext_iff ] ;
          erw [ deriv_const_sub ] ; norm_num [ Complex.ext_iff ] ; ring_nf ; norm_num;
          erw [ Complex.ofRealCLM.deriv ] ; norm_num
        have h_affine_derivative2 : ∀ t ∈ Set.Ioo 1 2, deriv fdPolygon t = i_point - rho' := by
          intros t ht
          have h_affine_derivative2 : deriv fdPolygon t = deriv (fun t => chordSegment rho' i_point (t - 1)) t := by
            refine' Filter.EventuallyEq.deriv_eq _;
            filter_upwards [ Ioo_mem_nhds ht.1 ht.2 ] with x hx using by rw [ show fdPolygon x = chordSegment rho' i_point ( x - 1 ) by exact if_neg ( by linarith [ hx.1, hx.2 ] ) |> fun h => h.trans ( if_pos <| by linarith [ hx.1, hx.2 ] ) ] ;
          rw [ h_affine_derivative2 ];
          unfold chordSegment; norm_num [ Complex.ext_iff ] ; ring;
          erw [ HasDerivAt.deriv ( HasDerivAt.add ( HasDerivAt.add ( HasDerivAt.neg ( HasDerivAt.mul ( hasDerivAt_id' t |> HasDerivAt.ofReal_comp ) ( hasDerivAt_const _ _ ) ) ) ( HasDerivAt.mul ( hasDerivAt_id' t |> HasDerivAt.ofReal_comp ) ( hasDerivAt_const _ _ ) ) ) ( hasDerivAt_const _ _ ) ) ] ; norm_num [ Complex.ext_iff ] ; ring;
          decide +kernel
        have h_affine_derivative3 : ∀ t ∈ Set.Ioo 2 3, deriv fdPolygon t = rho - i_point := by
          intro t ht; rw [ show deriv fdPolygon t = deriv ( fun t => chordSegment i_point rho ( t - 2 ) ) t from _ ] ; norm_num [ chordSegment ] ;
          · convert HasDerivAt.deriv ( HasDerivAt.add ( HasDerivAt.mul ( HasDerivAt.sub ( hasDerivAt_const _ _ ) ( HasDerivAt.sub ( HasDerivAt.ofReal_comp ( hasDerivAt_id' t ) ) ( hasDerivAt_const _ _ ) ) ) ( hasDerivAt_const _ _ ) ) ( HasDerivAt.mul ( HasDerivAt.sub ( HasDerivAt.ofReal_comp ( hasDerivAt_id' t ) ) ( hasDerivAt_const _ _ ) ) ( hasDerivAt_const _ _ ) ) ) using 1 ; norm_num ; ring;
          · exact Filter.EventuallyEq.deriv_eq ( Filter.eventuallyEq_of_mem ( Ioo_mem_nhds ht.1 ht.2 ) fun x hx => by rw [ show fdPolygon x = chordSegment i_point rho ( x - 2 ) by exact if_neg ( by linarith [ hx.1, hx.2 ] ) |> fun h => h.trans ( by rw [ if_neg ( by linarith [ hx.1, hx.2 ] ) ] ; rw [ if_pos ( by linarith [ hx.1, hx.2 ] ) ] ) ] )
        have h_affine_derivative4 : ∀ t ∈ Set.Ioo 3 4, deriv fdPolygon t = (H_height - Real.sqrt 3 / 2) * I := by
          -- Since `fdPolygon` is affine on the interval (3, 4), its derivative is constant.
          intros t ht
          have h_affine : deriv (fun t => -1 / 2 + (Real.sqrt 3 / 2 + (t - 3) * (H_height - Real.sqrt 3 / 2)) * I) t = (H_height - Real.sqrt 3 / 2) * I := by
            convert HasDerivAt.deriv ( HasDerivAt.add ( hasDerivAt_const _ _ ) ( HasDerivAt.mul ( HasDerivAt.add ( hasDerivAt_const _ _ ) ( HasDerivAt.mul ( hasDerivAt_id' t |> HasDerivAt.sub <| hasDerivAt_const _ _ ) <| hasDerivAt_const _ _ ) ) <| hasDerivAt_const _ _ ) ) using 1 ; norm_num;
            · exact 0;
            · exact 0;
            · exact 0;
            · exact 0;
            · exact 0;
          rw [ show deriv fdPolygon t = deriv ( fun t : ℝ => -1 / 2 + ( Real.sqrt 3 / 2 + ( t - 3 ) * ( H_height - Real.sqrt 3 / 2 ) ) * Complex.I ) t from ?_ ] ; norm_num [ h_affine ] ; ring;
          · erw [ Complex.ofRealCLM.deriv ] ; norm_num;
          · exact Filter.EventuallyEq.deriv_eq ( by filter_upwards [ Ioo_mem_nhds ht.1 ht.2 ] with x hx using by rw [ show fdPolygon x = -1 / 2 + ( Real.sqrt 3 / 2 + ( x - 3 ) * ( H_height - Real.sqrt 3 / 2 ) ) * Complex.I from by rw [ fdPolygon ] ; rw [ if_neg ( by linarith [ hx.1 ] ), if_neg ( by linarith [ hx.1, hx.2 ] ), if_neg ( by linarith [ hx.1, hx.2 ] ), if_pos ( by linarith [ hx.1, hx.2 ] ) ] ] )
        have h_affine_derivative5 : ∀ t ∈ Set.Ioo 4 5, deriv fdPolygon t = 1 := by
          intro t ht; rw [ show deriv fdPolygon t = deriv ( fun t : ℝ => ( t - 9 / 2 ) + H_height * I ) t from _ ] ; norm_num [ Complex.ext_iff, sub_eq_add_neg ] ;
          · erw [ Complex.ofRealCLM.deriv ] ; norm_num;
          · refine' Filter.EventuallyEq.deriv_eq _ ; filter_upwards [ Ioo_mem_nhds ht.1 ht.2 ] with u hu ; rw [ show fdPolygon u = ( u - 9 / 2 ) + H_height * Complex.I from _ ] ; norm_num [ Complex.ext_iff, sub_eq_add_neg ] ; ring;
            unfold fdPolygon; norm_num [ hu.1.le, hu.2.le ] ; ring;
            split_ifs <;> norm_num <;> linarith [ hu.1, hu.2 ] ;
        rcases h_cases with ( h_cases | h_cases | h_cases | h_cases | h_cases ) <;> [ exact ContinuousOn.congr ( continuousOn_const ) fun t ht => h_affine_derivative _ ( h_cases ht ) ; exact ContinuousOn.congr ( continuousOn_const ) fun t ht => h_affine_derivative2 _ ( h_cases ht ) ; exact ContinuousOn.congr ( continuousOn_const ) fun t ht => h_affine_derivative3 _ ( h_cases ht ) ; exact ContinuousOn.congr ( continuousOn_const ) fun t ht => h_affine_derivative4 _ ( h_cases ht ) ; exact ContinuousOn.congr ( continuousOn_const ) fun t ht => h_affine_derivative5 _ ( h_cases ht ) ] ;
      · -- Since the interval (p₁, p₂) is connected and avoids the partition points {1, 2, 3, 4}, it must be a subset of one of the intervals (0,1), (1,2), (2,3), (3,4), or (4,5).
        have h_connected : ∀ t ∈ Set.Ioo p₁ p₂, t < 1 ∨ (1 < t ∧ t < 2) ∨ (2 < t ∧ t < 3) ∨ (3 < t ∧ t < 4) ∨ (4 < t) := by
          grind;
        have h_connected : ∀ t ∈ Set.Ioo p₁ p₂, ∀ u ∈ Set.Ioo p₁ p₂, (t < 1 ↔ u < 1) ∧ ((1 < t ∧ t < 2) ↔ (1 < u ∧ u < 2)) ∧ ((2 < t ∧ t < 3) ↔ (2 < u ∧ u < 3)) ∧ ((3 < t ∧ t < 4) ↔ (3 < u ∧ u < 4)) ∧ ((4 < t) ↔ (4 < u)) := by
          intros t ht u hu
          have h_connected : ∀ x ∈ Set.Ioo p₁ p₂, ∀ y ∈ Set.Ioo p₁ p₂, x < y → (x < 1 → y < 1) ∧ (1 < x ∧ x < 2 → 1 < y ∧ y < 2) ∧ (2 < x ∧ x < 3 → 2 < y ∧ y < 3) ∧ (3 < x ∧ x < 4 → 3 < y ∧ y < 4) ∧ (4 < x → 4 < y) := by
            intros x hx y hy hxy
            have h_connected : ∀ z ∈ Set.Ioo x y, z ∉ ({1, 2, 3, 4} : Finset ℝ) := by
              exact fun z hz => hpiece z ⟨ by linarith [ hz.1, hx.1 ], by linarith [ hz.2, hy.2 ] ⟩ |> fun h => by simpa using h;
          ...
          by_cases htu : t < u;
          · grind;
          · by_cases htu : u < t <;> simp_all +decide [ lt_asymm ] ; (
            grind);
            grind)
        generalize_proofs at *; (
        grind)
...
/-
If a curve is C1 and non-zero, its normalized direction has a continuous derivative.
-/
lemma deriv_normalized_continuous {γ : ℝ → ℂ} {S : Set ℝ}
    (hS : IsOpen S)
    (hγ_diff : ∀ t ∈ S, DifferentiableAt ℝ γ t)
    (hγ_deriv_cont : ContinuousOn (deriv γ) S)
    (hγ_ne : ∀ t ∈ S, γ t ≠ 0) :
    ContinuousOn (deriv (fun t => γ t / ‖γ t‖)) S := by
      refine' ContinuousOn.congr _ _;
      exact fun t => ( deriv γ t / ‖γ t‖ ) - ( γ t * ( ( deriv γ t |> Complex.re ) * ( γ t |> Complex.re ) + ( deriv γ t |> Complex.im ) * ( γ t |> Complex.im ) ) ) / ( ‖γ t‖ ^ 3 );
      · refine' ContinuousOn.sub _ _;
        · exact ContinuousOn.div hγ_deriv_cont ( Complex.continuous_ofReal.comp_continuousOn ( continuous_norm.comp_continuousOn ( show ContinuousOn γ S from fun t ht => DifferentiableAt.continuousAt ( hγ_diff t ht ) |> ContinuousAt.continuousWithinAt ) ) ) fun t ht => Complex.ofReal_ne_zero.mpr ( norm_ne_zero_iff.mpr ( hγ_ne t ht ) );
        · refine' ContinuousOn.div _ _ _;
          · refine' ContinuousOn.mul _ _;
            · exact fun t ht => DifferentiableAt.continuousAt ( hγ_diff t ht ) |> ContinuousAt.continuousWithinAt;
            · exact ContinuousOn.add ( ContinuousOn.mul ( Complex.continuous_ofReal.comp_continuousOn ( Complex.continuous_re.comp_continuousOn hγ_deriv_cont ) ) ( Complex.continuous_ofReal.comp_continuousOn ( Complex.continuous_re.comp_continuousOn ( show ContinuousOn γ S from fun t ht => DifferentiableAt.continuousAt ( hγ_diff t ht ) |> ContinuousAt.continuousWithinAt ) ) ) ) ( ContinuousOn.mul ( Complex.continuous_ofReal.comp_continuousOn ( Complex.continuous_im.comp_continuousOn hγ_deriv_cont ) ) ( Complex.continuous_ofReal.comp_continuousOn ( Complex.continuous_im.comp_continuousOn ( show ContinuousOn γ S from fun t ht => DifferentiableAt.continuousAt ( hγ_diff t ht ) |> ContinuousAt.continuousWithinAt ) ) ) );
          · exact ContinuousOn.pow ( Complex.continuous_ofReal.comp_continuousOn ( ContinuousOn.norm ( show ContinuousOn γ S from fun t ht => DifferentiableAt.continuousAt ( hγ_diff t ht ) |> ContinuousAt.continuousWithinAt ) ) ) _;
          · exact fun t ht => pow_ne_zero _ <| Complex.ofReal_ne_zero.mpr <| ne_of_gt <| norm_pos_iff.mpr <| hγ_ne t ht;
      · intro t ht; have := hγ_diff t ht; have := this.hasDerivAt; simp_all +decide [ hasDerivAt_iff_tendsto_slope_zero ] ;
        convert HasDerivAt.deriv ( HasDerivAt.div ( hγ_diff t ht |> DifferentiableAt.hasDerivAt ) ( HasDerivAt.ofReal_comp ( HasDerivAt.norm_sq ( hγ_diff t ht |> DifferentiableAt.hasDerivAt ) |> HasDerivAt.sqrt <| ?_ ) ) <| ?_ ) using 1 <;> norm_num [ hγ_ne t ht ];
        · rfl;
        · field_simp
...
lemma polygonToCircleRadial_deriv_cont_on_piece (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  have h_deriv_radial : ContinuousOn (fun (q : ℝ × ℝ) => deriv (fun t' => polygonToCircleRadial p (t', q.2)) q.1) (Set.Ioo p₁ p₂ ×ˢ Set.Icc 0 1) := by
    have h_diff : ∀ t ∈ Set.Ioo p₁ p₂, DifferentiableAt ℝ (fun t' => fdPolygon t' - p) t := by
      exact fun t ht => DifferentiableAt.sub ( fdPolygon_differentiableAt_off_partition t ( h_sub ht ) ( hpiece t ht ) ) ( differentiableAt_const _ )
    have h_cont_diff : ContinuousOn (fun t => deriv (fun t' => fdPolygon t' - p) t) (Set.Ioo p₁ p₂) := by
      have h_cont_diff : ContinuousOn (fun t => deriv fdPolygon t) (Set.Ioo p₁ p₂) := by
        apply_rules [ deriv_fdPolygon_continuousOn_piece ];
      exact h_cont_diff.congr fun x hx => by rw [ deriv_sub_const ] ;
    have h_norm_cont : ContinuousOn (fun t => ‖fdPolygon t - p‖) (Set.Ioo p₁ p₂) := by
      exact ContinuousOn.norm ( ContinuousOn.sub ( fdPolygon_continuous.continuousOn ) continuousOn_const )
    have h_norm_ne_zero : ∀ t ∈ Set.Ioo p₁ p₂, ‖fdPolygon t - p‖ ≠ 0 := by
      exact fun t ht => ne_of_gt <| norm_pos_iff.mpr <| sub_ne_zero.mpr <| fdPolygon_avoids_interior p hp_norm hp_re hp_im t <| h_sub ht |> Set.Ioo_subset_Icc_self
    have h_deriv_normalized : ContinuousOn (fun t => deriv (fun t' => (fdPolygon t' - p) / ‖fdPolygon t' - p‖) t) (Set.Ioo p₁ p₂) := by
      apply_rules [ deriv_normalized_continuous ];
      · exact isOpen_Ioo;
      · exact fun t ht => sub_ne_zero_of_ne <| by specialize h_norm_ne_zero t ht; aesop;
    have h_deriv_radial : ∀ q : ℝ × ℝ, q.1 ∈ Set.Ioo p₁ p₂ → q.2 ∈ Set.Icc 0 1 → deriv (fun t' => polygonToCircleRadial p (t', q.2)) q.1 = ((1 - q.2) * deriv (fun t' => ‖fdPolygon t' - p‖) q.1) • ((fdPolygon q.1 - p) / ‖fdPolygon q.1 - p‖) + ((1 - q.2) * ‖fdPolygon q.1 - p‖ + q.2) • deriv (fun t' => (fdPolygon t' - p) / ‖fdPolygon t' - p‖) q.1 := by
      intros q hq₁ hq₂
      have h_deriv_radial : deriv (fun t' => polygonToCircleRadial p (t', q.2)) q.1 = deriv (fun t' => p + ((1 - q.2) * ‖fdPolygon t' - p‖ + q.2) • ((fdPolygon t' - p) / ‖fdPolygon t' - p‖)) q.1 := by
        exact?;
      convert HasDerivAt.deriv ( HasDerivAt.const_add p <| HasDerivAt.smul ( HasDerivAt.add ( HasDerivAt.const_mul ( 1 - q.2 ) <| hasDerivAt_deriv_iff.mpr <| show DifferentiableAt ℝ ( fun t' => ‖fdPolygon t' - p‖ ) q.1 from ?_ ) <| hasDerivAt_const _ _ ) <| hasDerivAt_deriv_iff.mpr <| show DifferentiableAt ℝ ( fun t' => ( fdPolygon t' - p ) / ( ‖fdPolygon t' - p‖ : ℂ ) ) q.1 from ?_ ) using 1;
      · norm_num [ add_comm, add_left_comm, add_assoc ];
      · have h_diff_norm : DifferentiableAt ℝ (fun t' => ‖fdPolygon t' - p‖) q.1 := by
          have h_diff : DifferentiableAt ℝ (fun t' => fdPolygon t' - p) q.1 := h_diff q.1 hq₁
          exact DifferentiableAt.norm ℝ h_diff ( by simpa using h_norm_ne_zero q.1 hq₁ );
        exact h_diff_norm;
      · exact DifferentiableAt.div ( h_diff _ hq₁ ) ( Complex.ofRealCLM.differentiableAt.comp _ ( DifferentiableAt.norm ℝ ( h_diff _ hq₁ ) ( by aesop ) ) ) ( by aesop );
    have h_deriv_norm_cont : ContinuousOn (fun t => deriv (fun t' => ‖fdPolygon t' - p‖) t) (Set.Ioo p₁ p₂) := by
      have h_deriv_norm_cont : ∀ t ∈ Set.Ioo p₁ p₂, deriv (fun t' => ‖fdPolygon t' - p‖) t = (Complex.re (starRingEnd ℂ (fdPolygon t - p) * deriv (fun t' => fdPolygon t' - p) t)) / ‖fdPolygon t - p‖ := by
        intro t ht;
        convert HasDerivAt.deriv ( HasDerivAt.sqrt ( HasDerivAt.norm_sq ( h_diff t ht |> DifferentiableAt.hasDerivAt ) ) ?_ ) using 1 <;> norm_num [ h_norm_ne_zero t ht ];
        ring;
      rw [ continuousOn_congr fun t ht => h_deriv_norm_cont t ht ];
      refine' ContinuousOn.div _ _ _;
      · refine' ContinuousOn.comp ( Complex.continuous_re.comp_continuousOn _ ) _ _;
        exact Set.univ;
        · exact continuousOn_id;
        · refine' ContinuousOn.mul _ h_cont_diff;
          exact ContinuousOn.star ( ContinuousOn.sub ( fdPolygon_continuous.continuousOn ) continuousOn_const );
        · exact Set.mapsTo_univ _ _;
      · exact h_norm_cont;
      · exact h_norm_ne_zero;
    refine' ContinuousOn.congr _ fun q hq => h_deriv_radial q hq.1 hq.2;
    refine' ContinuousOn.add _ _;
    · refine' ContinuousOn.smul _ _;
      · exact ContinuousOn.mul ( continuousOn_const.sub continuousOn_snd ) ( h_deriv_norm_cont.comp continuousOn_fst fun x hx => hx.1 );
      · refine' ContinuousOn.div _ _ _;
        · exact ContinuousOn.sub ( ContinuousOn.comp ( show ContinuousOn ( fun t => fdPolygon t ) ( Set.Ioo 0 5 ) from Continuous.continuousOn ( by exact? ) ) ( continuousOn_fst ) fun x hx => h_sub hx.1 ) continuousOn_const;
        · exact Complex.continuous_ofReal.comp_continuousOn ( h_norm_cont.comp continuousOn_fst fun x hx => hx.1 );
        · exact fun x hx => mod_cast h_norm_ne_zero x.1 hx.1;
    · refine' ContinuousOn.smul _ _;
      · exact ContinuousOn.add ( ContinuousOn.mul ( continuousOn_const.sub continuousOn_snd ) ( h_norm_cont.comp continuousOn_fst fun x hx => hx.1 ) ) continuousOn_snd;
      · exact h_deriv_normalized.comp ( continuousOn_fst ) fun x hx => hx.1;
  convert h_deriv_radial using 1

/- Aristotle failed to find a proof. -/
-- Technical: continuity of derivative formula
...
-- Technical: explicit bound computation
...
lemma fdPolygonRadialCircle_wrapCount (p : ℂ) (hp_norm : ‖p‖ > 1)
  ...
  negate_state;
  -- Proof starts here:
  refine' ⟨ 1 / 4 + 1.1 * Complex.I, _, _, _, _ ⟩ <;> norm_num [ Complex.normSq, Complex.norm_def ];
  · rw [ one_lt_div ] <;> norm_num [ Real.sqrt_lt ];
  · exact show 11 / 10 < Real.sqrt 3 / 2 + 1 by nlinarith [ Real.sqrt_nonneg 3, Real.sq_sqrt zero_le_three ] ;
  · unfold fdPolygonRadialCircle_angle;
    unfold fdPolygonRadialCircle angleOnCircle; norm_num [ Complex.normSq, Complex.norm_def, Complex.arg ] ; ring_nf ; norm_num [ Real.pi_pos.ne' ] ;
    unfold polygonToCircleRadial; norm_num [ Complex.normSq, Complex.norm_def, Complex.exp_re, Complex.exp_im ] ; ring_nf ; norm_num [ Real.pi_pos.ne' ] ;
    unfold fdPolygon; norm_num [ Complex.ext_iff, Complex.exp_re, Complex.exp_im, Complex.cos, Complex.sin ] ; ring_nf ; norm_num [ Real.pi_pos.ne' ] ;
...
-- CORE MATHEMATICAL CONTENT: wrap count analysis
...
/- Aristotle ran out of time. -/
-- Technical: continuity of angle functions
...
/- Aristotle ran out of time. -/
-- Technical: winding number depends only on total angle change
...
/- Aristotle ran out of time. -/
-- Technical: wrap count matching ensures closedness
...
/- Aristotle ran out of time. -/
-- Technical: differentiability of angle interpolation
...
/- Aristotle ran out of time. -/
-- Technical: continuity of derivative
...
/- Aristotle ran out of time. -/
-- Technical: bounded derivative on compact domain
