...
- Done: A-1, A-1b, A-2, A-2-wrapper, B-1, **B-2/B-5/B-6 fully closed**, B-3, B-4, **D-1 (a/b/c/d)**, **W-0..W-5**, **C-1**, **C-2 ALL Steps**, **higher-order avoidance closed**, **Phase 3.1-3.8** (k-odd transverse), **ExitTime.lean (full two-sided API)**, **HW33ExitTimeWrapper.lean**, **C-3 avoidance**, **C-4 avoidance closed** + auto-discharge, **HW33Bridge.lean** (cpvIntegrandOn ↔ excision integral bridge with shape from strict mono), **HW33Monotonicity.lean** (strict mono of `‖γ-s‖` from transverse data), **HW33Final.lean** (single-pole transverse via `HasCauchyPVOn`), **HW33MultiPole.lean** (multi-pole composition via pole-set extension under avoidance), **CLEANUP-B (Dixon files, -35 lines)**
- Open: CLEANUP-C (HigherOrderCancel.lean, in progress), CLEANUP-FINAL, k-even sector PV under angle (B) for the curve case (genuinely incomplete — requires C-2 Step C/D homotopy comparison or new direct argument)
