...
- Done: A-1, A-1b, A-2, A-2-wrapper, B-1 (partial + cocompact-bounded + continuity), **B-2/B-5/B-6 fully closed for arbitrary open U**, B-3, B-4, **D-1 (a/b/c/d all done)**, **W-0..W-5 (entire W-stream — B-1 full closed)**, **C-1**, **C-2 ALL Steps (A, B, C, D)**, **higher-order avoidance closed**, **Phase 3.1-3.8 (analytical kernel + main asymptotic theorems for HW Theorem 3.3 higher-order, k-odd transverse)**, **ExitTime.lean (full two-sided API: existence, exact-radius equality, continuity modulus, witness bounds, monotonicity, asymptotic Tendsto, eventual radius equality, HW33ExitData bundle)**, **HW33ExitTimeWrapper.lean (bundled + concrete forms of `hw_theorem_3_3_odd_transverse_*`)**, **C-3 avoidance form (`hasCauchyPVOn_finset_pow_inv_of_avoids`, `HasCauchyPVOn.finset_sum`, `contourIntegral_finset_sum`)**, **C-4 avoidance form (`generalizedResidueTheorem_higherOrder_avoids_closed`)** — composes B-6 with C-3
- Open: CLEANUP-B, CLEANUP-C, CLEANUP-FINAL, HW 3.3 final statement composition (transverse-crossing case), k-even sector PV under angle (B) for the curve
