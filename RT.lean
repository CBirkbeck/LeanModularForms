...
This project request had uuid: 83c202b7-2f61-4c5b-92c2-d7cbcc334ef1
...
theorem classicalResidueTheorem
  ...
  have := @generalizedResidueTheorem';
  specialize this U hU S hS_in_U (fun s hs => by
    have h_finite : Set.Finite (S \ {s}) := by
      exact S.finite_toSet.subset fun x hx => hx.1;
    have h_min_dist : ∃ ε > 0, ∀ s' ∈ (S \ {s} : Set ℂ), ε ≤ ‖s' - s‖ := by
      have h_nonempty : (S \ {s} : Set ℂ).Nonempty → ∃ ε > 0, ∀ s' ∈ (S \ {s} : Set ℂ), ε ≤ ‖s' - s‖ := by
        intros h_nonempty
        obtain ⟨ε, hε_pos, hε⟩ : ∃ ε > 0, ∀ s' ∈ (S \ {s} : Set ℂ), ε ≤ ‖s' - s‖ := by
          have h_min_dist : ∃ m ∈ (S \ {s} : Set ℂ), ∀ s' ∈ (S \ {s} : Set ℂ), ‖s' - s‖ ≥ ‖m - s‖ := by
            apply_rules [ Set.exists_min_image ]
          obtain ⟨ m, hm₁, hm₂ ⟩ := h_min_dist; exact ⟨ ‖m - s‖, norm_pos_iff.mpr ( sub_ne_zero.mpr <| by aesop ), hm₂ ⟩ ;
        exact ⟨ε, hε_pos, hε⟩
        skip
      by_cases h_empty : (S \ {s} : Set ℂ).Nonempty;
      · exact h_nonempty h_empty;
      · exact ⟨ 1, zero_lt_one, fun s' hs' => False.elim <| h_empty ⟨ s', hs' ⟩ ⟩;
    exact ⟨ h_min_dist.choose, h_min_dist.choose_spec.1, fun s' hs' hs'' => h_min_dist.choose_spec.2 s' ⟨ hs', hs'' ⟩ ⟩) (by
  exact S.finite_toSet.isClosed) f hf γ hγ_closed hγ_in_U S (fun s hs => by
    exact hs) (fun t ht hst => by
    exact hst) (fun s hs t ht hst => by
    exact False.elim <| hγ_avoids_S s hs t ht hst);
  ...
  -- Since γ is piecewise C¹ and avoids S, the distance from γ(t) to any s in S is positive. Therefore, there exists a minimum distance δ > 0 such that for all t in [a, b], ‖γ(t) - s‖ ≥ δ for all s in S.
  obtain ⟨δ, hδ_pos, hδ⟩ : ∃ δ > 0, ∀ t ∈ Set.Icc γ.a γ.b, ∀ s ∈ S, δ ≤ ‖γ.toFun t - s‖ := by
    have h_dist_pos : ∀ s ∈ S, ∃ δ > 0, ∀ t ∈ Set.Icc γ.a γ.b, δ ≤ ‖γ.toFun t - s‖ := by
      intro s hs
      have h_dist_pos : ∃ δ > 0, ∀ t ∈ Set.Icc γ.a γ.b, δ ≤ ‖γ.toFun t - s‖ := by
        have h_cont : ContinuousOn (fun t => ‖γ.toFun t - s‖) (Set.Icc γ.a γ.b) := by
          have h_cont : ContinuousOn γ.toFun (Set.Icc γ.a γ.b) := by
            exact?;
          exact ContinuousOn.norm ( h_cont.sub continuousOn_const )
        have := IsCompact.exists_isMinOn ( CompactIccSpace.isCompact_Icc ) ⟨ γ.a, Set.left_mem_Icc.mpr γ.hab.le ⟩ h_cont;
        exact ⟨ ‖γ.toFun this.choose - s‖, norm_pos_iff.mpr ( sub_ne_zero.mpr ( hγ_avoids_S s hs _ this.choose_spec.1 ) ), fun t ht => this.choose_spec.2 ht ⟩;
      exact h_dist_pos;
    choose! δ hδ_pos hδ using h_dist_pos;
    by_cases hS_empty : S = ∅;
    · exact ⟨ 1, zero_lt_one, by aesop ⟩;
    · exact ⟨ Finset.min' ( S.image δ ) ⟨ _, Finset.mem_image_of_mem δ ( Classical.choose_spec ( Finset.nonempty_of_ne_empty hS_empty ) ) ⟩, by have := Finset.min'_mem ( S.image δ ) ⟨ _, Finset.mem_image_of_mem δ ( Classical.choose_spec ( Finset.nonempty_of_ne_empty hS_empty ) ) ⟩ ; aesop, fun t ht s hs => Finset.min'_le _ _ ( Finset.mem_image_of_mem δ hs ) |> le_trans <| hδ s hs t ht ⟩;
  refine' ⟨ δ, hδ_pos, fun y hy₁ hy₂ => _ ⟩;
  rw [ intervalIntegral.integral_congr fun t ht => if_neg <| by rintro ⟨ s, hs, hs' ⟩ ; linarith [ hδ t ( by simpa [ γ.hab.le ] using ht ) s hs, abs_lt.mp hy₁ ] ]
